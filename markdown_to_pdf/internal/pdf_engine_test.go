...
// Test_Pdf_Fixedpoint_Adapters protects the typed boundary because all parsed PDF coordinates
// cross these adapters.
func Test_Pdf_Fixedpoint_Adapters(t *testing.T) {
	t.Parallel()
	integer := pdf_fixed_from_integer(2)
	if integer != fixedpoint.Number(2*fixedpoint.SCALE) {
		t.Fatalf("integer = %d, want %d", integer, 2*fixedpoint.SCALE)
	}
	ratio := pdf_fixed_from_ratio(fixedpoint.Numerator(1), fixedpoint.Denominator(2))
	if ratio != fixedpoint.Number(fixedpoint.SCALE/2) {
		t.Fatalf("ratio = %d, want %d", ratio, fixedpoint.SCALE/2)
	}
	product := pdf_fixed_multiply(&Pdf_Fixed_Multiply_Input{
		Left:  integer,
		Right: ratio,
	})
	if product != fixedpoint.Number(fixedpoint.SCALE) {
		t.Fatalf("product = %d, want %d", product, fixedpoint.SCALE)
	}
}
...
func Test_Pdf_Form_Table_Continuation_Rows(t *testing.T) {
	...
	content, is_form := pdf_form_content(&Pdf_Form_Content_Input{
		Words: words, Page_Width: pdf_fixed_from_integer(612),
		Page_Height: pdf_fixed_from_integer(792),
	})
	if !is_form {
...
func Test_Pdf_Form_Table_Wide_Continuation_Row(t *testing.T) {
	...
	columns := []fixedpoint.Number{
		pdf_fixed_from_integer(72), pdf_fixed_from_integer(172),
		pdf_fixed_from_integer(272), pdf_fixed_from_integer(372),
		pdf_fixed_from_integer(472), pdf_fixed_from_integer(552),
	}
...
func Test_Pdf_Ruled_Table_Top_Glyph_Overlap(t *testing.T) {
	...
	table := Pdf_Ruled_Table{
		Top: pdf_fixed_from_integer(100), Bottom: pdf_fixed_from_integer(130),
		Rows: []Pdf_Ruled_Row{{
			Top: pdf_fixed_from_integer(100), Bottom: pdf_fixed_from_integer(130),
			Cells: []Pdf_Rectangle{
				{X0: pdf_fixed_from_integer(40), X1: pdf_fixed_from_integer(140),
					Top:    pdf_fixed_from_integer(100),
					Bottom: pdf_fixed_from_integer(130)},
				{X0: pdf_fixed_from_integer(140), X1: pdf_fixed_from_integer(300),
					Top:    pdf_fixed_from_integer(100),
					Bottom: pdf_fixed_from_integer(130)},
			},
		}},
	...
	second_table := Pdf_Ruled_Table{
		Top: pdf_fixed_from_integer(150), Bottom: pdf_fixed_from_integer(180),
		Rows: []Pdf_Ruled_Row{{
			Top: pdf_fixed_from_integer(150), Bottom: pdf_fixed_from_integer(180),
			Cells: []Pdf_Rectangle{
				{X0: pdf_fixed_from_integer(40), X1: pdf_fixed_from_integer(140),
					Top:    pdf_fixed_from_integer(150),
					Bottom: pdf_fixed_from_integer(180)},
				{X0: pdf_fixed_from_integer(140), X1: pdf_fixed_from_integer(300),
					Top:    pdf_fixed_from_integer(150),
					Bottom: pdf_fixed_from_integer(180)},
			},
...
func pdf_test_table_word(input *pdf_test_table_word_input) (word Pdf_Word) {
	...
	word.X0 = pdf_fixed_from_integer(input.X)
	word.X1 = pdf_fixed_from_integer(input.X + int64(len(input.Text))*5)
	word.Top = pdf_fixed_from_integer(input.Top)
	word.Bottom = pdf_fixed_from_integer(input.Top + 10)
	return word
}
...
func pdf_test_integer(value int) (text string) {
	...
	const PDF_TEST_INTEGER_CAPACITY = 20
	if value == 0 {
		return "0"
	}
	var reversed [PDF_TEST_INTEGER_CAPACITY]byte
	count := len(reversed)
	for value > 0 {
		count--
...
func Test_Main_Direction_And_Paths(t *testing.T) {
	...
	if filepath.Ext(main_preview_path(&Main_Preview_Path_Input{
...
func Test_Main_Preview_Document(t *testing.T) {
	...
	pdf_document, pdf_status := main_preview_document(input, &Main_Preview_Document_Input{
	...
	failed_document, failed_status := main_preview_document(input, &Main_Preview_Document_Input{
...
func Test_Main_Password_Usage(t *testing.T) {
	...
	for _, password := range []string{"secret"} {
		...
		if status := main_render_command(input, render); status != EXIT_USAGE {
	...
	if main_conversion_error_status(Pdf_Incorrect_Password_Error{}) !=
		EXIT_USAGE {
	...
	for _, password := range []string{"secret"} {
		...
		if status := main_preview(input, preview); status != EXIT_USAGE {
...
func Test_Main_Incorrect_Password_Preserves_Output(t *testing.T) {
	...
	status := main_convert_to_path_with_password(
		input,
		&Main_Convert_To_Path_With_Password_Input{
	...
	if main_path_exists(input, absent_path) {
...
func Test_Main_Output_Preservation(t *testing.T) {
	...
	status := main_convert_to_path(input, []byte("not a PDF"), true, path)
...
func Test_Main_Conversion_Directions(t *testing.T) {
	...
	if status := main_convert_to_path(input, []byte("# Hello"), false, pdf_path); status != 0 {
	...
	source := read_test_file(t, "testdata/movie_booking.pdf")
	markdown_path := filepath.Join(directory, "extracted.md")
	if status := main_convert_to_path(input, source, true, markdown_path); status != 0 {
		t.Fatalf("PDF status = %d", status)
	}
	want := read_test_file(t, "testdata/movie_booking.golden")
...
func Test_Main_Overwrite_Statuses(t *testing.T) {
	...
	if status := main_render_command(input, command); status != EXIT_EXISTS {
	...
	if _, write_err := source.Write(
		read_test_file(t, "testdata/movie_booking.pdf"),
	); write_err != nil {
	...
	if status := main_render_command(input, command); status != 0 {
	...
	if status := main_render_command(input, command); status != EXIT_FAILURE {
...
// The host-backed test input preserves the command's dependency boundary while
// the existing transaction tests continue to exercise real file behavior.
func main_test_input(t *testing.T) (input *Main_Input) {
	t.Helper()
	return &Main_Input{
		Output:       io.Discard,
