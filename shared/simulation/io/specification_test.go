...
func Test_Stream_Derived(t *testing.T) {
	full := make([]byte, 6)
	count, err := io.Read_At_Least(memory_stream([]byte("abcdef")), full, int64(len(full)))
	if err != nil {
		t.Fatalf("read at least full error: %v", err)
	}
	if count != 6 {
		t.Fatalf("read at least full moved %d bytes, want 6", count)
	}
	short := make([]byte, 8)
	count, err = io.Read_At_Least(memory_stream([]byte("abcdef")), short, int64(len(short)))
	if err != io.Stream_Unexpected_EOF {
		t.Fatalf("read past the end reported %v, want Stream_Unexpected_EOF", err)
	}
	if count != 6 {
		t.Fatalf("read past the end moved %d bytes, want 6", count)
	}
...
// Test_Stream_Read_Full verifies the rearm loop reads a whole file across passes and stops
// on a pass with no bytes instead of spinning.
func Test_Stream_Read_Full(t *testing.T) {
	loop, driver := sim_timeline(1)
	writer := sim_create(t, loop, driver, "full")
	wrote := false
	...
	reader := sim_open(t, loop, driver, "full")
	// The buffer is larger than the file, so the first pass is short and the rearm loop
	// takes a second pass that yields no bytes — the stop this test exists to witness.
	buffer := make([]byte, 8)
	count := -1
	var completion time.Completion
	io.Read_Full(loop, &completion, func(_ *time.Completion, bytes int, err error) {
		if err != nil {
	...
	if count != 5 {
		t.Fatalf("read full moved %d bytes, want 5", count)
	}
	if string(buffer[:count]) != "hello" {
		t.Fatalf("read full read %q, want hello", buffer[:count])
	}
}

// Test_Stream_Write_Full verifies the rearm loop persists a whole buffer and delivers one
// callback with the count.
func Test_Stream_Write_Full(t *testing.T) {
	loop, driver := sim_timeline(1)
	writer := sim_create(t, loop, driver, "full")
	count := -1
	var completion time.Completion
	io.Write_Full(loop, &completion, func(_ *time.Completion, bytes int, err error) {
		if err != nil {
	...
	driver.Run_Until(func() (finished bool) { return count >= 0 }, SIM_DEADLINE)
	if count != 5 {
		t.Fatalf("write full moved %d bytes, want 5", count)
	}
	...
	io.Read_Full(loop, &read_completion, func(_ *time.Completion, bytes int, _ error) {
		read = bytes
	}, reader, buffer, 0)
	driver.Run_Until(func() (finished bool) { return read >= 0 }, SIM_DEADLINE)
	if string(buffer[:read]) != "hello" {
		t.Fatalf("write full persisted %q, want hello", buffer[:read])
	}
}

// The Run_Until cap for the loop-backed tests: generous, since a completion returns the pump
// the instant it fires — this bound only bites a genuine stall.
const SIM_DEADLINE = 4096 * time.NANOSECOND

// Builds a simulated IO surface beside the timeline that owns its order, seeded by seed.
func sim_timeline(seed uint64) (loop nbio.IO, driver time.Driver) {
	pump, driver, clock := time.New_Virtual_Timeline(
		time.Virtual_Clock{Resolution: time.NANOSECOND})
	return nbio.New_Simulated_IO(seed, pump, clock), driver
}

// Opens path through Open_At with the given options, driving until the descriptor arrives.
func sim_open_options(
	...
	opened := nbio.File(-1)
	var completion time.Completion
	loop.Open_At(&completion, func(_ *time.Completion, ready nbio.File, err error) {
		if err != nil {
			t.Fatalf("open at %q: %v", path, err)
		}
		opened = ready
	}, nbio.DIRECTORY_CURRENT, path, options)
	driver.Run_Until(func() (finished bool) { return opened >= 0 }, SIM_DEADLINE)
	return opened
}

// Creates a fresh writable file at path on the simulated tree.
func sim_create(
...
// Opens an existing file at path for reading on the simulated tree.
func sim_open(
	t *testing.T, loop nbio.IO, driver time.Driver, path string,
) (file nbio.File) {
	t.Helper()
