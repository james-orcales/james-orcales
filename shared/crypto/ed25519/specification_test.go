...
import (
	standard_ed25519 "crypto/ed25519"
	"testing"
...
func Test_Keys(t *testing.T) {
	...
	public_key := ed25519.Public_Key_From_Private(&private_key)
	testify.Equal(t, vector_public_key(), public_key)
}

// Test_Signatures matches RFC 8032 and both verification implementations.
func Test_Signatures(t *testing.T) {
	...
	testify.Equal(t, vector_signature(), signature)
	testify.True(t, bool(ed25519.Verify(public_key, nil, signature[:])))
	testify.True(t, standard_ed25519.Verify(
		standard_ed25519.PublicKey(public_key[:]), nil, signature[:],
	))
	signature[len(signature)-binary.UINT_8_SIZE] ^= byte(binary.UINT_8_SIZE)
	testify.False(t, bool(ed25519.Verify(public_key, nil, signature[:])))

	message := []byte("bounded Ed25519")
	ed25519.Sign(&signature, &private_key, message)
	testify.True(t, standard_ed25519.Verify(
		standard_ed25519.PublicKey(public_key[:]), message, signature[:],
	))
	standard_private_key := standard_ed25519.NewKeyFromSeed(seed[:])
	standard_signature := standard_ed25519.Sign(standard_private_key, message)
	testify.True(t, bool(ed25519.Verify(
		public_key, message, standard_signature,
	)))
}
...
func Test_Bounds(t *testing.T) {
	...
	testify.Panics(t, func() {
		ed25519.Verify(public_key, message_oversized[:], signature[:])
	})
...
func Test_Allocation(t *testing.T) {
	seed := vector_seed()
	var private_key ed25519.Private_Key
	var public_key ed25519.Public_Key
	var signature ed25519.Signature
	...
	testify.Zero_Allocation(t, func() {
		verified = ed25519.Verify(public_key, []byte("allocation"), signature[:])
	})
...
func Test_Invariant_Domains(t *testing.T) {
	...
	for _, size := range [...]int{
		...
		ed25519.Verify(public_key, message[:size], signature[:])
	}
