...
// Test_Construction verifies New, the zero List value, and Initialize.
func Test_Construction(t *testing.T) {
	t.Parallel()
	check_list(t, empty_list(), []int{})
	zero := zero_list()
	list.Push_Back(zero, 1)
	check_list(t, zero, []int{1})
	list.Initialize(zero)
	check_list(t, zero, []int{})
}
...
func Test_Handles(t *testing.T) {
	...
	testify.Equal(t, 1, list.Value_At(subject, kept),
...
func Test_Traversal(t *testing.T) {
	...
	testify.Equal(t, 2, list.Value_At(subject, second),
...
func Test_Removal(t *testing.T) {
	...
	list.Push_Back(subject, 2)
	testify.Equal(t, 1, list.Remove(subject, handle(first)),
...
type list_factory func() (subject *list.List[int])

// Names each list state that the declared domains need, from a zero value to a list whose
// elements sit at the highest positions of a full pool.
func probe_states() (states []list_factory) {
	return []list_factory{
		zero_list,
		empty_list,
		func() (subject *list.List[int]) { return fill_list(1) },
		func() (subject *list.List[int]) { return fill_list(2) },
		func() (subject *list.List[int]) { return fill_list(3) },
		func() (subject *list.List[int]) {
...
// Makes a zero List value, which holds no sentinel.
func zero_list() (subject *list.List[int]) {
	return &list.List[int]{}
}

// Makes an empty list that holds both sentinels.
func empty_list() (subject *list.List[int]) {
...
func reuse_list() (subject *list.List[int]) {
	subject = fill_list(2)
...
func nearly_full_list() (subject *list.List[int]) {
...
func high_reversed_list() (subject *list.List[int]) {
...
func high_front_list() (subject *list.List[int]) {
...
func high_pair_list() (subject *list.List[int]) {
...
func drain_below(kept list.Position) (subject *list.List[int]) {
	subject = fill_list(list.ELEMENT_COUNT_MAXIMUM)
...
func fill_list(element_count int) (subject *list.List[int]) {
	subject = empty_list()
	for value := range element_count {
		list.Push_Back(subject, value)
	}
	return subject
}
...
func check_list(t *testing.T, subject *list.List[int], wanted []int) {
	t.Helper()
	...
	for position != list.POSITION_NONE {
		forward = append(forward, list.Value_At(subject, position))
	...
	for position != list.POSITION_NONE {
		backward = append(backward, list.Value_At(subject, position))
