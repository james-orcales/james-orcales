...
// DISTANCE_EXTRA_SYMBOL_GROUP_SHIFT divides paired distance symbols.
const DISTANCE_EXTRA_SYMBOL_GROUP_SHIFT = 1
...
// FIXED_FIRST_CODE_BASE is canonical base for first fixed-tree range.
const FIXED_FIRST_CODE_BASE = 0x30

// FIXED_SECOND_CODE_BASE is canonical base for second fixed-tree range.
const FIXED_SECOND_CODE_BASE = 0x190

// FIXED_FOURTH_CODE_BASE is canonical base for fourth fixed-tree range.
const FIXED_FOURTH_CODE_BASE = 0xc0
...
// Pointer equality finds aliases without unsafe address arithmetic.
...
func matching_size(
	...
	for index := Matching_Size(0); index < Matching_Size(maximum); index++ {
...
func sequence_hash(
	...
	value := uint32(0)
	for index := 0; index < HASH_INPUT_BYTE_COUNT; index++ {
...
func Decode_Into(
	...
	Destination_Unvalidated_Invariants(destination, "Decode_Into.destination")
	Compressed_Unvalidated_Invariants(compressed, "Decode_Into.compressed")
	return Decode_Dictionary_Into(destination, compressed, nil)
}
...
func decode_block(
	...
	case BLOCK_KIND_FIXED:
		var literal_sizes [FIXED_LITERAL_COUNT]uint8
		for index := 0; index <= FIXED_LITERAL_FIRST_MAXIMUM; index++ {
			literal_sizes[index] = FIXED_CODE_SIZE
		}
		second_maximum := FIXED_LITERAL_SECOND_MAXIMUM
		for index := FIXED_LITERAL_SECOND_MINIMUM; index <= second_maximum; index++ {
			literal_sizes[index] = FIXED_SECOND_CODE_SIZE
		}
		third_maximum := FIXED_LITERAL_THIRD_MAXIMUM
		for index := FIXED_LITERAL_THIRD_MINIMUM; index <= third_maximum; index++ {
			literal_sizes[index] = FIXED_THIRD_CODE_SIZE
		}
		for index := FIXED_LITERAL_FOURTH_MINIMUM; index < len(literal_sizes); index++ {
			literal_sizes[index] = FIXED_CODE_SIZE
		}
		var literal_decoder Huffman_Decoder
		if !huffman_build(&literal_decoder, Code_Sizes(literal_sizes[:])) {
			return count, STATUS_INPUT_INVALID
		}
		var distance_sizes [FIXED_DISTANCE_COUNT]uint8
		for index := range distance_sizes {
			distance_sizes[index] = FIXED_DISTANCE_BIT_COUNT
		}
		var distance_decoder Huffman_Decoder
		if !huffman_build(&distance_decoder, Code_Sizes(distance_sizes[:])) {
			return count, STATUS_INPUT_INVALID
		}
...
func decode_huffman(
	...
	for more := Boolean(true); more; {
		...
		count, match_status = decode_match(
			reader,
			destination, dictionary, count, Match_Symbol(symbol),
			distance_decoder,
		)
...
func decode_match(
	...
	Huffman_Decoder_Invariants(
		distance_decoder, "decode_match.distance_decoder",
	)
...
func distance_code_values(
	...
	extra_count = Distance_Extra_Bit_Count(
		(int(symbol) - DISTANCE_EXTRA_SYMBOL_OFFSET) >> DISTANCE_EXTRA_SYMBOL_GROUP_SHIFT,
	)
...
func huffman_read(
	...
	for index := 1; index <= int(decoder.Maximum_Code_Size); index++ {
		bit, bit_available := bit_reader_read(reader, 1)
		if !bit_available {
			return 0, false
		}
		code |= bit
		...
		if code < first+count {
			position := symbol_index + code - first
			if position >= Bit_Value(decoder.Symbol_Count) {
				return 0, false
			}
			return Symbol(decoder.Symbols[position]), true
		}
		symbol_index += count
