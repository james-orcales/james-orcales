...
func Test_All_Runtime_Entry_Points_Have_Zero_Allocations(t *testing.T) {
	...
	testify.Zero_Allocation(t, func() {
		invariant.Recorder_Always(recorder, true, "always")
		invariant.Recorder_Sometimes(recorder, true, "sometimes")
		assert_inline_ranges(recorder)
		assert_inline_holed_ranges(recorder)
		assert_inline_enums(recorder)
		builder := invariant.Recorder_Tree(
...
func assert_inline_ranges(recorder *invariant.Recorder) {
	invariant.Recorder_Range(recorder, 5, 0, 10, "range int")
	invariant.Recorder_Range(recorder, int8(5), int8(0), int8(10), "range int8")
	invariant.Recorder_Range(recorder, int16(5), int16(0), int16(10), "range int16")
	invariant.Recorder_Range(recorder, int32(5), int32(0), int32(10), "range int32")
	invariant.Recorder_Range(recorder, int64(5), int64(0), int64(10), "range int64")
	invariant.Recorder_Range(recorder, uint(5), uint(0), uint(10), "range uint")
	invariant.Recorder_Range(recorder, uint8(5), uint8(0), uint8(10), "range uint8")
	invariant.Recorder_Range(recorder, uint16(5), uint16(0), uint16(10), "range uint16")
	invariant.Recorder_Range(recorder, uint32(5), uint32(0), uint32(10), "range uint32")
	invariant.Recorder_Range(recorder, uint64(5), uint64(0), uint64(10), "range uint64")
}

func assert_inline_holed_ranges(recorder *invariant.Recorder) {
	invariant.Recorder_Range_Holed(recorder, 5, 0, 10, 1, 2, 3, 4, "holed int")
	invariant.Recorder_Range_Holed(recorder,
...
func assert_inline_enums(recorder *invariant.Recorder) {
	invariant.Recorder_Enum(recorder, 1, 1, 2, "enum int")
	invariant.Recorder_Enum(recorder, int8(1), int8(1), int8(2), "enum int8")
	invariant.Recorder_Enum(recorder, int16(1), int16(1), int16(2), "enum int16")
	invariant.Recorder_Enum(recorder, int32(1), int32(1), int32(2), "enum int32")
	invariant.Recorder_Enum(recorder, int64(1), int64(1), int64(2), "enum int64")
	invariant.Recorder_Enum(recorder, uint(1), uint(1), uint(2), "enum uint")
	invariant.Recorder_Enum(recorder, uint8(1), uint8(1), uint8(2), "enum uint8")
	invariant.Recorder_Enum(recorder, uint16(1), uint16(1), uint16(2), "enum uint16")
	invariant.Recorder_Enum(recorder, uint32(1), uint32(1), uint32(2), "enum uint32")
	invariant.Recorder_Enum(recorder, uint64(1), uint64(1), uint64(2), "enum uint64")
}

func assert_builder_ranges(
	builder invariant.Assertion_Builder,
) (next invariant.Assertion_Builder) {
