...
// Recorder_Always's policy; a message registration never seeded no-ops inside recorder_increment.
func Recorder_Sometimes[T ~bool](recorder *Recorder, condition T, message string) {
	...
	recorder_increment(recorder, message, bool(condition))
}

// Recorder_Range is the bare bounds guard: eager like Always, panicking with the value and the
// violated bound. Trailing exclusions are holes inside the interval the value must also avoid.
// The formatting and the hole loop are outlined so the passing path stays under the inliner's
// budget — a hole-less call pays one len check on a nil slice.
func Recorder_Range[T Integer](recorder *Recorder, value T, minimum T, maximum T, excluded ...T) {
	if value < minimum {
		range_minimum_panic(value, minimum)
	}
	if value > maximum {
		range_maximum_panic(value, maximum)
	}
	if len(excluded) != 0 {
		range_excluded(value, excluded)
	}
}

func range_excluded[T Integer](value T, excluded []T) {
	for _, hole := range excluded {
		if value == hole {
			panic(ASSERTION_FAILURE_MESSAGE_PREFIX + "Range — value " +
				integer_text(value) + " is excluded")
		}
	}
}

// Recorder_Enum is the bare membership guard: the value must equal one of members. The panic text
// lists the members so a failure names the whole legal set; the member loop already keeps this
// body out of the inliner, so the formatting stays inline on the failure path.
func Recorder_Enum[T Integer](recorder *Recorder, value T, members ...T) {
	...
	panic(ASSERTION_FAILURE_MESSAGE_PREFIX + "Enum — value " + integer_text(value) +
		" is not among the members " + members_text)
}
...
func range_minimum_panic[T Integer](value T, minimum T) {
	panic(ASSERTION_FAILURE_MESSAGE_PREFIX + "Range — value " + integer_text(value) +
		" is below the minimum " + integer_text(minimum))
}

func range_maximum_panic[T Integer](value T, maximum T) {
	panic(ASSERTION_FAILURE_MESSAGE_PREFIX + "Range — value " + integer_text(value) +
		" is above the maximum " + integer_text(maximum))
}
