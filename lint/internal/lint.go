...
func diagnostic_within_scope(d Diagnostic, scope_prefix string) (within bool) {
	...
	tier_boundary := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
		X: d.Tier, Lo: 0, Hi: tier_max, Message: "Tier is in [0,2]",
	})
...
func check_shadows_function_body(file_set *token.FileSet, global_scope *scope, function *ast.FuncDecl, diags *[]Diagnostic) {
	...
	if function.Body != nil {
		check_shadows_function_body_walk_body(file_set, function_scope, function.Body.List, diags)
	}
}
...
func scope_new_block(parent *scope) (new_scope *scope) {
	defer func() {
		// New_scope.Parent is `parent`; the entry asserts parent.Parent is
		// non-nil, so new_scope.Parent.Parent inherits that guarantee. The
		// names axis is bounded by AST budget; constructor returns with the
		// freshly-made empty map so Lo=0 is invariant — paired with the
		// parent_parent_parent_nil axis to keep Hi unreachable in the
		// observable space (a brand-new map can never be at the safety cap).
...
func check_shadows_function_body_walk_body_walk_statement_declaration_statement(
	...
	generic_declarationaration, ok := x.Decl.(*ast.GenDecl)
	if !ok {
		return
	}
	for _, specification := range generic_declarationaration.Specs {
...
func Check_File(file_set *token.FileSet, file *ast.File, source []byte) (diags []Diagnostic) {
	defer func() {
		...
		// Hi(diags) = max_diagnostics_per_call and Hi(source) =
		// max_file_size_bytes are the per-file budget caps — they bound
		// runaway input rather than mark a legitimate working endpoint.
		// (Lo diags, Hi source) is also logically impossible for a Go
		// source at the byte cap: a non-trivial Go file in that range
		// would necessarily fire at least one diagnostic shape (eg. a
		// non-asserted parameter), so 0 diags is unreachable from
		// realistic inputs even before the budget argument applies.
		invariant.Cross_Product(
	...
	invariant.Cross_Product(
		...
		// Hi(source) = max_file_size_bytes is the per-file budget cap, so
		// the (Hi source, *) tuples mark runaway-input bounds rather than
...
func check_casing(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	defer func() {
		...
		// Tests don't carry budget-sized decl lists or diag lists; (Hi, *)
...
func check_function_line_count(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	defer func() {
		diags_boundary := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(diags), Lo: 0, Hi: max_diagnostics_per_call,
			Message: "Diags is bounded by per-file diag budget",
...
func git_input_check_short_hash(h string) (s string) {
	...
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X:       len(h),
			Lo:      0,
			Hi:      git_full_hash_chars_sha_256,
...
func check_no_recursion(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	invariant.Cross_Product(invariant.Sometimes(graph.Caller_Order == nil, "Caller_order is empty for files with zero func decls"))
...
func build_file_call_graph(
	...
	defer func() {
		...
		invariant.Cross_Product(caller_order_axis, decls_axis, edges_axis,
			invariant.Excluding("Caller_Order and Decls track in lockstep — Hi one, Lo other is impossible",
				invariant.Bucket_Hi(caller_order_axis), invariant.Bucket_Lo(decls_axis)),
			invariant.Excluding("Caller_Order and Decls track in lockstep — Hi one, Lo other is impossible",
				invariant.Bucket_Lo(caller_order_axis), invariant.Bucket_Hi(decls_axis)),
			invariant.Excluding("Caller_Order at AST safety cap unreachable in test corpus (Lo edges)",
				invariant.Bucket_Hi(caller_order_axis), invariant.Bucket_Lo(edges_axis)),
			invariant.Excluding("Caller_Order at AST safety cap unreachable in test corpus (Hi edges)",
				invariant.Bucket_Hi(caller_order_axis), invariant.Bucket_Hi(edges_axis)),
			invariant.Excluding("Decls at AST safety cap unreachable in test corpus (Lo edges)",
				invariant.Bucket_Hi(decls_axis), invariant.Bucket_Lo(edges_axis)),
			invariant.Excluding("Decls at AST safety cap unreachable in test corpus (Hi edges)",
				invariant.Bucket_Hi(decls_axis), invariant.Bucket_Hi(edges_axis)),
			invariant.Excluding("Edges at AST safety cap unreachable in test corpus (Lo callers)",
				invariant.Bucket_Hi(edges_axis), invariant.Bucket_Lo(caller_order_axis)),
		)
	}()
	invariant.Cross_Product(
		invariant.Always(file_set != nil, "File_set is non-nil"),
...
func (v *recursion_visitor) Visit(n ast.Node) (next ast.Visitor) {
	defer func() {
		scopes_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Scopes is bounded by AST budget"})
		e := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*v.Edges),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Edges is bounded by AST budget"})
		caller_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Caller),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "V.Caller is a non-empty function name bounded by identifier budget"})
		targets_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Targets),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call,
			Message: "V.Targets is non-empty by construction (visitor created per function) and bounded by AST budget"})
		history_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Push_History is bounded by AST budget"})
		invariant.Cross_Product(scopes_axis, e, caller_axis, targets_axis, history_axis,
			...
			invariant.Excluding("Scopes at safety cap with zero edges unreachable in test corpus",
				invariant.Bucket_Hi(scopes_axis), invariant.Bucket_Lo(e)),
			invariant.Excluding("Zero scopes with edges at safety cap unreachable in test corpus",
				invariant.Bucket_Lo(scopes_axis), invariant.Bucket_Hi(e)),
			invariant.Excluding("Hi targets at AST safety cap unreachable in test corpus (Lo scopes)",
				invariant.Bucket_Hi(targets_axis), invariant.Bucket_Lo(scopes_axis)),
			invariant.Excluding("Hi targets at AST safety cap unreachable in test corpus (Hi scopes)",
				invariant.Bucket_Hi(targets_axis), invariant.Bucket_Hi(scopes_axis)),
			invariant.Excluding("Hi push_history at AST safety cap unreachable in test corpus (Lo scopes)",
				invariant.Bucket_Hi(history_axis), invariant.Bucket_Lo(scopes_axis)),
			invariant.Excluding("Hi push_history at AST safety cap unreachable in test corpus (Hi scopes)",
	...
	invariant.Cross_Product(invariant.Sometimes(pushed == 0, "Pushed can be zero on this branch"))
...
func recursion_visitor_enter(v *recursion_visitor, n ast.Node) (pushed int) {
	defer func() {
		pb_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: pushed, Lo: 0, Hi: count_one, Message: "Pushed is either 0 or 1"})
		pz_axis := invariant.Sometimes(pushed == 0, "Pushed is zero for nodes that leave the scope stack untouched")
		e := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*v.Edges),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Edges is bounded by AST budget"})
		caller_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Caller),
			Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "V.Caller is non-empty bounded"})
		targets_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Targets),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Targets is non-empty by construction"})
		history_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Push_History is bounded by AST budget"})
		sd := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
			Lo: count_one, Hi: max_ast_nodes_per_call, Message: "V.Scopes spans 1 to AST budget"})
		so := invariant.Sometimes(len(v.Scopes) == count_one, "V.Scopes has exactly 1 entry sometimes")
		invariant.Cross_Product(
			pb_axis, pz_axis, e, caller_axis, targets_axis, history_axis, sd, so,
			invariant.Excluding("Zero pushed pz true", invariant.Bucket_Lo(pb_axis), invariant.Bucket_False(pz_axis)),
			invariant.Excluding("Max pushed pz true", invariant.Bucket_Hi(pb_axis), invariant.Bucket_True(pz_axis)),
			invariant.Excluding("Zero pushed Hi edges", invariant.Bucket_Lo(pb_axis), invariant.Bucket_Hi(e)),
			invariant.Excluding("Max pushed Hi edges", invariant.Bucket_Hi(pb_axis), invariant.Bucket_Hi(e)),
			invariant.Excluding("Hi targets Lo pushed", invariant.Bucket_Hi(targets_axis), invariant.Bucket_Lo(pb_axis)),
			invariant.Excluding("Hi targets Hi pushed", invariant.Bucket_Hi(targets_axis), invariant.Bucket_Hi(pb_axis)),
			invariant.Excluding("Hi history Lo pushed", invariant.Bucket_Hi(history_axis), invariant.Bucket_Lo(pb_axis)),
			invariant.Excluding("Hi history Hi pushed", invariant.Bucket_Hi(history_axis), invariant.Bucket_Hi(pb_axis)),
			invariant.Excluding("First Visit pushes a frame",
				invariant.Bucket_Lo(pb_axis), invariant.Bucket_Lo(history_axis)),
			invariant.Excluding("Hi sd with one true", invariant.Bucket_Hi(sd), invariant.Bucket_True(so)),
			invariant.Excluding("Hi sd with one false", invariant.Bucket_Hi(sd), invariant.Bucket_False(so)),
			invariant.Excluding("Lo sd implies one true", invariant.Bucket_Lo(sd), invariant.Bucket_False(so)),
		)
	}()
	e := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*v.Edges),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Edges is bounded by AST budget"})
	scopes_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Scopes is bounded by AST budget"})
	p_caller := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Caller),
		Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "V.Caller is non-empty function name bounded"})
	p_targets := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Targets),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Targets is non-empty by construction"})
	p_history := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Push_History is bounded by AST budget"})
	invariant.Cross_Product(
		invariant.Always(v != nil, "V is non-nil"),
		invariant.Always(v.File_Set != nil, "V.File_Set is non-nil"),
		...
		invariant.Excluding("Hi scopes Hi edges bad", invariant.Bucket_Hi(scopes_axis), invariant.Bucket_Hi(e)),
		invariant.Excluding("Hi scopes Lo edges bad", invariant.Bucket_Hi(scopes_axis), invariant.Bucket_Lo(e)),
		invariant.Excluding("Hi targets Lo scopes bad", invariant.Bucket_Hi(p_targets), invariant.Bucket_Lo(scopes_axis)),
		invariant.Excluding("Hi targets Hi scopes bad", invariant.Bucket_Hi(p_targets), invariant.Bucket_Hi(scopes_axis)),
		invariant.Excluding("Hi history Lo scopes bad", invariant.Bucket_Hi(p_history), invariant.Bucket_Lo(scopes_axis)),
		invariant.Excluding("Hi history Hi scopes bad", invariant.Bucket_Hi(p_history), invariant.Bucket_Hi(scopes_axis)),
		invariant.Excluding("Lo scopes Hi edges bad", invariant.Bucket_Lo(scopes_axis), invariant.Bucket_Hi(e)),
	)
...
func recursion_visitor_enter_define_statement(v *recursion_visitor, s ast.Stmt) {
	defer func() {
		...
		caller_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Caller),
			Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "V.Caller is non-empty bounded by identifier budget"})
		targets_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Targets),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Targets is non-empty by construction"})
		scopes_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
			Lo: min_pair, Hi: max_ast_nodes_per_call,
			Message: "V.Scopes is ≥2 (Walk's BlockStmt push + caller's for/if push)"})
		history_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call,
			Message: "V.Push_History is ≥1 (Visit pushed the containing BlockStmt before reaching for/if)"})
		invariant.Cross_Product(e, s_nil, caller_axis, targets_axis, scopes_axis, history_axis,
...
func recursion_visitor_define_ident(v *recursion_visitor, e ast.Expr) {
	defer func() {
		edges_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*v.Edges),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Edges is bounded by AST budget"})
		...
		targets_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Targets),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Targets is non-empty by construction"})
		scopes_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Scopes is non-empty (define requires a scope)"})
		history_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Push_History is non-empty (define needs history)"})
		invariant.Cross_Product(edges_axis, e_nil, caller_axis, targets_axis, scopes_axis, history_axis,
			...
			invariant.Excluding("E_nil true comes only from RangeStmt which appends a scope before calling",
...
func recursion_visitor_enter_record_call_edge(v *recursion_visitor, call *ast.CallExpr) {
	defer func() {
		...
		targets_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Targets),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Targets is non-empty"})
		scopes_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "V.Scopes is ≥1 (Walk pushed BlockStmt scope)"})
		...
		history_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
			Lo: min_pair, Hi: max_ast_nodes_per_call, Message: "V.Push_History is ≥2 at this site"})
		caller_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Caller),
			Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "V.Caller is non-empty bounded by identifier budget"})
		invariant.Cross_Product(e, function_ident_axis, targets_axis, scopes_axis, history_axis, caller_axis,
			invariant.Excluding("Edges at safety cap with bare-ident call unreachable in test corpus",
				invariant.Bucket_Hi(e), invariant.Bucket_True(function_ident_axis)),
			invariant.Excluding("Edges at safety cap with selector call unreachable in test corpus",
			...
			invariant.Excluding("Hi targets unreachable (bare-ident call)",
				invariant.Bucket_Hi(targets_axis), invariant.Bucket_True(function_ident_axis)),
			invariant.Excluding("Hi targets unreachable (selector call)",
				invariant.Bucket_Hi(targets_axis), invariant.Bucket_False(function_ident_axis)),
			invariant.Excluding("Hi scopes unreachable (bare-ident call)",
				invariant.Bucket_Hi(scopes_axis), invariant.Bucket_True(function_ident_axis)),
			invariant.Excluding("Hi scopes unreachable (selector call)",
				invariant.Bucket_Hi(scopes_axis), invariant.Bucket_False(function_ident_axis)),
			invariant.Excluding("Hi push_history unreachable (bare-ident call)",
				invariant.Bucket_Hi(history_axis), invariant.Bucket_True(function_ident_axis)),
			invariant.Excluding("Hi push_history unreachable (selector call)",
				invariant.Bucket_Hi(history_axis), invariant.Bucket_False(function_ident_axis)),
		)
	}()
	e := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*v.Edges),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "V.Edges is bounded by AST budget"})
	...
	scopes_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Scopes),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call,
		Message: "V.Scopes is ≥1 (Walk(Body) pushed BlockStmt scope before reaching CallExpr)"})
	...
	history_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Push_History),
		Lo: min_pair, Hi: max_ast_nodes_per_call,
		Message: "V.Push_History is ≥2 at this site (BlockStmt push + at least one non-pushing descent to reach CallExpr)"})
	caller_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(v.Caller),
		Lo: min_non_empty, Hi: Max_Identifier_Chars,
		Message: "V.Caller is a non-empty function name bounded by identifier budget"})
	invariant.Cross_Product(
...
func check_no_recursion_find_cycles(
	...
	defer func() {
		...
		invariant.Cross_Product(d, c, a,
			invariant.Excluding("Diags at diag-budget safety cap with callers at slice-budget at safety cap is bad",
				invariant.Bucket_Hi(d), invariant.Bucket_Hi(c)),
			invariant.Excluding("Diags at diag-budget at safety cap is bad",
				invariant.Bucket_Hi(d), invariant.Bucket_Lo(c)),
			invariant.Excluding("Callers at slice-budget safety cap with adj at AST at safety cap is bad",
	...
	for _, start := range callers {
		...
		diags = append(diags, check_no_recursion_find_cycles_dfs(file_set, start, color, adj)...)
	}
	return diags
}
...
func check_no_recursion_find_cycles_dfs(
	...
	defer func() {
		// Color always contains at least start (set to gray below) by exit;
		// adj is bounded by budget at exit (unchanged from input).
		co := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(color), Lo: min_non_empty,
	...
	co := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(color), Lo: 0,
		Hi: max_ast_nodes_per_call, Message: "Color is bounded by budget"})
	a := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(adj), Lo: 0,
		Hi: max_ast_nodes_per_call, Message: "Adj is bounded by budget"})
...
func check_no_recursion_find_cycles_dfs_diag(
	...
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(cycle_nodes),
			Lo: min_non_empty, Hi: max_string_slice_per_call, Message: "Cycle_nodes is ≥1 per caller gate"})
		a := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(adj), Lo: min_non_empty,
			Hi: max_ast_nodes_per_call, Message: "Adj is ≥1 (cycle path requires at least one edge)"})
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Name),
			Lo: 0, Hi: Max_Identifier_Chars, Message: "Diag.Name is empty for recursion diagnostics"})
		want_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Want),
			Lo: 0, Hi: Max_Identifier_Chars, Message: "Diag.Want is empty for recursion diagnostics"})
		message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Message),
			Lo: min_recursion_message_chars, Hi: max_recursion_message_chars,
			Message: "Diag.Message is `recursion: <node> calls itself`"})
		invariant.Cross_Product(c, a, name_axis, want_axis, message_axis,
			invariant.Always(diag.Tier == 0, "Tier is 0 at diagnostic construction (set later by tier dispatcher)"),
			invariant.Excluding("Cycle_nodes at slice-budget safety cap with adj at AST at safety cap is bad",
				invariant.Bucket_Hi(c), invariant.Bucket_Hi(a)),
			invariant.Excluding("Cycle_nodes at slice-budget at safety cap is bad",
				invariant.Bucket_Hi(c), invariant.Bucket_Lo(a)),
			invariant.Excluding("Adj at AST at safety cap is bad",
				invariant.Bucket_Lo(c), invariant.Bucket_Hi(a)),
			invariant.Excluding("Hi name unreachable (Lo c)", invariant.Bucket_Hi(name_axis), invariant.Bucket_Lo(c)),
			invariant.Excluding("Hi name unreachable (Hi c)", invariant.Bucket_Hi(name_axis), invariant.Bucket_Hi(c)),
			invariant.Excluding("Hi want unreachable (Lo c)", invariant.Bucket_Hi(want_axis), invariant.Bucket_Lo(c)),
			invariant.Excluding("Hi want unreachable (Hi c)", invariant.Bucket_Hi(want_axis), invariant.Bucket_Hi(c)),
		)
	}()
	a := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(adj), Lo: min_non_empty,
		Hi: max_ast_nodes_per_call, Message: "Adj is ≥1 (cycle path requires at least one edge)"})
	c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(cycle_nodes),
		Lo: min_non_empty, Hi: max_string_slice_per_call, Message: "Cycle_nodes is ≥1 per caller gate"})
	caller := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(back_edge.Caller),
		Lo: min_non_empty, Hi: Max_Identifier_Chars,
		Message: "Back_edge.Caller is a non-empty function name"})
	callee := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(back_edge.Callee),
		Lo: min_non_empty, Hi: Max_Identifier_Chars,
		Message: "Back_edge.Callee is a non-empty function name"})
	invariant.Cross_Product(
		invariant.Always(file_set != nil, "File_set is non-nil"),
		invariant.Always(!back_edge.Shadowed, "Back_edge is unshadowed (asserted by callers)"),
		a, c, caller, callee,
		invariant.Excluding("Adj at AST safety cap with cycle_nodes at slice-budget at safety cap is bad",
		...
		invariant.Excluding("Hi caller unreachable (Hi a)", invariant.Bucket_Hi(caller), invariant.Bucket_Hi(a)),
		invariant.Excluding("Hi callee unreachable (Hi a)", invariant.Bucket_Hi(callee), invariant.Bucket_Hi(a)),
		invariant.Excluding("Self-cycle requires caller=callee (Hi caller Lo callee)",
			invariant.Bucket_Lo(c), invariant.Bucket_Hi(caller), invariant.Bucket_Lo(callee)),
		invariant.Excluding("Self-cycle requires caller=callee (Lo caller Hi callee)",
			invariant.Bucket_Lo(c), invariant.Bucket_Lo(caller), invariant.Bucket_Hi(callee)),
	)
	...
	for i_index := 0; i_index < len(cycle_nodes)-1; i_index++ {
		...
		for _, ce := range adj[from] {
			if ce.Callee == to {
				invariant.Cross_Product(invariant.Always(!ce.Shadowed, "Check_no_recursion: cycle edge shadowed"))
				break
			}
		}
	}
	invariant.Cross_Product(invariant.Always(!back_edge.Shadowed, "Check_no_recursion: back-edge shadowed"))
	return Diagnostic{
...
func check_no_recursion_find_cycles_dfs_diag_message(cycle_nodes []string) (message string) {
	defer func() {
		...
		message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(message), Lo: min_recursion_message_chars, Hi: max_recursion_message_chars,
			Message: "Message is `recursion: <node> calls itself` (25 for 1-char node, 152 for 128-char node)",
		})
	...
	c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(cycle_nodes),
		Lo: min_non_empty, Hi: max_string_slice_per_call, Message: "Cycle_nodes is ≥1 per caller gate"})
	single_axis := invariant.Sometimes(len(cycle_nodes) == count_one, "Cycle_nodes is exactly 1 sometimes (self-cycle)")
...
func check_no_discard_all_blank_exprs(exprs []ast.Expr) (all bool) {
	defer func() {
		e := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(exprs),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Exprs is non-empty in observed paths"})
	...
	e := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(exprs),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Exprs is non-empty in observed paths"})
	single_axis := invariant.Sometimes(len(exprs) == count_one, "Exprs has exactly one entry sometimes")
...
func check_no_discard_all_blank_idents(names []*ast.Ident) (all bool) {
	defer func() {
		n := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(names),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Names is non-empty in observed paths"})
		all_axis := invariant.Sometimes(all, "Every entry passed the check")
	...
	n := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(names),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Names is non-empty in observed paths"})
	single_axis := invariant.Sometimes(len(names) == count_one, "Names has exactly one entry sometimes")
...
func check_public_struct_fields_named(file_set *token.FileSet, identifier *ast.Ident, diags *[]Diagnostic) {
	defer func() {
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(identifier.Name), Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Identifier name length is bounded by identifier budget"})
	...
	invariant.Cross_Product(invariant.Always(suggested != "", "Suggested is non-empty at this point"))
	*diags = append(*diags, Diagnostic{
		Position: file_set.Position(identifier.Pos()),
	...
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				...
				Lo:      min_non_empty,
				Hi:      Max_Identifier_Chars,
				Message: "Output_string is non-empty per non-empty name input and bounded by identifier budget",
			}),
		)
	}()
	invariant.Cross_Product(
...
func check_exported_type_exposes_private(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, type_specification := range same_file_types {
		...
		entry_type_params := check_exported_type_exposes_private_type_params(type_specification.TypeParams)
		invariant.Cross_Product(invariant.Always(entry_type_params != nil, "Entry_type_params is non-nil at this point"))
		entry_name := type_specification.Name.Name
		if type_specification.Assign != token.NoPos {
			check_exported_type_exposes_private_check(&check_exported_type_exposes_private_check_input{
				File_Set:    file_set,
		...
		check_exported_type_exposes_private_walk(&check_exported_type_exposes_private_walk_input{
			File_Set:         file_set,
			Entry_Name:       entry_name,
...
func check_exported_type_exposes_private_walk(input *check_exported_type_exposes_private_walk_input) {
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by per-file diag budget"})
		same_file_types := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Types),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Same_file_types is bounded by AST budget"})
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Entry_Name),
			Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Entry_Name is non-empty bounded by identifier budget"})
		params_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Root_Type_Params),
			Lo: 0, Hi: Max_Identifier_Chars, Message: "Root_Type_Params is bounded by identifier budget"})
		invariant.Cross_Product(d, same_file_types, name_axis, params_axis,
			invariant.Excluding("Hi d unreachable", invariant.Bucket_Hi(d), invariant.Bucket_Hi(same_file_types)),
			invariant.Excluding("Hi d unreachable", invariant.Bucket_Hi(d), invariant.Bucket_Lo(same_file_types)),
	...
	invariant.Cross_Product(
		...
		invariant.Excluding("Hi d unreachable", invariant.Bucket_Hi(d), invariant.Bucket_Hi(same_file_types)),
		invariant.Excluding("Hi d unreachable", invariant.Bucket_Hi(d), invariant.Bucket_Lo(same_file_types)),
		invariant.Excluding("Hi sf with Lo d unreachable", invariant.Bucket_Hi(same_file_types), invariant.Bucket_Lo(d)),
		invariant.Excluding("Hi params unreachable", invariant.Bucket_Hi(params_axis), invariant.Bucket_Lo(name_axis)),
		invariant.Excluding("Hi params unreachable", invariant.Bucket_Hi(params_axis), invariant.Bucket_Hi(name_axis)))
	...
	stack := []frame{{Struct_Type: input.Root_Struct, Type_Params: input.Root_Type_Params}}
	for len(stack) > 0 {
		top := len(stack) - 1
		current := stack[top]
		stack = stack[:top]
		...
		for _, field := range current.Struct_Type.Fields.List {
			...
			if !ast.IsExported(identifier.Name) {
				...
				*input.Diags = append(*input.Diags, Diagnostic{
					...
					Message: fmt.Sprintf("exported type %s exposes unexported type %s",
			...
			// Generic struct types only appear via IndexExpr/IndexListExpr, not bare Ident.
			// The walk only recurses on bare-Ident targets, so target.TypeParams is unreachable here.
			stack = append(stack, frame{Struct_Type: target_struct, Type_Params: current.Type_Params})
		}
	}
}
...
func check_exported_type_exposes_private_check(input *check_exported_type_exposes_private_check_input) {
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by per-file diag budget"})
		...
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Entry_Name),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Entry_Name is a non-empty exported identifier bounded by identifier budget"})
		params_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Type_Params),
			Lo: 0, Hi: Max_Identifier_Chars,
		...
		invariant.Cross_Product(d, generic, name_axis, params_axis,
			...
			invariant.Excluding("Diag-budget cap with non-generic type unreachable in test corpus",
	...
	*input.Diags = append(*input.Diags, Diagnostic{
		Position: input.File_Set.Position(identifier.Pos()),
		Message:  fmt.Sprintf("exported type %s exposes unexported type %s", input.Entry_Name, identifier.Name),
	})
}
...
func check_exported_type_exposes_private_type_params(field_list *ast.FieldList) (names map[string]bool) {
	defer func() {
		n := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(names), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Names is bounded by AST budget"})
			...
			if d.Tok == token.CONST {
				if seen_non_constant {
					diags = append(diags, Diagnostic{
						...
						Message:  "const declaration must precede all var/type/func declarations in the file",
					})
				}
				continue
			}
	...
	ast.Inspect(file, func(n ast.Node) (descend bool) {
		...
		for _, key := range keys {
			...
			diags = append(diags, Diagnostic{
				...
				Message:  fmt.Sprintf("struct tag key %q is not stdlib; only json, xml, and asn1 are permitted", key),
			})
		}
		return true
	})
...
func check_no_third_party_struct_tag_parse_keys(tag *ast.BasicLit) (keys []string) {
	defer func() {
		k := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(keys),
			Lo: 0, Hi: max_string_slice_per_call, Message: "Keys is bounded by string-slice budget"})
		empty := invariant.Sometimes(len(keys) == 0, "Keys is empty for malformed or unparseable tags")
		invariant.Cross_Product(k, empty,
			...
			invariant.Excluding("Max keys at safety cap marks the pathological-input bound",
				invariant.Bucket_Hi(k), invariant.Bucket_False(empty)),
			invariant.Excluding("Zero keys implies empty true; (Lo, false) is impossible",
		...
		for _, field := range struct_type.Fields.List {
			...
			for _, name := range field.Names {
				...
				diags = append(diags, Diagnostic{
					...
					Message: "blank-named sync mutex has no usable Lock receiver; " +
						"use the noCopy idiom for non-copy semantics or name the field to lock it",
				})
			}
		}
		return true
...
func check_assertion_named_constant(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	idents_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	empty_idents_axis := invariant.Sometimes(len(invariant_idents) == 0, "Invariant_idents is empty for files without invariant import")
	invariant.Cross_Product(idents_axis, empty_idents_axis,
		...
		invariant.Excluding("Idents at safety cap is bad",
			invariant.Bucket_Hi(idents_axis), invariant.Bucket_False(empty_idents_axis)),
		invariant.Excluding("Zero idents implies empty true",
			invariant.Bucket_Lo(idents_axis), invariant.Bucket_False(empty_idents_axis)),
	)
	...
	ast.Inspect(file, func(n ast.Node) (descend bool) {
		...
		if name == "Distinct_Boundary" {
			diags = append(diags, check_assertion_named_constant_distinct_boundary(file_set, call)...)
			return true
		}
		if name == "Always" {
			diags = append(diags, check_assertion_named_constant_predicate(file_set, call)...)
			return true
		}
		if name == "Sometimes" {
			diags = append(diags, check_assertion_named_constant_predicate(file_set, call)...)
...
func check_assertion_named_constant_distinct_boundary(file_set *token.FileSet, call *ast.CallExpr) (diags []Diagnostic) {
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by budget"})
	...
	for _, element := range composite.Elts {
		...
		diags = append(diags, Diagnostic{
			...
			Message:  "assertion bound must be a file-level named constant, not an inline literal",
		})
	}
	return diags
}

func check_assertion_named_constant_predicate(file_set *token.FileSet, call *ast.CallExpr) (diags []Diagnostic) {
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by budget"})
		empty := invariant.Sometimes(len(diags) == 0, "Diags is empty for clean calls")
...
func assertion_bound_is_valid(expression ast.Expr) (yes bool) {
	defer func() {
		invariant.Cross_Product(invariant.Sometimes(yes, "Yes is true for allowed bound shapes"))
	}()
	for range invariant.Game_Loop() {
...
func basic_lit_is_zero_or_empty(basic *ast.BasicLit) (yes bool) {
	defer func() {
		invariant.Cross_Product(invariant.Sometimes(yes, "Yes is true for zero/empty literals"))
	}()
...
func check_keyed_struct_init(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		generic_declarationaration, ok := declaration.(*ast.GenDecl)
		if !ok {
			continue
		}
		if generic_declarationaration.Tok != token.TYPE {
			continue
		}
		for _, specification := range generic_declarationaration.Specs {
			type_specificationification, is_type_specificationification := specification.(*ast.TypeSpec)
			...
			_, is_struct := type_specificationification.Type.(*ast.StructType)
			if !is_struct {
				continue
			}
			struct_names[type_specificationification.Name.Name] = true
		}
	}
	ast.Inspect(file, func(n ast.Node) (descend bool) {
		...
		invariant.Cross_Product(invariant.Sometimes(name == "", "Name is empty for unnamed types"))
		if name == "" {
			return true
		}
...
func check_default_package_alias(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, import_specification := range file.Imports {
		...
		diags = append(diags, Diagnostic{
			...
			Message: fmt.Sprintf(
				"%q must be imported with alias %q; *_default packages re-export their library and must shadow its name",
				path, want,
			),
		})
	}
...
func check_test_package(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	return []Diagnostic{{
		...
		Message:  fmt.Sprintf("test file must declare 'package <X>_test'; got 'package %s'", name),
	}}
}
...
func check_banned_identifiers(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	package_hit := check_banned_identifiers_find_hit(file.Name.Name, []string{"util", "utils", "utility", "utilities", "length", "len"})
	invariant.Cross_Product(invariant.Sometimes(package_hit == "", "Package_hit can be empty on this branch"))
	if package_hit != "" {
		diags = append(diags, Diagnostic{
			Position: file_set.Position(file.Name.Pos()),
			Message:  fmt.Sprintf("package name %s contains banned word '%s'", file.Name.Name, package_hit),
		})
	}
...
func check_banned_identifiers_file_name(file_set *token.FileSet, file *ast.File) (diags []Diagnostic) {
	...
	hit := check_banned_identifiers_find_hit(stem, []string{"util", "utils", "utility", "utilities", "length", "len"})
...
func check_banned_identifiers_walk(file_set *token.FileSet, file *ast.File) (diags []Diagnostic) {
	...
	validate_banned_name := func(pos token.Pos, scope, name string, extra []string) {
		hit := check_banned_identifiers_find_hit(name, extra, []string{"util", "utils", "utility", "utilities", "length", "len"})
		invariant.Cross_Product(invariant.Sometimes(hit == "", "Hit can be empty on this branch"))
		if hit == "" {
			return
		}
		diags = append(diags, Diagnostic{
			Position: file_set.Position(pos),
			Message:  fmt.Sprintf("%s name %s contains banned word '%s'", scope, name, hit),
		})
	}
...
func check_banned_identifiers_walk_visit(
	...
	case *ast.RangeStmt:
		...
		for _, e := range []ast.Expr{x.Key, x.Value} {
			...
			validate_banned_name(identifier.Pos(), "range variable", identifier.Name, nil)
		}
...
func check_banned_identifiers_find_hit(name string, banned_lists ...[]string) (hit string) {
	defer func() {
		...
		bl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(banned_lists),
			Lo: min_non_empty, Hi: max_banned_lists_per_check, Message: "Banned_lists is small static set"})
...
func check_input_struct(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		...
		invariant.Cross_Product(invariant.Always(want_name != "", "Want_name is non-empty at this point"))
		diag := check_input_struct_validate(file_set, function_declaration, want_name)
		invariant.Cross_Product(invariant.Always(diag != nil, "Diag is non-nil at this point"))
		if diag != nil {
...
func check_input_struct_expected_name(function_name string) (want string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X: len(want), Lo: min_want_name_chars, Hi: max_want_name_chars,
				Message: "Want is `<function_name>_Input` shape; bounded by want_name budget",
			}),
		)
	}()
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(function_name), Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Function_name is a non-empty Go identifier bounded by identifier budget",
		}),
	)
...
func check_input_struct_validate(
	...
	defer func() {
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Name),
			Lo: 0, Hi: Max_Identifier_Chars, Message: "Diag.Name is bounded by identifier budget"})
		want_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Want),
			Lo: 0, Hi: max_invariant_suggestion_chars, Message: "Diag.Want is bounded by suggestion text budget"})
		message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Message),
			Lo: min_convert_to_message_chars, Hi: max_convert_to_message_chars,
			Message: "Diag.Message is `convert to <sig>` bounded by suggested-sig shape"})
		...
		invariant.Cross_Product(
			...
			invariant.Always(diag.Tier == 0,
				"Tier is 0 at diagnostic construction (set later by tier dispatcher)"),
			...
			invariant.Always(diag.Message != "", "Diag.Message is non-empty by construction"),
	...
	invariant.Cross_Product(invariant.Always(non_variadic != nil, "Non_variadic is non-nil at this point"))
	if len(non_variadic) == 1 {
		if len(non_variadic[0].Names) == 1 {
			...
			invariant.Unreachable("Check_input_struct: trigger fired with single-name param")
		}
	}
	return &Diagnostic{
		Position: file_set.Position(function.Pos()),
		Message:  "convert to " + check_input_struct_validate_suggest_sig(function, want_name),
	}
}

func check_input_struct_validate_suggest_sig(function *ast.FuncDecl, want_name string) (sig string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X: len(sig), Lo: min_suggested_sig_chars, Hi: max_suggested_sig_chars,
				Message: "Sig is the suggested function signature; sized for funcname plus `_Input` template",
			}),
		)
	}()
	invariant.Cross_Product(
...
func check_input_struct_validate_non_variadic_params(function *ast.FuncDecl) (output []*ast.Field) {
	defer func() {
		o := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(output), Lo: min_non_empty,
			Hi: max_ast_nodes_per_call, Message: "Output is ≥1 non-variadic param"})
		single_axis := invariant.Sometimes(len(output) == count_one, "Output has exactly one param sometimes")
...
func check_no_empty_function_body(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		...
		diags = append(diags, Diagnostic{
			Position: file_set.Position(function_declaration.Pos()),
			Message:  fmt.Sprintf("func %s has an empty body", function_declaration.Name.Name),
		})
	}
	return diags
}
	...
	ast.Inspect(file, func(n ast.Node) (recurse bool) {
		...
		for _, f := range interface_type.Methods.List {
			...
			diags = append(diags, Diagnostic{
				...
				Message:  "interface method sets are banned; use a concrete type or free function",
			})
			return true
		}
		return true
...
func check_no_package_vars(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		generic_declarationaration, is_generic_declarationaration := declaration.(*ast.GenDecl)
		if !is_generic_declarationaration {
			continue
		}
		if generic_declarationaration.Tok != token.VAR {
			continue
		}
		for _, specification := range generic_declarationaration.Specs {
...
func check_unnecessary_method(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		...
		results := check_unnecessary_method_field_list_types(function_declaration.Type.Results)
		invariant.Cross_Product(invariant.Sometimes(results == nil, "Results can be empty or zero on this branch"))
		match := check_unnecessary_method_matches_stdlib(&check_unnecessary_method_matches_stdlib_input{
			Name:    function_declaration.Name.Name,
			Params:  strings.Join(params, ","),
			Results: strings.Join(results, ","),
		})
		invariant.Cross_Product(invariant.Sometimes(match, "Match can be false on this branch"))
		if match {
			continue
		}
		message := fmt.Sprintf(
			"method %s does not satisfy any stdlib interface; "+
				"convert to a free function with the receiver as the first parameter",
...
func check_unnecessary_method_matches_stdlib(input *check_unnecessary_method_matches_stdlib_input) (yes bool) {
	defer func() {
		invariant.Cross_Product(invariant.Sometimes(
			yes, "Affirmative branch is exercised"))
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Name),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
	...
	name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Name),
		...
		Message: "Input.Name is a Go method name bounded by identifier budget"})
	invariant.Cross_Product(name_axis)
	params_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Params),
		Lo: 0, Hi: max_method_params_test_corpus,
		Message: "Input.Params spans empty () to Bar fixture `A,xxx128` (130)"})
	invariant.Cross_Product(params_axis)
	results_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Results),
		Lo: 0, Hi: Max_Identifier_Chars,
...
func check_unnecessary_method_field_list_types(fl *ast.FieldList) (output_list []string) {
	defer func() {
		o := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(output_list), Lo: 0,
			Hi: max_string_slice_per_call, Message: "Output_list is bounded by budget"})
		empty_axis := invariant.Sometimes(len(output_list) == 0, "Output_list is empty sometimes")
	...
	for _, f := range fl.List {
		...
		invariant.Cross_Product(invariant.Always(rendered != "", "Rendered is non-empty at this point"))
		count := len(f.Names)
		if count == 0 {
			count = 1
		}
...
func check_unnecessary_method_field_list_types_render_type(expression ast.Expr) (output_string string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X:       len(output_string),
				Lo:      min_non_empty,
				Hi:      Max_Identifier_Chars,
				Message: "Output_string is non-empty per leaf cases and bounded by identifier budget",
			}),
		)
	}()
...
func check_snap_backtick(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	ast.Inspect(file, func(n ast.Node) (descend bool) {
		...
		diags = append(diags, Diagnostic{
			Position: file_set.Position(lit.Pos()),
			Message:  fmt.Sprintf("snap.%s must use a backticked raw string literal", method),
		})
		return true
	})
	return diags
...
func check_test_documentation_comment(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		...
		diags = append(diags, Diagnostic{
			Position: file_set.Position(function_declaration.Pos()),
			Message:  fmt.Sprintf("test %s is missing a doc comment", function_declaration.Name.Name),
		})
	}
	return diags
}
...
func check_exported_documentation_comment(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		switch d := declaration.(type) {
		case *ast.FuncDecl:
			diags = append(diags, check_exported_documentation_comment_function(file_set, d)...)
		case *ast.GenDecl:
			diags = append(diags, check_exported_documentation_comment_generic(file_set, d)...)
		}
	}
	return diags
}
...
func check_exported_documentation_comment_function(file_set *token.FileSet, function_declaration *ast.FuncDecl) (diags []Diagnostic) {
	...
	return []Diagnostic{{
		...
		Message:  fmt.Sprintf("exported %s %s is missing a doc comment", label, function_declaration.Name.Name),
	}}
}

func check_exported_documentation_comment_generic(file_set *token.FileSet, generic_declarationaration *ast.GenDecl) (diags []Diagnostic) {
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by budget"})
	...
	invariant.Cross_Product(
		invariant.Always(file_set != nil, "File_set is non-nil"),
		invariant.Always(generic_declarationaration != nil, "Generic_declaration is non-nil"),
	)

	switch generic_declarationaration.Tok {
	case token.TYPE, token.VAR, token.CONST:
	default:
		return nil
	}
	group_has_documentation := check_exported_documentation_comment_has_documentation(generic_declarationaration.Doc)
	invariant.Cross_Product(invariant.Sometimes(group_has_documentation, "Group_has_documentation can be false on this branch"))
	kind := "var"
	switch generic_declarationaration.Tok {
	case token.TYPE:
		kind = "type"
	...
	for _, specification := range generic_declarationaration.Specs {
		...
		case *ast.TypeSpec:
			...
			diags = append(diags, Diagnostic{
				...
				Message:  fmt.Sprintf("exported type %s is missing a doc comment", s.Name.Name),
			})
		case *ast.ValueSpec:
			specification_has_documentation := check_exported_documentation_comment_has_documentation(s.Doc)
			invariant.Cross_Product(invariant.Sometimes(
				specification_has_documentation,
				"Specification_has_documentation is true for documented value specs"))
			...
			for _, name := range s.Names {
				...
				diags = append(diags, Diagnostic{
					...
					Message:  fmt.Sprintf("exported %s %s is missing a doc comment", kind, name.Name),
				})
			}
		}
	}
...
func check_names(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, v := range violations {
		diags = append(diags, Diagnostic{Position: file_set.Position(v.Position), Message: v.Message})
	}
	sort.Slice(diags, func(i, j int) (less bool) {
	...
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				...
				Message: "Term is the stdlib-allowlist terminology suffix; longest is `offset` (6)",
			}),
		)
	}()
	invariant.Cross_Product(
...
func check_names_terminology(file *ast.File) (violations []name_violation) {
	defer func() {
		v := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(violations), Lo: 0,
			Hi: max_diagnostics_per_call, Message: "Violations is bounded by budget"})
		decls_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(file.Decls),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "File.Decls is bounded by AST budget"})
	...
	for _, declaration := range file.Decls {
		...
		declarations_map := check_names_terminology_function_declarations(function_declaration)
		invariant.Cross_Product(invariant.Always(declarations_map != nil, "Declarations_map is non-nil at this point"))
...
func check_names_terminology_function_declarations(function *ast.FuncDecl) (declarations map[string]*ast.Ident) {
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(declarations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Declarations is bounded by budget"})
		empty_axis := invariant.Sometimes(len(declarations) == 0, "Declarations is empty sometimes")
...
func check_names_terminology_attach(input *check_names_terminology_attach_input) {
	defer func() {
		decls_empty := invariant.Sometimes(len(input.Declarations) == 0, "Input.Declarations is empty sometimes")
		decls_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Declarations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Declarations is bounded by AST budget"})
	...
	case *ast.ForStmt:
		...
		invariant.Cross_Product(invariant.Sometimes(ind == nil, "Ind is nil for body-only for-loops"))
		if ind != nil {
...
func check_names_terminology_attach_make(
	...
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(decls), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Decls is bounded by budget"})
	...
	d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(decls), Lo: 0,
		Hi: max_ast_nodes_per_call, Message: "Decls is bounded by budget"})
	args_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(x.Args), Lo: 0,
		Hi: max_ast_nodes_per_call, Message: "X.Args is bounded by AST budget"})
	invariant.Cross_Product(
		invariant.Always(x != nil, "X is non-nil"),
		d, args_axis,
...
func check_names_terminology_attach_callee_term(function_expression ast.Expr) (terms []string) {
	defer func() {
		t := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(terms), Lo: 0,
			Hi: max_string_slice_per_call, Message: "Terms is bounded by budget"})
	...
	case *ast.SelectorExpr:
		...
		if is_ident {
			...
			if t := check_names_terminology_attach_callee_term_stdlib_required(key); t != "" {
				invariant.Cross_Product(invariant.Always(t != "", "T is non-empty at this point"))
				return []string{t}
			}
		}
		if f.Sel.Name == "Len" {
...
func check_names_terminology_emit(
	...
	defer func() {
		v := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(violations), Lo: 0,
			Hi: max_diagnostics_per_call, Message: "Violations is bounded by budget"})
		r := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by budget"})
	...
	for identifier, terms := range requirements {
		...
		invariant.Cross_Product(invariant.Sometimes(last == "", "Last can be empty on this branch"))
		matched := false
		...
		entries = append(entries, entry{Name: identifier.Name, Terms: term_list, Position: identifier.Pos()})
	}
	...
	for _, e := range entries {
		...
		preferred := e.Terms[0]
		suggestion := check_names_terminology_emit_rename(&check_names_terminology_emit_rename_input{
			Name: e.Name, Term: preferred,
		})
		invariant.Cross_Product(invariant.Always(suggestion != "", "Suggestion is non-empty at this point"))
		violations = append(violations, name_violation{
			Position: e.Position,
			Message:  fmt.Sprintf("naming convention: %s (used as %s) → rename to %s", e.Name, category, suggestion),
		})
	}
	return violations
}
...
func check_names_terminology_emit_rename(input *check_names_terminology_emit_rename_input) (output_string string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X:       len(output_string),
				...
				Hi:      Max_Identifier_Chars,
				Message: "Output_string is `<name>_<term>` (5 for a replaced single word like `count`)",
			}),
		)
	}()
	invariant.Cross_Product(invariant.Always(
		input != nil, "Input is non-nil"))
	term_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Term),
		Lo: min_stdlib_term_chars, Hi: max_stdlib_term_chars,
		Message: "Input.Term is the preferred terminology suffix in [size,count,index,offset]"})
	invariant.Cross_Product(term_axis)
	name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Name),
		Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Input.Name is a flagged identifier, one char and up"})
	name_short := invariant.Sometimes(len(input.Name) == min_non_empty, "Name is a single char sometimes")
	invariant.Cross_Product(name_axis, name_short,
		invariant.Excluding("Hi name short T", invariant.Bucket_Hi(name_axis), invariant.Bucket_True(name_short)),
		invariant.Excluding("Hi name short F", invariant.Bucket_Hi(name_axis), invariant.Bucket_False(name_short)),
		invariant.Excluding("Lo name short F", invariant.Bucket_Lo(name_axis), invariant.Bucket_False(name_short)),
	)
...
func check_names_suffix_of(name string) (suffix string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X: len(suffix), Lo: 0, Hi: max_stdlib_term_chars,
				Message: "Suffix is one of the terminology suffixes; longest is `offset` (6)",
			}),
		)
	}()
	invariant.Cross_Product(
...
func check_names_arithmetic(file *ast.File) (violations []name_violation) {
	...
	ast.Inspect(file, func(n ast.Node) (descend bool) {
		...
		violations = append(violations,
			check_names_arithmetic_check_binary(&check_names_arithmetic_check_binary_input{
				Binary_Expression: binary_expression, Lhs: rhs_of[binary_expression],
			})...)
		return true
	})
	return violations
...
func check_names_arithmetic_rhs_map(file *ast.File) (m map[*ast.BinaryExpr]*ast.Ident) {
	defer func() {
		x := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(m),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "M is bounded by budget"})
		decls_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(file.Decls),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "File.Decls is bounded by AST budget"})
...
func check_names_arithmetic_check_binary(input *check_names_arithmetic_check_binary_input) (violations []name_violation) {
	defer func() {
		v := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(violations), Lo: 0,
			Hi: max_diagnostics_per_call, Message: "Violations is bounded by budget"})
		lhs_axis := invariant.Sometimes(input.Lhs == nil, "Lhs is nil for free-standing BinaryExprs")
	...
	invariant.Cross_Product(invariant.Sometimes(left_suffix == "", "Left_suffix can be empty on this branch"))
	if left_suffix == "" {
		return nil
	}
	right_suffix := check_names_suffix_of(right.Name)
	invariant.Cross_Product(invariant.Always(right_suffix != "", "Right_suffix is non-empty at this point"))
	if right_suffix == "" {
		return nil
	}
	op_string := "+"
	...
	result := check_names_arithmetic_check_binary_result(&check_names_arithmetic_check_binary_result_input{
		Left: left_suffix, Op: input.Binary_Expression.Op, Right: right_suffix,
	})
	invariant.Cross_Product(invariant.Sometimes(result == "", "Result can be empty on this branch"))
	if result == "" {
		violations = append(violations, name_violation{
			Position: input.Binary_Expression.Pos(),
			Message:  fmt.Sprintf("arithmetic: _%s %s _%s is incoherent", left_suffix, op_string, right_suffix),
		})
		return violations
	}
	if input.Lhs == nil {
	...
	invariant.Cross_Product(invariant.Sometimes(lhs_suffix == "", "Lhs_suffix can be empty on this branch"))
...
func check_names_arithmetic_check_binary_result(
	...
	invariant.Cross_Product(invariant.Always(
		input != nil, "Input is non-nil"))
	left_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Left),
		Lo: min_stdlib_term_chars, Hi: max_stdlib_term_chars,
		Message: "Input.Left is a terminology term in [size,count,index,offset]"})
	invariant.Cross_Product(left_axis)
	right_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Right),
		Lo: min_stdlib_term_chars, Hi: max_stdlib_term_chars,
...
func banned_abbreviation_candidates(word string) (candidates []string) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(candidates), Lo: 0,
			Hi: max_string_slice_per_call, Message: "Candidates is bounded by budget"})
		word_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(word), Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Word is bounded by identifier budget",
		})
	...
	if candidates = banned_abbreviation_candidates_a_b(word); candidates != nil {
		invariant.Cross_Product(invariant.Always(candidates != nil, "Candidates is non-nil at this point"))
		return candidates
	}
...
func banned_abbreviation_candidates_a_b(word string) (candidates []string) {
	...
	single_axis := invariant.Sometimes(len(word) == count_one, "Word is exactly 1 char sometimes")
...
func banned_abbreviation_candidates_d_f(word string) (candidates []string) {
	...
	invariant.Cross_Product(invariant.Sometimes(d_candidates == nil, "D_candidates is nil for non-d words"))
	if d_candidates != nil {
		return d_candidates
	}
	switch word {
	...
	case "fp":
		return []string{"file_processor", "function_pointer", "floating_point", "false_positive"}
	case "fr":
...
func is_allowed_ing_noun(word string) (allowed bool) {
	...
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(word), Lo: min_ing_word_chars, Hi: Max_Identifier_Chars,
			Message: "Word ends in `ing` (minimum 3 chars) and is bounded by identifier budget",
		}),
	)
...
func check_names_walk_declarations_function(function *ast.FuncDecl, emit func(identifier *ast.Ident)) {
	invariant.Cross_Product(invariant.Always(
		function != nil, "Function is non-nil"))
...
func check_names_abbreviations(file *ast.File) (violations []name_violation) {
	...
	check_names_walk_decls(file, func(identifier *ast.Ident) {
		...
		for word_index, w := range words {
			...
			invariant.Cross_Product(invariant.Sometimes(cands == nil, "Cands can be empty or zero on this branch"))
			if cands == nil {
				continue
			}
			...
			violations = append(violations, name_violation{
				...
				Message:  fmt.Sprintf("rename %s -> %s", identifier.Name, strings.Join(renames, ",")),
			})
		}
	})
	return violations
...
func check_names_participles(file *ast.File) (violations []name_violation) {
	...
	check_names_walk_decls(file, func(identifier *ast.Ident) {
		...
		violations = append(violations, name_violation{
			Position: identifier.Pos(),
			Message:  fmt.Sprintf("present participle %q → rename to a noun form", last),
		})
	})
	return violations
}

func check_comments_group_is_inline(file_set *token.FileSet, source []byte, c *ast.Comment) (inline bool) {
	defer func() {
		s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(source), Lo: min_source_with_comment_bytes,
			Hi: max_file_size_bytes, Message: "Source is ≥16 bytes (`package x\\n\\n// c\\n` minimum)"})
	...
	s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(source), Lo: min_source_with_comment_bytes,
		Hi: max_file_size_bytes, Message: "Source is ≥16 bytes (`package x\\n\\n// c\\n` minimum)"})
	multi_axis := invariant.Sometimes(strings.HasPrefix(c.Text, "/*"), "C is a block comment sometimes")
	invariant.Cross_Product(
		invariant.Always(file_set != nil, "File_set is non-nil"),
		invariant.Always(c != nil, "C is non-nil"),
		s, multi_axis,
...
type check_function_stream struct {
	Name     string
	Visit    func(p string, info fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic)
...
func check_file_system_stream_checkers(
	...
	defer func() {
		invariant.Cross_Product(invariant.Always(len(checks) == stream_checker_count, "Checks is the fixed stream-tier set"))
	}()
	rd := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(root_directory),
		Lo: 0, Hi: max_filesystem_path_chars, Message: "Root_directory spans empty to deep path"})
	rd_empty := invariant.Sometimes(len(root_directory) == 0, "Root_directory is empty sometimes")
	invariant.Cross_Product(rd, rd_empty,
		invariant.Excluding("Hi rd implies non-empty", invariant.Bucket_Hi(rd), invariant.Bucket_True(rd_empty)),
		invariant.Excluding("Lo rd implies empty true", invariant.Bucket_Lo(rd), invariant.Bucket_False(rd_empty)),
	)
	return [stream_checker_count]check_function_stream{
		...
		check_file_system_stream_checks_stream_symlinks_checker(&check_file_system_stream_checks_stream_symlinks_checker_input{
...
func check_file_system_stream(input *check_file_system_stream_input) (diags []Diagnostic, go_paths []string, err error) {
	defer func() {
		...
		invariant.Cross_Product(d, g,
			invariant.Excluding("Both d and g at safety caps is bad", invariant.Bucket_Hi(d), invariant.Bucket_Hi(g)),
			invariant.Excluding("Max d with min g is bad", invariant.Bucket_Hi(d), invariant.Bucket_Lo(g)),
			invariant.Excluding("Zero d with max g is bad", invariant.Bucket_Lo(d), invariant.Bucket_Hi(g)),
		)
	}()
	tracked := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Tracked),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Tracked is bounded by AST budget"})
	tracked_empty := invariant.Sometimes(len(input.Tracked) == 0, "Tracked is empty sometimes")
	dht := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Directory_Has_Tracked),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Directory_has_tracked is bounded by AST budget"})
	invariant.Cross_Product(
		invariant.Always(input != nil, "Input is non-nil"),
		tracked, tracked_empty, dht,
		invariant.Excluding("Hi tracked empty unreachable", invariant.Bucket_Hi(tracked), invariant.Bucket_True(tracked_empty)),
		invariant.Excluding("Lo tracked implies empty true", invariant.Bucket_Lo(tracked), invariant.Bucket_False(tracked_empty)),
		invariant.Excluding("Hi dht empty unreachable", invariant.Bucket_Hi(dht), invariant.Bucket_True(tracked_empty)),
		invariant.Excluding("Hi dht non-empty unreachable", invariant.Bucket_Hi(dht), invariant.Bucket_False(tracked_empty)),
	)
	checks := check_file_system_stream_checkers(input.Root_Directory, input.Readlink, input.Stat)
	invariant.Cross_Product(invariant.Always(len(checks) == stream_checker_count, "Checks is the fixed stream-tier set"))
	per_check := make([][]Diagnostic, len(checks))
	err = fs.WalkDir(input.Fsys, input.Root, func(p string, d fs.DirEntry, walk_err error) (out error) {
		if walk_err != nil {
			return walk_err
		}
		if d.IsDir() {
		...
		invariant.Cross_Product(
			invariant.Always(information != nil, "Information is non-nil at this point"),
			invariant.Always(information_error == nil, "Information_error is nil at this point"),
		)
...
func check_stream_conflict_markers(p string, information fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
			Message: "P length is bounded; shortest path is `a.go` style 4-char Go file name"})
		o := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*output),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Output is bounded by per-file diag budget"})
		invariant.Cross_Product(path_axis, o,
			...
			invariant.Excluding("Minimal path with diag-budget cap unreachable in test corpus",
	...
	o := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*output),
		Lo: 0, Hi: max_diagnostics_per_call, Message: "Output is bounded by per-file diag budget"})
	invariant.Cross_Product(
		invariant.Always(output != nil, "Output is non-nil"),
		path_axis, o,
		invariant.Excluding("Path at safety cap with diag-budget cap is bad",
			invariant.Bucket_Hi(path_axis), invariant.Bucket_Hi(o)),
		invariant.Excluding("Minimal path with diag-budget cap unreachable in test corpus",
...
// drop-zone at root.
func check_stream_banned_scripts(p string, information fs.FileInfo, _ func() (data []byte, err error), output *[]Diagnostic) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
			Message: "P length is bounded; shortest path is `a.go` style 4-char Go file name"})
	...
	*output = append(*output, Diagnostic{
		...
		Message:  fmt.Sprintf("banned scripting file %q; move under top-level third_party/ or vendor/, or remove", p),
	})
}
...
func check_stream_github_actions_uses(p string, information fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
			Message: "P length is bounded; shortest path is `a.go` style 4-char Go file name"})
	...
	for i := 0; i <= len(source); i++ {
		...
		if bytes.HasPrefix(trimmed, []byte("uses:")) {
			*output = append(*output, Diagnostic{
				Position: token.Position{Filename: p, Line: line_number, Column: 1},
				Message:  "third-party github action banned; replace `uses:` with an inline `run:` step",
			})
		}
		line_number++
		line_start = i + 1
...
func check_stream_copyleft(p string, information fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
			Message: "P length is bounded; shortest path is `a.go` style 4-char Go file name"})
	...
	*output = append(*output, Diagnostic{
		Position: token.Position{Filename: p},
		Message:  fmt.Sprintf("%s: replace with a permissive license (MIT/Apache/BSD)", family),
	})
}

func check_stream_max_file_size(p string, information fs.FileInfo, _ func() (data []byte, err error), output *[]Diagnostic) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
			Message: "P length is bounded; shortest path is `a.go` style 4-char Go file name"})
	...
	*output = append(*output, Diagnostic{
		Position: token.Position{Filename: p},
		Message:  fmt.Sprintf("file exceeds 1 MiB (%d bytes); move it out of the tree (LFS or external store)", information.Size()),
	})
}

func check_stream_agent_documentation_max_lines(
	p string, information fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic,
) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
...
func check_file_system_stream_checks_stream_path_casing_checker() (c check_function_stream) {
	defer func() {
		invariant.Cross_Product(
			invariant.Always(c.Name != "", "C.Name is non-empty for the path-casing checker"),
			invariant.Always(len(c.Name) == path_casing_check_name_chars, "C.Name is the fixed path-casing label"),
		)
	}()
	...
	return check_function_stream{
		...
		Visit: func(p string, info fs.FileInfo, _ func() (data []byte, err error), output *[]Diagnostic) {
			if strings.HasPrefix(p, "third_party/") {
				return
			}
			if p == "third_party" {
			...
			for i, seg := range segments {
				...
				*output = append(*output, Diagnostic{
					...
					Message: fmt.Sprintf(
						"%s name %q must be snake_case, Ada_Case, or SCREAMING_SNAKE_CASE",
						kind, name,
					),
				})
			}
...
func check_file_system_stream_checks_stream_symlinks_checker(
	...
	defer func() {
		rd_empty := invariant.Sometimes(len(input.Root_Directory) == 0, "Root_Directory is empty sometimes")
		rd_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Root_Directory),
			Lo: 0, Hi: max_filesystem_path_chars,
			Message: "Input.Root_Directory is bounded by filesystem path budget"})
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(c.Name),
			Lo: min_stream_check_name_chars, Hi: max_stream_check_name_chars,
		...
		invariant.Cross_Product(rd_empty, rd_axis, name_axis,
			...
			invariant.Excluding("Hi name unreachable — Name is `symlink` (7), Hi (20) is for other checkers",
	...
	rd_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Root_Directory),
		Lo: 0, Hi: max_filesystem_path_chars,
		Message: "Input.Root_Directory is bounded by filesystem path budget"})
	invariant.Cross_Product(invariant.Always(input != nil, "Input is non-nil"), rd_empty, rd_axis,
		invariant.Excluding("Lo rd implies empty=true",
			invariant.Bucket_Lo(rd_axis), invariant.Bucket_False(rd_empty)),
		invariant.Excluding("Hi rd implies empty=false",
			invariant.Bucket_Hi(rd_axis), invariant.Bucket_True(rd_empty)))
	...
	return check_function_stream{
		...
		Visit: func(p string, info fs.FileInfo, _ func() (data []byte, err error), output *[]Diagnostic) {
			...
			if !filepath.IsAbs(target) {
				resolved = filepath.Join(filepath.Dir(operating_system_path), target)
			}
...
// dominant token.
func check_stream_markdown_line_max(p string, information fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic) {
	defer func() {
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(p), Lo: min_go_filename_chars, Hi: max_filesystem_path_chars,
			Message: "P length is bounded; shortest path is `a.go` style 4-char Go file name"})
	...
	for i := 0; i < len(source); {
		...
		is_table_row := bytes.HasPrefix(trimmed, []byte("|")) && bytes.HasSuffix(trimmed, []byte("|"))
		if is_table_row {
			continue
		}
		...
		if runes_count > markdown_line_max {
			*output = append(*output, Diagnostic{
				Position: token.Position{Filename: p, Line: line_number, Column: 1},
				Message:  fmt.Sprintf("markdown line is %d chars; visual limit is %d", runes_count, markdown_line_max),
			})
		}
	}
}
...
func check_file_system_stream_checks_stream_agents_claude_pair_checker() (c check_function_stream) {
	defer func() {
		invariant.Cross_Product(
			invariant.Always(c.Name != "", "C.Name is non-empty for the agents-claude-pair checker"),
			invariant.Always(len(c.Name) == agents_pair_check_name_chars, "C.Name is the fixed agents-claude-pair label"),
		)
	}()
	...
	return check_function_stream{
		...
		Visit: func(p string, info fs.FileInfo, load func() (data []byte, err error), _ *[]Diagnostic) {
			name := info.Name()
			invariant.Cross_Product(invariant.Always(name != "", "Name is non-empty at this point"))
			if name != "AGENTS.md" {
				if name != "CLAUDE.md" {
					return
				}
			...
			for _, d := range dirs {
				...
				case !pp.Has_Agents:
					*output = append(*output, Diagnostic{
						...
						Message:  "AGENTS.md is missing; it must mirror CLAUDE.md byte-for-byte",
					})
				case !pp.Has_Claude:
					*output = append(*output, Diagnostic{
						Position: token.Position{Filename: d},
						Message:  "CLAUDE.md is missing; it must mirror AGENTS.md byte-for-byte",
					})
				case !bytes.Equal(pp.Agents, pp.Claude):
					*output = append(*output, Diagnostic{
						Position: token.Position{Filename: d},
						Message:  "AGENTS.md and CLAUDE.md differ; make them byte-identical",
					})
				}
			}
		},
...
func parsed_file_is_composition_tier(pf parsed_file, modules *module_index) (yes bool) {
	defer func() {
		...
		modules_empty_d := invariant.Sometimes(len(modules.Modules) == 0, "Modules slice is empty sometimes")
		modules_axis_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(modules.Modules),
			Lo: 0, Hi: max_parsed_files_per_call, Message: "Modules.Modules spans empty workspace to budget"})
		invariant.Cross_Product(modules_axis_d, modules_empty_d,
			invariant.Excluding("Hi mod empty T", invariant.Bucket_Hi(modules_axis_d), invariant.Bucket_True(modules_empty_d)),
			invariant.Excluding("Hi mod empty F", invariant.Bucket_Hi(modules_axis_d), invariant.Bucket_False(modules_empty_d)),
			invariant.Excluding("Lo mod empty F", invariant.Bucket_Lo(modules_axis_d), invariant.Bucket_False(modules_empty_d)),
		)
		f2m_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(modules.File_To_Module),
			Lo: min_non_empty, Hi: max_parsed_files_per_call, Message: "File_To_Module spans single-file workspace to budget"})
		f2m_single_d := invariant.Sometimes(len(modules.File_To_Module) == min_non_empty, "File_To_Module has one entry sometimes")
		invariant.Cross_Product(f2m_d, f2m_single_d,
			invariant.Excluding("Hi f2m single", invariant.Bucket_Hi(f2m_d), invariant.Bucket_True(f2m_single_d)),
			invariant.Excluding("F2m within budget", invariant.Bucket_Hi(f2m_d), invariant.Bucket_False(f2m_single_d)),
			invariant.Excluding("Lo f2m is single", invariant.Bucket_Lo(f2m_d), invariant.Bucket_False(f2m_single_d)),
		)
		source_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(pf.Source),
			Lo: min_go_source_bytes, Hi: max_file_size_bytes, Message: "Pf.Source byte length"})
		source_minimal_d := invariant.Sometimes(len(pf.Source) == min_go_source_bytes, "Pf.Source is minimal sometimes")
		invariant.Cross_Product(source_d, source_minimal_d,
			invariant.Excluding("Hi source min", invariant.Bucket_Hi(source_d), invariant.Bucket_True(source_minimal_d)),
			invariant.Excluding("Source in budget", invariant.Bucket_Hi(source_d), invariant.Bucket_False(source_minimal_d)),
			invariant.Excluding("Lo source min", invariant.Bucket_Lo(source_d), invariant.Bucket_False(source_minimal_d)),
		)
	}()
	invariant.Cross_Product(
		invariant.Always(pf.File_Set != nil, "Pf.File_Set is non-nil"),
	...
	path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(pf.Path),
		Lo: min_go_filename_chars, Hi: max_filesystem_path_chars, Message: "Pf.Path spans 4-char (a.go) to deep paths"})
	invariant.Cross_Product(path_axis)
	modules_empty := invariant.Sometimes(len(modules.Modules) == 0, "Modules slice is empty sometimes")
	modules_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(modules.Modules),
		Lo: 0, Hi: max_parsed_files_per_call, Message: "Modules.Modules spans empty workspace to budget"})
	invariant.Cross_Product(modules_axis, modules_empty,
		invariant.Excluding("Hi mod empty T", invariant.Bucket_Hi(modules_axis), invariant.Bucket_True(modules_empty)),
		invariant.Excluding("Hi mod empty F", invariant.Bucket_Hi(modules_axis), invariant.Bucket_False(modules_empty)),
		invariant.Excluding("Lo mod empty F", invariant.Bucket_Lo(modules_axis), invariant.Bucket_False(modules_empty)),
	)
	f2m := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(modules.File_To_Module),
		Lo: min_non_empty, Hi: max_parsed_files_per_call, Message: "File_To_Module spans single-file workspace to budget"})
	f2m_single := invariant.Sometimes(len(modules.File_To_Module) == min_non_empty, "File_To_Module has one entry sometimes")
	invariant.Cross_Product(f2m, f2m_single,
		invariant.Excluding("Hi f2m single", invariant.Bucket_Hi(f2m), invariant.Bucket_True(f2m_single)),
		invariant.Excluding("F2m within budget", invariant.Bucket_Hi(f2m), invariant.Bucket_False(f2m_single)),
		invariant.Excluding("Lo f2m is single", invariant.Bucket_Lo(f2m), invariant.Bucket_False(f2m_single)),
	)
	source := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(pf.Source),
		Lo: min_go_source_bytes, Hi: max_file_size_bytes, Message: "Pf.Source byte length"})
	source_minimal := invariant.Sometimes(len(pf.Source) == min_go_source_bytes, "Pf.Source is minimal sometimes")
	invariant.Cross_Product(source, source_minimal,
		invariant.Excluding("Hi source minimal", invariant.Bucket_Hi(source), invariant.Bucket_True(source_minimal)),
		invariant.Excluding("Source within budget", invariant.Bucket_Hi(source), invariant.Bucket_False(source_minimal)),
		invariant.Excluding("Lo source is minimal", invariant.Bucket_Lo(source), invariant.Bucket_False(source_minimal)),
	)
	module_index_number := modules.File_To_Module[pf.Path]
	if module_index_number < 0 {
		return false
...
func is_ambient_hard_import(path string) (yes bool) {
	...
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			...
			Lo:      min_non_empty,
			Hi:      Max_Identifier_Chars,
			Message: "Path is an import path; shortest is `C` (cgo, 1 char), longest realistic is bounded by identifier budget",
		}),
	)
...
func is_ambient_soft_ident(input *is_ambient_soft_ident_input) (yes bool) {
	...
	invariant.Cross_Product(invariant.Always(
		input != nil, "Input is non-nil"))
	name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Name),
		Lo: min_non_empty, Hi: max_filesystem_path_chars, Message: "Input.Name is a selector symbol, one char and up"})
	name_short := invariant.Sometimes(len(input.Name) == min_non_empty, "Name is a single char sometimes")
	invariant.Cross_Product(name_axis, name_short,
		invariant.Excluding("Hi vs single char", invariant.Bucket_Hi(name_axis), invariant.Bucket_True(name_short)),
		invariant.Excluding("Symbol within budget", invariant.Bucket_Hi(name_axis), invariant.Bucket_False(name_short)),
		invariant.Excluding("Lo is single char", invariant.Bucket_Lo(name_axis), invariant.Bucket_False(name_short)),
	)
	package_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Package),
		Lo: min_non_empty, Hi: max_filesystem_path_chars, Message: "Input.Package is an import path, one char and up"})
	package_short := invariant.Sometimes(len(input.Package) == min_non_empty, "Package is a single char sometimes")
	invariant.Cross_Product(package_axis, package_short,
		invariant.Excluding("Hi vs single char", invariant.Bucket_Hi(package_axis), invariant.Bucket_True(package_short)),
		invariant.Excluding("Path within budget", invariant.Bucket_Hi(package_axis), invariant.Bucket_False(package_short)),
		invariant.Excluding("Lo is single char", invariant.Bucket_Lo(package_axis), invariant.Bucket_False(package_short)),
	)
...
func check_invariant_assertions(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	invariant_idents := check_invariant_assertions_collect_invariant_idents(file)
	invariant.Cross_Product(invariant.Always(invariant_idents != nil, "Invariant_idents is non-nil at this point"))
	stdlib_imports := check_invariant_assertions_collect_stdlib_imports(file)
	invariant.Cross_Product(invariant.Always(stdlib_imports != nil, "Stdlib_imports is non-nil at this point"))
	struct_field_index := check_invariant_assertions_build_struct_field_index(file, stdlib_imports)
	invariant.Cross_Product(invariant.Always(struct_field_index != nil, "Struct_field_index is non-nil at this point"))
	same_file_consts := check_invariant_assertions_collect_same_file_constants(file)
	invariant.Cross_Product(invariant.Always(same_file_consts != nil, "Same_file_consts is non-nil at this point"))
	constant_signs := check_invariant_assertions_collect_same_file_constant_signs(file)
	invariant.Cross_Product(invariant.Always(constant_signs != nil, "Const_signs is non-nil at this point"))
	for _, declaration := range file.Decls {
...
func check_invariant_assertions_one_function(
	...
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by budget"})
		diags_empty := invariant.Sometimes(len(diags) == 0, "Diags is empty sometimes")
		sfc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Consts),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by AST budget"})
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by AST budget"})
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Constant_Signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by AST budget"})
		sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Struct_Field_Index),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by AST budget"})
		stdlib := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Stdlib_Local_Names),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_local_names is bounded by AST budget"})
		invariant.Cross_Product(d, diags_empty, sfc, ii, cs, sfi, stdlib,
			invariant.Excluding("Max d empty true", invariant.Bucket_Hi(d), invariant.Bucket_True(diags_empty)),
			invariant.Excluding("Hi d cap is bad", invariant.Bucket_Hi(d), invariant.Bucket_False(diags_empty)),
			invariant.Excluding("Zero d implies empty true", invariant.Bucket_Lo(d), invariant.Bucket_False(diags_empty)),
			invariant.Excluding("Hi sfc empty", invariant.Bucket_Hi(sfc), invariant.Bucket_True(diags_empty)),
			invariant.Excluding("Hi sfc non-empty", invariant.Bucket_Hi(sfc), invariant.Bucket_False(diags_empty)),
			invariant.Excluding("Hi ii empty", invariant.Bucket_Hi(ii), invariant.Bucket_True(diags_empty)),
			invariant.Excluding("Hi ii non-empty", invariant.Bucket_Hi(ii), invariant.Bucket_False(diags_empty)),
			invariant.Excluding("Hi cs empty", invariant.Bucket_Hi(cs), invariant.Bucket_True(diags_empty)),
			invariant.Excluding("Hi cs non-empty", invariant.Bucket_Hi(cs), invariant.Bucket_False(diags_empty)),
			invariant.Excluding("Hi sfi empty", invariant.Bucket_Hi(sfi), invariant.Bucket_True(diags_empty)),
			invariant.Excluding("Hi sfi non-empty", invariant.Bucket_Hi(sfi), invariant.Bucket_False(diags_empty)),
			invariant.Excluding("Hi stdlib empty", invariant.Bucket_Hi(stdlib), invariant.Bucket_True(diags_empty)),
			invariant.Excluding("Hi stdlib non-empty", invariant.Bucket_Hi(stdlib), invariant.Bucket_False(diags_empty)),
		)
	}()
	sfc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by AST budget"})
	sfc_empty := invariant.Sometimes(len(input.Same_File_Consts) == 0, "Same_file_consts is empty sometimes")
	ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by AST budget"})
	cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Constant_Signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by AST budget"})
	...
	stdlib := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Stdlib_Local_Names),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_local_names is bounded by AST budget"})
	invariant.Cross_Product(
		invariant.Always(input != nil, "Input is non-nil"), invariant.Always(input.File_Set != nil, "Input.File_Set is non-nil"),
		invariant.Always(input.Function != nil, "Input.Function is non-nil"),
		sfc, sfc_empty, ii, cs, sfi, stdlib,
		invariant.Excluding("Hi sfc with empty unreachable", invariant.Bucket_Hi(sfc), invariant.Bucket_True(sfc_empty)),
		invariant.Excluding("Hi sfc with non-empty unreachable", invariant.Bucket_Hi(sfc), invariant.Bucket_False(sfc_empty)),
		invariant.Excluding("Lo sfc implies empty true", invariant.Bucket_Lo(sfc), invariant.Bucket_False(sfc_empty)),
		invariant.Excluding("Hi ii with empty sfc unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_True(sfc_empty)),
		invariant.Excluding("Hi ii with non-empty sfc unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_False(sfc_empty)),
		invariant.Excluding("Hi cs with empty sfc unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_True(sfc_empty)),
		invariant.Excluding("Hi cs with non-empty sfc unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_False(sfc_empty)),
		invariant.Excluding("Hi sfi with empty sfc unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_True(sfc_empty)),
		invariant.Excluding("Hi sfi with non-empty sfc unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_False(sfc_empty)),
		invariant.Excluding("Hi stdlib with empty sfc unreachable", invariant.Bucket_Hi(stdlib), invariant.Bucket_True(sfc_empty)),
		invariant.Excluding("Hi stdlib non-empty sfc unreachable", invariant.Bucket_Hi(stdlib), invariant.Bucket_False(sfc_empty)),
	)
	function_label := check_invariant_assertions_function_label(input.Function)
	invariant.Cross_Product(invariant.Always(function_label != "", "Function_label is non-empty at this point"))
	defer_diag := check_invariant_assertions_validate_defer_position(input.File_Set, input.Function, input.Struct_Field_Index)
	invariant.Cross_Product(invariant.Sometimes(defer_diag == nil, "Defer_diag is nil when the defer position is valid"))
	if defer_diag != nil {
	...
	diags = append(diags, check_invariant_assertions_boundary_literal_block(&check_invariant_assertions_boundary_literal_block_input{
		Body: input.Function.Body, Invariant_Idents: input.Invariant_Idents, Function_Label: function_label,
		Same_File_Consts: input.Same_File_Consts, File_Set: input.File_Set, Is_Top_Of_Function: true})...)
	diags = append(diags, check_invariant_assertions_validate_declarations(&check_invariant_assertions_validate_declarations_input{
		File_Set: input.File_Set, Function: input.Function, Function_Label: function_label,
		Invariant_Idents: input.Invariant_Idents, Stdlib_Local_Names: input.Stdlib_Local_Names,
...
func check_invariant_assertions_one_function_pipeline(
	...
	ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by AST budget"})
	ii_empty := invariant.Sometimes(len(input.Invariant_Idents) == 0, "Invariant_idents is empty sometimes")
	cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Constant_Signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by AST budget"})
	sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Struct_Field_Index),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by AST budget"})
	invariant.Cross_Product(
		...
		invariant.Always(input.Function != nil, "Input.Function is non-nil"),
		ii, ii_empty, cs, sfi,
		invariant.Excluding("Hi ii with empty unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_True(ii_empty)),
		invariant.Excluding("Hi ii with non-empty unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_False(ii_empty)),
		invariant.Excluding("Lo ii implies empty true", invariant.Bucket_Lo(ii), invariant.Bucket_False(ii_empty)),
		invariant.Excluding("Hi cs with empty ii unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_True(ii_empty)),
		invariant.Excluding("Hi cs with non-empty ii unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_False(ii_empty)),
		invariant.Excluding("Hi sfi with empty ii unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_True(ii_empty)),
		invariant.Excluding("Hi sfi with non-empty ii unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_False(ii_empty)),
	)
	parameter_covered, defer_covered, always_nil_pointers := check_invariant_assertions_scan_function_prologue(
		input.Function.Body, input.Invariant_Idents, input.Constant_Signs)
	invariant.Cross_Product(invariant.Always(parameter_covered != nil, "Parameter_covered is non-nil at this point"),
		invariant.Always(defer_covered != nil, "Defer_covered is non-nil at this point"),
		invariant.Always(always_nil_pointers != nil, "Always_nil_pointers is non-nil at this point"))
	...
	parameter_empty := invariant.Sometimes(parameter_requirements == nil, "Parameter_requirements is empty sometimes")
	defer_empty := invariant.Sometimes(parameter_defer_requirements == nil, "Parameter_defer_requirements is empty sometimes")
	named_empty := invariant.Sometimes(named_return_requirements == nil, "Named_return_requirements is empty sometimes")
	invariant.Cross_Product(parameter_empty, defer_empty, named_empty,
		invariant.Excluding("Empty params implies empty defer requirements",
			invariant.Bucket_True(parameter_empty), invariant.Bucket_False(defer_empty)))
...
type check_invariant_assertions_validate_one_function_input struct {
	...
	Parameter_Covered            map[string]map[string]check_invariant_assertions_coverage_origin
	Named_Return_Covered         map[string]map[string]check_invariant_assertions_coverage_origin
}
...
func check_invariant_assertions_validate_one_function(
	...
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diags),
			Lo: 0, Hi: max_diagnostics_per_call, Message: "Diags is bounded by budget"})
		pr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Parameter_Requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Parameter_requirements is bounded by AST budget"})
		pdr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Parameter_Defer_Requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Parameter_defer_requirements is bounded by AST budget"})
		nrr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Named_Return_Requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_requirements is bounded by AST budget"})
		pc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Parameter_Covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Parameter_covered is bounded by AST budget"})
		nrc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Named_Return_Covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_covered is bounded by AST budget"})
...
func check_invariant_assertions_is_invariant_package(file_set *token.FileSet, file *ast.File) (yes bool) {
	defer func() {
		invariant.Cross_Product(invariant.Sometimes(
			yes, "Affirmative branch is exercised"))
...
func check_invariant_assertions_collect_invariant_idents(file *ast.File) (idents map[string]bool) {
	defer func() {
		i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Idents is bounded by budget"})
		empty_axis := invariant.Sometimes(len(idents) == 0, "Idents is empty for files without an invariant import")
	...
	const default_path = "github.com/james-orcales/james-orcales/golang_snacks/invariant/v2/invariant_default"
	for _, import_specification := range file.Imports {
		path := strings.Trim(import_specification.Path.Value, `"`)
		if path != pure_path {
...
func check_invariant_assertions_struct_type_has_mutex_field(
	...
	defer func() {
		...
		stdlib_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
			Lo: 0, Hi: max_ast_nodes_per_call,
		...
		invariant.Cross_Product(yes_axis, stdlib_axis,
			...
			invariant.Excluding("Lo stdlib (=0) with yes=true impossible — match requires package in stdlib map",
				invariant.Bucket_Lo(stdlib_axis), invariant.Bucket_True(yes_axis)),
		)
	}()
	stdlib_empty := invariant.Sometimes(len(stdlib_imports) == 0, "Stdlib_imports is empty sometimes")
	stdlib_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
		Lo: 0, Hi: max_ast_nodes_per_call,
		Message: "Stdlib_imports is bounded by AST budget"})
...
func check_invariant_assertions_build_struct_field_index(
	...
	defer func() {
		i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(index),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Index is bounded by budget"})
		empty_axis := invariant.Sometimes(len(index) == 0, "Index is empty sometimes")
		stdlib := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
		invariant.Cross_Product(i, empty_axis, stdlib,
			invariant.Excluding("Max i contradicts empty true", invariant.Bucket_Hi(i), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Index at AST safety cap is bad", invariant.Bucket_Hi(i), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Zero i implies empty true", invariant.Bucket_Lo(i), invariant.Bucket_False(empty_axis)),
	...
	stdlib_p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
	stdlib_empty := invariant.Sometimes(len(stdlib_imports) == 0, "Stdlib_imports is empty sometimes")
	invariant.Cross_Product(invariant.Always(file != nil, "File is non-nil"), stdlib_p, stdlib_empty,
		invariant.Excluding("Hi stdlib unreachable (empty)", invariant.Bucket_Hi(stdlib_p), invariant.Bucket_True(stdlib_empty)),
		invariant.Excluding("Hi stdlib unreachable (non-empty)",
			invariant.Bucket_Hi(stdlib_p), invariant.Bucket_False(stdlib_empty)),
		invariant.Excluding("Lo stdlib implies empty true", invariant.Bucket_Lo(stdlib_p), invariant.Bucket_False(stdlib_empty)),
	)
	...
	for _, declaration := range file.Decls {
		...
		for _, specification := range generic_declaration.Specs {
			...
			index[type_specification.Name.Name] = check_invariant_assertions_struct_information{
				Fields: check_invariant_assertions_extract_struct_fields(struct_type),
				Locked: check_invariant_assertions_struct_type_has_mutex_field(struct_type, stdlib_imports),
			}
		}
	}
	return index
...
func check_invariant_assertions_extract_struct_fields(struct_type *ast.StructType) (fields []check_invariant_assertions_struct_field) {
	defer func() {
		f := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(fields), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Fields is bounded by budget"})
...
func check_invariant_assertions_collect_requirements(
	...
	defer func() {
		pr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(parameter_requirements), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Parameter_requirements is bounded by budget"})
		pd := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(parameter_defer_requirements), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Parameter_defer_requirements is bounded by budget"})
		nr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(named_return_requirements), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Named_return_requirements is bounded by budget"})
		anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Always_Nil_Pointers),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Always_Nil_Pointers is bounded"})
		sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Struct_Field_Index),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Struct_Field_Index is bounded"})
		invariant.Cross_Product(pr, pd, nr, anp, sfi,
			invariant.Excluding("Hi pr Hi pd unreachable", invariant.Bucket_Hi(pr), invariant.Bucket_Hi(pd)),
			invariant.Excluding("Hi pr Lo pd unreachable", invariant.Bucket_Hi(pr), invariant.Bucket_Lo(pd)),
			invariant.Excluding("Lo pr Hi pd unreachable", invariant.Bucket_Lo(pr), invariant.Bucket_Hi(pd)),
			invariant.Excluding("Hi pd Hi nr unreachable", invariant.Bucket_Hi(pd), invariant.Bucket_Hi(nr)),
			invariant.Excluding("Hi pd Lo nr unreachable", invariant.Bucket_Hi(pd), invariant.Bucket_Lo(nr)),
			invariant.Excluding("Lo pd Hi nr unreachable", invariant.Bucket_Lo(pd), invariant.Bucket_Hi(nr)),
			invariant.Excluding("Hi anp Lo pr unreachable", invariant.Bucket_Hi(anp), invariant.Bucket_Lo(pr)),
			invariant.Excluding("Hi sfi Lo pr unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_Lo(pr)),
		)
	}()
	anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Always_Nil_Pointers),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Always_Nil_Pointers is bounded"})
	...
	invariant.Cross_Product(
		...
		anp, sfi,
		invariant.Excluding("Hi sfi Lo anp unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_Lo(anp)),
		invariant.Excluding("Hi anp Lo sfi unreachable", invariant.Bucket_Hi(anp), invariant.Bucket_Lo(sfi)),
		invariant.Excluding("Hi anp Hi sfi unreachable", invariant.Bucket_Hi(anp), invariant.Bucket_Hi(sfi)),
	)
	...
	for _, requirement := range parameter_requirements {
		if requirement.Requires_Defer_Position {
			parameter_defer_requirements = append(parameter_defer_requirements, requirement)
		}
	}
	if input.Function.Type.Results != nil {
		...
		for _, field := range input.Function.Type.Results.List {
			invariant.Cross_Product(invariant.Always(len(field.Names) > 0,
				"Return field is named (tier-1 guarantee from check_namesd_returns)"))
...
func check_invariant_assertions_function_label(function *ast.FuncDecl) (label string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X: len(label), Lo: min_non_empty, Hi: Max_Identifier_Chars,
				Message: "Label is non-empty per construction and bounded by identifier budget",
			}),
		)
	}()
	invariant.Cross_Product(invariant.Always(
...
func check_invariant_assertions_requirements_from_field_list(
	...
	defer func() {
		...
		invariant.Cross_Product(r, empty_axis, anp, sfi,
			invariant.Excluding("Max r contradicts empty true", invariant.Bucket_Hi(r), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Axis r at safety cap is bad", invariant.Bucket_Hi(r), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Zero r implies empty true", invariant.Bucket_Lo(r), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi anp with empty", invariant.Bucket_Hi(anp), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi anp with reqs", invariant.Bucket_Hi(anp), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi sfi with empty", invariant.Bucket_Hi(sfi), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi sfi with reqs", invariant.Bucket_Hi(sfi), invariant.Bucket_False(empty_axis)),
		)
	}()
	anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Always_Nil_Pointers),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Always_Nil_Pointers is bounded"})
	sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Struct_Field_Index),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Struct_Field_Index is bounded"})
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars,
	...
	invariant.Cross_Product(
		...
		anp, sfi, fl,
		invariant.Excluding("Hi anp unreachable (Lo fl)", invariant.Bucket_Hi(anp), invariant.Bucket_Lo(fl)),
		invariant.Excluding("Hi anp unreachable (Hi fl)", invariant.Bucket_Hi(anp), invariant.Bucket_Hi(fl)),
		invariant.Excluding("Hi sfi unreachable (Lo fl)", invariant.Bucket_Hi(sfi), invariant.Bucket_Lo(fl)),
		invariant.Excluding("Hi sfi unreachable (Hi fl)", invariant.Bucket_Hi(sfi), invariant.Bucket_Hi(fl)),
	)
...
func check_invariant_assertions_recursive_frame_emit(
	...
	defer func() {
		r := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by budget"})
		empty_axis := invariant.Sometimes(len(requirements) == 0, "Requirements is empty sometimes")
		anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(tables.Always_Nil_Pointers),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Always_nil_pointers is bounded by budget"})
		sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(tables.Struct_Field_Index),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by budget"})
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Visited is bounded by budget"})
		invariant.Cross_Product(r, empty_axis, anp, sfi, vis,
			invariant.Excluding("Max r contradicts empty true", invariant.Bucket_Hi(r), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi r is bad", invariant.Bucket_Hi(r), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Lo r implies empty true", invariant.Bucket_Lo(r), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi anp unreachable (empty)", invariant.Bucket_Hi(anp), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi anp unreachable (non-empty)", invariant.Bucket_Hi(anp), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi sfi unreachable (empty)", invariant.Bucket_Hi(sfi), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi sfi unreachable (non-empty)", invariant.Bucket_Hi(sfi), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi vis unreachable (empty)", invariant.Bucket_Hi(vis), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi vis unreachable (non-empty)", invariant.Bucket_Hi(vis), invariant.Bucket_False(empty_axis)),
		)
	}()
	input_axis := invariant.Always(input != nil, "Input is non-nil")
	ancestor_axis := invariant.Sometimes(input.Has_Nillable_Ancestor, "Input is under a nillable ancestor sometimes")
	anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(tables.Always_Nil_Pointers),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Always_nil_pointers is bounded by budget"})
	sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(tables.Struct_Field_Index),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by budget"})
	...
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars,
		Message: "Input.Function_Label is non-empty bounded by identifier budget"})
	pa := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Path),
		Lo: min_non_empty, Hi: Max_Identifier_Chars,
	...
	invariant.Cross_Product(input_axis, invariant.Always(tables != nil, "Tables is non-nil"),
		...
		invariant.Excluding("Top-level entry is always at depth zero", invariant.Bucket_True(ancestor_axis)),
		invariant.Excluding("Hi anp unreachable", invariant.Bucket_Hi(anp), invariant.Bucket_False(ancestor_axis)),
		invariant.Excluding("Hi sfi unreachable", invariant.Bucket_Hi(sfi), invariant.Bucket_False(ancestor_axis)),
		invariant.Excluding("Hi vis unreachable", invariant.Bucket_Hi(vis), invariant.Bucket_False(ancestor_axis)),
	)
	...
	for range invariant.Game_Loop() {
		...
		stack, requirements, expand_identifier =
			check_invariant_assertions_recursive_frame_emit_step(stack, top, requirements, is_always_nil)
		loop_sk := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
		loop_stack_empty := invariant.Sometimes(len(stack) == 0, "Stack is empty sometimes")
		loop_requirement_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by budget"})
		loop_requirement_empty := invariant.Sometimes(len(requirements) == 0, "Requirements is empty sometimes")
		invariant.Cross_Product(loop_sk, loop_stack_empty,
			invariant.Excluding("Max loop_stack contradicts loop_stack_empty true",
				invariant.Bucket_Hi(loop_sk), invariant.Bucket_True(loop_stack_empty)),
			invariant.Excluding("Loop_stack at safety cap is bad",
				invariant.Bucket_Hi(loop_sk), invariant.Bucket_False(loop_stack_empty)),
			invariant.Excluding("Zero loop_stack implies loop_stack_empty true",
				invariant.Bucket_Lo(loop_sk), invariant.Bucket_False(loop_stack_empty)),
		)
		invariant.Cross_Product(loop_requirement_axis, loop_requirement_empty,
			invariant.Excluding("Max loop_requirement contradicts loop_requirement_empty true",
				invariant.Bucket_Hi(loop_requirement_axis), invariant.Bucket_True(loop_requirement_empty)),
			invariant.Excluding("Loop_requirement at safety cap is bad",
				invariant.Bucket_Hi(loop_requirement_axis), invariant.Bucket_False(loop_requirement_empty)),
			invariant.Excluding("Zero loop_requirement implies loop_requirement_empty true",
				invariant.Bucket_Lo(loop_requirement_axis), invariant.Bucket_False(loop_requirement_empty)),
		)
		invariant.Cross_Product(invariant.Sometimes(expand_identifier == nil, "Expand_identifier is nil sometimes"))
		// The driver owns struct expansion; is_pointer is re-derived here (not a 4th return).
		if expand_identifier != nil {
			...
			stack, _ = check_invariant_assertions_recursive_frame_push_struct_fields(
				stack, top, expand_identifier, tables.Struct_Field_Index, expand_is_pointer)
			invariant.Cross_Product(invariant.Always(stack != nil, "Stack is non-nil after expansion"))
		}
	}
	return requirements
}
...
func check_invariant_assertions_recursive_frame_emit_step(
	...
	defer func() {
		r := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(next_requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Next_requirements is bounded by budget"})
		re := invariant.Sometimes(len(next_requirements) == 0, "Next_requirements is empty sometimes")
		ns := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(next_stack),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Next_stack is bounded by budget"})
		ds := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
		dr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by budget"})
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
		invariant.Cross_Product(r, re, ns, ds, dr, vis,
			invariant.Excluding("Max r contradicts re true", invariant.Bucket_Hi(r), invariant.Bucket_True(re)),
			invariant.Excluding("Axis r at safety cap is bad", invariant.Bucket_Hi(r), invariant.Bucket_False(re)),
			invariant.Excluding("Zero r implies re true", invariant.Bucket_Lo(r), invariant.Bucket_False(re)),
			invariant.Excluding("Hi ns unreachable (empty)", invariant.Bucket_Hi(ns), invariant.Bucket_True(re)),
			invariant.Excluding("Hi ns unreachable (non-empty)", invariant.Bucket_Hi(ns), invariant.Bucket_False(re)),
			invariant.Excluding("Hi ds unreachable (empty)", invariant.Bucket_Hi(ds), invariant.Bucket_True(re)),
			invariant.Excluding("Hi ds unreachable (non-empty)", invariant.Bucket_Hi(ds), invariant.Bucket_False(re)),
			invariant.Excluding("Hi dr unreachable", invariant.Bucket_Hi(dr), invariant.Bucket_True(re)),
			invariant.Excluding("Hi vis unreachable (empty)", invariant.Bucket_Hi(vis), invariant.Bucket_True(re)),
			invariant.Excluding("Hi vis unreachable (non-empty)", invariant.Bucket_Hi(vis), invariant.Bucket_False(re)),
		)
		expand_axis := invariant.Sometimes(expand_identifier == nil,
			"Expand_identifier is nil for non-struct frames sometimes")
		invariant.Cross_Product(expand_axis)
	}()
	sa := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
	se := invariant.Sometimes(len(stack) == 0, "Stack is empty sometimes")
	p_vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
	invariant.Cross_Product(
		invariant.Always(top != nil, "Top is non-nil"),
		sa, se, p_vis,
		invariant.Excluding("Max stack contradicts se true", invariant.Bucket_Hi(sa), invariant.Bucket_True(se)),
		invariant.Excluding("Stack at safety cap is bad", invariant.Bucket_Hi(sa), invariant.Bucket_False(se)),
		invariant.Excluding("Zero stack implies se true", invariant.Bucket_Lo(sa), invariant.Bucket_False(se)),
		invariant.Excluding("Hi vis unreachable (empty)", invariant.Bucket_Hi(p_vis), invariant.Bucket_True(se)),
		invariant.Excluding("Hi vis unreachable (non-empty)", invariant.Bucket_Hi(p_vis), invariant.Bucket_False(se)),
	)
	pr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by budget"})
	pr_empty := invariant.Sometimes(len(requirements) == 0, "Requirements is empty sometimes")
	invariant.Cross_Product(pr, pr_empty,
		invariant.Excluding("Max pr contradicts pr_empty true", invariant.Bucket_Hi(pr), invariant.Bucket_True(pr_empty)),
		invariant.Excluding("Axis pr at safety cap is bad", invariant.Bucket_Hi(pr), invariant.Bucket_False(pr_empty)),
		invariant.Excluding("Zero pr implies pr_empty true", invariant.Bucket_Lo(pr), invariant.Bucket_False(pr_empty)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Top.function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
	path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Path),
		Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Top.path is non-empty bounded"})
	invariant.Cross_Product(path_axis)
	always_nil_axis := invariant.Sometimes(is_always_nil, "Frame pointer is always-nil sometimes")
	invariant.Cross_Product(always_nil_axis)
	nillable_axis := invariant.Sometimes(top.Has_Nillable_Ancestor, "Top is under a nillable ancestor sometimes")
	...
	if is_pointer {
		requirements = append(requirements, check_invariant_assertions_recursive_frame_pointer_requirement(top))
		if is_always_nil {
			return stack, requirements, nil
		}
	}
	leaf_requirements := check_invariant_assertions_recursive_frame_leaf_dispatch(top, type_expression, is_pointer)
	invariant.Cross_Product(invariant.Sometimes(leaf_requirements == nil, "Leaf_requirements is nil for non-leaf types"))
	...
	if is_ident {
		leaves := check_invariant_assertions_recursive_frame_emit_leaf(top, identifier, is_pointer)
		invariant.Cross_Product(invariant.Sometimes(leaves == nil, "Leaves is nil for non-leaf types"))
		if leaves != nil {
	...
	// When this frame's pointer is stripped, push_inline descendants inherit a nillable ancestor.
	...
	invariant.Cross_Product(invariant.Always(stack != nil, "Stack is non-nil after push_inline"))
	return stack, requirements, nil
}
...
func check_invariant_assertions_recursive_frame_push_struct_fields(
	...
	defer func() {
		ns := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(next_stack),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Next_stack is bounded by budget"})
		ns_empty := invariant.Sometimes(len(next_stack) == 0, "Next_stack is empty sometimes")
		ds := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
		dsfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(struct_field_index),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by budget"})
		dvis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded by budget"})
		invariant.Cross_Product(ns, ns_empty, ds, dsfi, dvis,
			invariant.Excluding("Max ns contradicts ns_empty true", invariant.Bucket_Hi(ns), invariant.Bucket_True(ns_empty)),
			invariant.Excluding("Axis ns at safety cap is bad", invariant.Bucket_Hi(ns), invariant.Bucket_False(ns_empty)),
			invariant.Excluding("Zero ns implies ns_empty true", invariant.Bucket_Lo(ns), invariant.Bucket_False(ns_empty)),
			invariant.Excluding("Hi ds unreachable (empty)", invariant.Bucket_Hi(ds), invariant.Bucket_True(ns_empty)),
			invariant.Excluding("Hi ds unreachable (non-empty)", invariant.Bucket_Hi(ds), invariant.Bucket_False(ns_empty)),
			invariant.Excluding("Hi dsfi unreachable (empty)", invariant.Bucket_Hi(dsfi), invariant.Bucket_True(ns_empty)),
			invariant.Excluding("Hi dsfi unreachable (non-empty)", invariant.Bucket_Hi(dsfi), invariant.Bucket_False(ns_empty)),
			invariant.Excluding("Hi dvis unreachable (empty)", invariant.Bucket_Hi(dvis), invariant.Bucket_True(ns_empty)),
			invariant.Excluding("Hi dvis unreachable (non-empty)", invariant.Bucket_Hi(dvis), invariant.Bucket_False(ns_empty)),
		)
		pushed_axis := invariant.Sometimes(pushed, "Pushed is true when struct fields were appended")
		invariant.Cross_Product(pushed_axis)
	}()
	sk := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
	stack_empty := invariant.Sometimes(len(stack) == 0, "Stack is empty sometimes")
	sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(struct_field_index),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by budget"})
	vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded by budget"})
	nillable_axis := invariant.Sometimes(top.Has_Nillable_Ancestor, "Top is under a nillable ancestor sometimes")
	invariant.Cross_Product(
		...
		invariant.Excluding("Hi stack with empty true is bad", invariant.Bucket_Hi(sk), invariant.Bucket_True(stack_empty)),
		invariant.Excluding("Hi stack with empty F is bad", invariant.Bucket_Hi(sk), invariant.Bucket_False(stack_empty)),
		invariant.Excluding("Lo stack implies empty true", invariant.Bucket_Lo(sk), invariant.Bucket_False(stack_empty)),
		invariant.Excluding("Hi sfi unreachable (empty)", invariant.Bucket_Hi(sfi), invariant.Bucket_True(stack_empty)),
		invariant.Excluding("Hi sfi unreachable (non-empty)", invariant.Bucket_Hi(sfi), invariant.Bucket_False(stack_empty)),
		invariant.Excluding("Hi vis unreachable (empty)", invariant.Bucket_Hi(vis), invariant.Bucket_True(stack_empty)),
		invariant.Excluding("Hi vis unreachable (non-empty)", invariant.Bucket_Hi(vis), invariant.Bucket_False(stack_empty)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Top.function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
...
func check_invariant_assertions_recursive_frame_emit_push_inline(
	...
	defer func() {
		s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
		u := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(updated), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Updated is bounded by budget"})
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
		invariant.Cross_Product(s, u, vis,
			invariant.Excluding("Both s and u at safety caps is bad", invariant.Bucket_Hi(s), invariant.Bucket_Hi(u)),
			invariant.Excluding("Max s with min u is bad", invariant.Bucket_Hi(s), invariant.Bucket_Lo(u)),
			invariant.Excluding("Zero s with max u is bad", invariant.Bucket_Lo(s), invariant.Bucket_Hi(u)),
			invariant.Excluding("Hi vis unreachable (Lo s)", invariant.Bucket_Hi(vis), invariant.Bucket_Lo(s)),
			invariant.Excluding("Hi vis unreachable (Hi s)", invariant.Bucket_Hi(vis), invariant.Bucket_Hi(s)),
		)
	}()
	top_ancestor_axis := invariant.Sometimes(top.Has_Nillable_Ancestor, "Top is under a nillable ancestor sometimes")
	child_ancestor_axis := invariant.Sometimes(child_has_nillable_ancestor, "Child is under a nillable ancestor sometimes")
	sk := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stack), Lo: 0,
		Hi: max_ast_nodes_per_call, Message: "Stack is bounded by budget"})
	vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
	...
	invariant.Cross_Product(
		...
		invariant.Excluding("Nillable ancestor propagates from parent to child",
			invariant.Bucket_True(top_ancestor_axis), invariant.Bucket_False(child_ancestor_axis)),
		...
		invariant.Excluding("Hi vis unreachable (top anc)", invariant.Bucket_Hi(vis), invariant.Bucket_True(top_ancestor_axis)),
		invariant.Excluding("Hi vis unreachable (top-level)", invariant.Bucket_Hi(vis), invariant.Bucket_False(top_ancestor_axis)),
		invariant.Excluding("Empty stack under nillable ancestor with empty visited impossible in test corpus",
			invariant.Bucket_Lo(sk), invariant.Bucket_True(top_ancestor_axis), invariant.Bucket_Lo(vis)),
	)
	path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Path),
		Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Top.path is a non-empty frame path"})
	path_mask := invariant.Sometimes(len(top.Path) == min_non_empty, "Top.path is a single char sometimes")
	invariant.Cross_Product(path_axis, path_mask,
		invariant.Excluding("Hi path mask T", invariant.Bucket_Hi(path_axis), invariant.Bucket_True(path_mask)),
		invariant.Excluding("Hi path mask F", invariant.Bucket_Hi(path_axis), invariant.Bucket_False(path_mask)),
		invariant.Excluding("Lo path mask F", invariant.Bucket_Lo(path_axis), invariant.Bucket_False(path_mask)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Top.function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
	...
	for _, child_field := range struct_type.Fields.List {
		...
		for _, child_name := range child_field.Names {
			...
			stack = append(stack, &check_invariant_assertions_recursive_frame{
				Type_Expression: child_field.Type, Path: top.Path + "." + child_name.Name, Position: child_name.Pos(),
...
func check_invariant_assertions_recursive_frame_emit_leaf(
	...
	defer func() {
		l := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(leaves),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Leaves is bounded by budget"})
		empty_axis := invariant.Sometimes(leaves == nil, "Leaves is nil for non-leaf types")
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Visited is bounded"})
		invariant.Cross_Product(l, empty_axis, vis,
			invariant.Excluding("Max len contradicts empty true", invariant.Bucket_Hi(l), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Axis l at safety cap is bad", invariant.Bucket_Hi(l), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Zero len implies empty true", invariant.Bucket_Lo(l), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi vis unreachable (empty)", invariant.Bucket_Hi(vis), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi vis unreachable (non-empty)", invariant.Bucket_Hi(vis), invariant.Bucket_False(empty_axis)),
		)
	}()
	ancestor_axis := invariant.Sometimes(input.Has_Nillable_Ancestor, "Input is under a nillable ancestor sometimes")
	pointer_axis := invariant.Sometimes(is_pointer, "Is_pointer is true when the leaf is behind a pointer")
	vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Visited is bounded"})
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars,
	...
	invariant.Cross_Product(
		...
		invariant.Excluding("Hi vis unreachable", invariant.Bucket_Hi(vis), invariant.Bucket_False(ancestor_axis)),
		invariant.Excluding("Hi vis unreachable (ancestor)", invariant.Bucket_Hi(vis), invariant.Bucket_True(ancestor_axis)),
	)
	invariant.Cross_Product(fl)
	invariant.Cross_Product(pa)
	...
	field_description := check_invariant_assertions_field_description(input.Path, input.Type_Expression)
	invariant.Cross_Product(invariant.Always(field_description != "", "Field_description is non-empty for tracked leaves"))
	kinds := check_invariant_assertions_keyword_kinds(identifier.Name)
	invariant.Cross_Product(invariant.Sometimes(kinds == nil, "Kinds is nil for non-primitive identifiers"))
	if kinds == nil {
		return nil
	}
...
func check_invariant_assertions_recursive_frame_pointer_requirement(
	...
	defer func() {
		...
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
		fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Function_Label),
			Lo: min_function_label_chars, Hi: Max_Identifier_Chars,
			Message: "Requirement.Function_Label is non-empty bounded by identifier budget"})
		ip := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Identifier_Path),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Requirement.Identifier_Path is non-empty bounded by identifier budget"})
		invariant.Cross_Product(nillable, requires_defer, vis,
			invariant.Always(requirement.Kind != "", "Requirement.Kind is non-empty"),
			invariant.Always(len(requirement.Kind) == pointer_requirement_kind_chars, "Kind is the fixed pointer label"),
			invariant.Excluding("Pointer requirements leave Requires_Defer_Position unset",
				invariant.Bucket_True(requires_defer)),
			invariant.Excluding("Hi vis unreachable (nillable)", invariant.Bucket_Hi(vis), invariant.Bucket_True(nillable)),
			invariant.Excluding("Hi vis unreachable (top-level)", invariant.Bucket_Hi(vis), invariant.Bucket_False(nillable)),
		)
		invariant.Cross_Product(fl)
		invariant.Cross_Product(ip)
		field_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Field_Description),
			Lo: min_requirement_field_description_chars, Hi: max_field_description_chars,
			Message: "Field_description spans short pointer to long slice typename"})
		invariant.Cross_Product(field_axis)
	}()
	nillable := invariant.Sometimes(top.Has_Nillable_Ancestor, "Top is under a nillable ancestor sometimes")
	vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars,
		Message: "Top.Function_Label is non-empty bounded by identifier budget"})
	pa := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Path),
		Lo: min_non_empty, Hi: Max_Identifier_Chars,
	...
	return check_invariant_assertions_requirement{
		...
		Field_Description:     check_invariant_assertions_field_description(top.Path, top.Type_Expression),
		Function_Label:        top.Function_Label,
...
func check_invariant_assertions_recursive_frame_leaf_dispatch(
	...
	defer func() {
		nil_axis := invariant.Sometimes(leaves == nil, "Leaves is nil when neither container nor channel matched")
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
		invariant.Cross_Product(nil_axis, vis,
			invariant.Excluding("Hi vis unreachable (nil)", invariant.Bucket_Hi(vis), invariant.Bucket_True(nil_axis)),
			invariant.Excluding("Hi vis unreachable (non-nil)", invariant.Bucket_Hi(vis), invariant.Bucket_False(nil_axis)),
		)
		fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
			Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Top.function_label is non-empty bounded"})
		invariant.Cross_Product(fl)
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Path),
			Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Top.path is non-empty bounded"})
		invariant.Cross_Product(path_axis)
		leaves_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(leaves),
			Lo: 0, Hi: max_leaf_requirements_per_dispatch, Message: "Leaves is 0 (miss) or 3 (channel triple)"})
		invariant.Cross_Product(leaves_axis)
	}()
	pointer_axis := invariant.Sometimes(is_pointer, "Container sits behind a pointer indirection sometimes")
	vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
	invariant.Cross_Product(
		invariant.Always(top != nil, "Top is non-nil"),
		...
		pointer_axis, vis,
		invariant.Excluding("Hi vis unreachable (pointer)", invariant.Bucket_Hi(vis), invariant.Bucket_True(pointer_axis)),
		invariant.Excluding("Hi vis unreachable (non-pointer)", invariant.Bucket_Hi(vis), invariant.Bucket_False(pointer_axis)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Top.function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
	...
	nillable_axis := invariant.Sometimes(top.Has_Nillable_Ancestor, "Top is under a nillable ancestor sometimes")
	invariant.Cross_Product(nillable_axis)
	leaves = check_invariant_assertions_recursive_frame_container_leaf(top, type_expression, is_pointer)
	leaves_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(leaves),
		Lo: 0, Hi: min_pair, Message: "Leaves is 0 (non-container) or 2 (container triple)"})
	invariant.Cross_Product(leaves_axis)
	if leaves != nil {
		return leaves
	}
	return check_invariant_assertions_recursive_frame_channel_leaf(top, type_expression, is_pointer)
}
...
func check_invariant_assertions_recursive_frame_container_leaf(
	...
	defer func() {
		l := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(leaves),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Leaves is bounded by budget"})
		empty_axis := invariant.Sometimes(leaves == nil, "Leaves is nil for non-length-mutable type expressions")
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
		invariant.Cross_Product(l, empty_axis, vis,
			invariant.Excluding("Max len contradicts empty true", invariant.Bucket_Hi(l), invariant.Bucket_True(empty_axis)),
	...
	is_slice := invariant.Sometimes(check_invariant_assertions_is_slice_type(type_expression), "Slice branch reachable")
	is_map := invariant.Sometimes(check_invariant_assertions_is_map_type(type_expression), "Map branch reachable")
	has_nillable := invariant.Sometimes(top.Has_Nillable_Ancestor, "Has_Nillable_Ancestor flag reachable")
	pointer_axis := invariant.Sometimes(is_pointer, "Container sits behind a pointer indirection sometimes")
	...
	invariant.Cross_Product(p_vis, p_vis_empty,
		invariant.Excluding("Max p_vis contradicts empty true", invariant.Bucket_Hi(p_vis), invariant.Bucket_True(p_vis_empty)),
		invariant.Excluding("Axis p_vis at safety cap is bad", invariant.Bucket_Hi(p_vis), invariant.Bucket_False(p_vis_empty)),
		invariant.Excluding("Zero p_vis implies empty true", invariant.Bucket_Lo(p_vis), invariant.Bucket_False(p_vis_empty)),
	)
	zero_kind := ""
	...
	field_description := check_invariant_assertions_field_description(leaf_path, type_expression)
...
func check_invariant_assertions_recursive_frame_channel_leaf(
	...
	defer func() {
		l := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(leaves),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Leaves is bounded by budget"})
		empty_axis := invariant.Sometimes(leaves == nil, "Leaves is nil for non-channel type expressions")
		vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
		invariant.Cross_Product(l, empty_axis, vis,
			invariant.Excluding("Max len contradicts empty true", invariant.Bucket_Hi(l), invariant.Bucket_True(empty_axis)),
	...
	is_channel := invariant.Sometimes(check_invariant_assertions_is_channel_type(type_expression),
		"Channel branch reachable")
	pointer_axis := invariant.Sometimes(is_pointer, "Channel sits behind a pointer indirection sometimes")
	vis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Visited),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Top.Visited is bounded"})
	invariant.Cross_Product(
		invariant.Always(top != nil, "Top is non-nil"),
		...
		invariant.Excluding("Hi vis unreachable (channel)", invariant.Bucket_Hi(vis), invariant.Bucket_True(is_channel)),
		invariant.Excluding("Hi vis unreachable (non-channel)", invariant.Bucket_Hi(vis), invariant.Bucket_False(is_channel)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(top.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Top.function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
	...
	field_description := check_invariant_assertions_field_description(channel_path, type_expression)
	invariant.Cross_Product(invariant.Always(field_description != "", "Field_description is non-empty for channels"))
	cap_field_description := check_invariant_assertions_field_description(cap_path, type_expression)
	invariant.Cross_Product(invariant.Always(cap_field_description != "", "Cap_field_description is non-empty for channels"))
	leaves = append(leaves, check_invariant_assertions_requirement{
...
func check_invariant_assertions_keyword_kinds(name string) (kinds []string) {
	defer func() {
		k := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(kinds),
			Lo: 0, Hi: max_string_slice_per_call, Message: "Kinds is bounded by budget"})
		empty_axis := invariant.Sometimes(kinds == nil, "Kinds is nil for non-primitive keywords")
	...
	name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
		X: len(name), Lo: min_non_empty, Hi: Max_Identifier_Chars,
		Message: "Name is a Go type identifier; primitives like `complex128` and user types up to identifier budget",
	})
	single_axis := invariant.Sometimes(len(name) == count_one, "Name is exactly 1 char sometimes")
...
func check_invariant_assertions_field_description(name string, type_expression ast.Expr) (description string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X: len(description), Lo: min_field_description_chars, Hi: max_field_description_chars,
				Message: "Description is `name type_str` (≥3 chars: 1-char name + space + 1-char type)",
			}),
		)
	}()
	invariant.Cross_Product(
...
type check_invariant_assertions_coverage_origin struct {
	From_Cross_Product bool
...
func check_invariant_assertions_scan_function_prologue(
	...
	defer func() {
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		pc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(parameter_covered), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Parameter_covered is bounded by budget"})
		nrc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(named_return_covered), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Named_return_covered is bounded by budget"})
		anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(always_nil_pointers), Lo: 0,
			Hi: max_ast_nodes_per_call, Message: "Always_nil_pointers is bounded by budget"})
		invariant.Cross_Product(ii, cs, pc, nrc, anp,
			invariant.Excluding("Both ii and cs at safety caps is bad", invariant.Bucket_Hi(ii), invariant.Bucket_Hi(cs)),
			invariant.Excluding("Axis ii at safety cap is bad", invariant.Bucket_Hi(ii), invariant.Bucket_Lo(cs)),
			invariant.Excluding("Both cs and pc at safety caps is bad", invariant.Bucket_Hi(cs), invariant.Bucket_Hi(pc)),
			invariant.Excluding("Max cs with zero pc is bad", invariant.Bucket_Hi(cs), invariant.Bucket_Lo(pc)),
			invariant.Excluding("Zero cs with max pc is bad", invariant.Bucket_Lo(cs), invariant.Bucket_Hi(pc)),
			invariant.Excluding("Both pc and nrc at safety caps is bad", invariant.Bucket_Hi(pc), invariant.Bucket_Hi(nrc)),
			invariant.Excluding("Max pc with zero nrc is bad", invariant.Bucket_Hi(pc), invariant.Bucket_Lo(nrc)),
			invariant.Excluding("Zero pc with max nrc is bad", invariant.Bucket_Lo(pc), invariant.Bucket_Hi(nrc)),
			invariant.Excluding("Both nrc and anp at safety caps is bad", invariant.Bucket_Hi(nrc), invariant.Bucket_Hi(anp)),
			invariant.Excluding("Max nrc with zero anp is bad", invariant.Bucket_Hi(nrc), invariant.Bucket_Lo(anp)),
			invariant.Excluding("Zero nrc with max anp is bad", invariant.Bucket_Lo(nrc), invariant.Bucket_Hi(anp)),
		)
	}()
	ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents), Lo: 0,
		Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	...
	invariant.Cross_Product(
		...
		ii, cs,
		invariant.Excluding("Both ii and cs at safety caps is bad", invariant.Bucket_Hi(ii), invariant.Bucket_Hi(cs)),
		invariant.Excluding("Max ii with zero cs is bad", invariant.Bucket_Hi(ii), invariant.Bucket_Lo(cs)),
		invariant.Excluding("Zero ii with max cs is bad", invariant.Bucket_Lo(ii), invariant.Bucket_Hi(cs)),
	)
	...
	for _, statement := range body.List {
		if defer_statement, is_defer := statement.(*ast.DeferStmt); is_defer {
			if !defer_seen {
				...
				check_invariant_assertions_credit_defer(
					defer_statement, invariant_idents, constant_signs, named_return_covered)
				continue
			}
			break
		}
		...
		is_cross := check_invariant_assertions_is_cross_product_statement(statement, invariant_idents)
		invariant.Cross_Product(invariant.Sometimes(is_cross, "Statement is a top-level Cross_Product sometimes"))
		if is_cross {
			if cross_product_seen {
				continue
			}
		}
...
type check_invariant_assertions_credit_prologue_statement_input struct {
	...
	Invariant_Idents    map[string]bool
	Constant_Signs      map[string]int
	Covered             map[string]map[string]check_invariant_assertions_coverage_origin
	Always_Nil_Pointers map[string]bool
	Outside_If_Guard    bool
}
...
func check_invariant_assertions_credit_prologue_statement(
	...
	defer func() {
		credited_axis := invariant.Sometimes(credited, "Statement was credited as an invariant call")
		idents_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded by AST budget"})
		signs_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Constant_Signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Constant_Signs is bounded by AST budget"})
		covered_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Covered is bounded by AST budget"})
		anp_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Always_Nil_Pointers),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Always_Nil_Pointers is bounded by AST budget"})
		invariant.Cross_Product(credited_axis, idents_axis, signs_axis, covered_axis, anp_axis,
			invariant.Excluding("Hi idents unreachable (credited)",
				invariant.Bucket_Hi(idents_axis), invariant.Bucket_True(credited_axis)),
			invariant.Excluding("Hi idents unreachable (non-credited)",
				invariant.Bucket_Hi(idents_axis), invariant.Bucket_False(credited_axis)),
			invariant.Excluding("Hi signs unreachable (credited)",
				invariant.Bucket_Hi(signs_axis), invariant.Bucket_True(credited_axis)),
			invariant.Excluding("Hi signs unreachable (non-credited)",
				invariant.Bucket_Hi(signs_axis), invariant.Bucket_False(credited_axis)),
			invariant.Excluding("Hi covered unreachable (credited)",
				invariant.Bucket_Hi(covered_axis), invariant.Bucket_True(credited_axis)),
			invariant.Excluding("Hi covered unreachable (non-credited)",
				invariant.Bucket_Hi(covered_axis), invariant.Bucket_False(credited_axis)),
			invariant.Excluding("Hi always_nil_pointers unreachable (credited)",
				invariant.Bucket_Hi(anp_axis), invariant.Bucket_True(credited_axis)),
			invariant.Excluding("Hi always_nil_pointers unreachable (non-credited)",
				invariant.Bucket_Hi(anp_axis), invariant.Bucket_False(credited_axis)),
			...
			invariant.Excluding("Crediting requires a populated Invariant_Idents map",
				invariant.Bucket_Lo(idents_axis), invariant.Bucket_True(credited_axis)),
		)
	}()
	idents_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded by AST budget"})
	...
	anp_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Always_Nil_Pointers),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Always_Nil_Pointers is bounded by AST budget"})
	outside_axis := invariant.Sometimes(input.Outside_If_Guard, "Statement is outside any if-guard")
	invariant.Cross_Product(
		invariant.Always(input != nil, "Input is non-nil"),
	...
	call, is_axis_builder_binding := check_invariant_assertions_extract_prologue_call(input.Statement, input.Invariant_Idents)
	call_axis := invariant.Sometimes(call == nil, "Call is nil for non-prologue statements")
	binding_axis := invariant.Sometimes(is_axis_builder_binding, "Statement is an axis-builder binding")
	...
	invariant.Cross_Product(
		...
		invariant.Excluding("Call nil with axis_builder_binding true is impossible by construction",
	...
	if is_axis_builder_binding {
		pairs = check_invariant_assertions_axis_builder_covered_pairs(call, input.Invariant_Idents, input.Constant_Signs)
		...
		invariant.Cross_Product(invariant.Always(pairs != nil, "Pairs is non-nil for axis-builder bindings"))
	} else {
		pairs = check_invariant_assertions_call_covered_pairs(call, input.Invariant_Idents, input.Constant_Signs)
		invariant.Cross_Product(invariant.Sometimes(pairs == nil, "Pairs is nil for unrecognised invariant calls"))
	}
	if pairs == nil {
		return false
	}
	...
	if input.Always_Nil_Pointers != nil {
		for _, path := range check_invariant_assertions_collect_always_nil_paths(call, input.Invariant_Idents) {
...
func check_invariant_assertions_is_cross_product_statement(
	...
	defer func() {
		yes_axis := invariant.Sometimes(yes, "Statement is a top-level Cross_Product sometimes")
		empty_idents_axis := invariant.Sometimes(len(invariant_idents) == 0,
			"Invariant_idents is empty for files without invariant import")
		defer_idents := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
		invariant.Cross_Product(yes_axis, empty_idents_axis, defer_idents,
			invariant.Excluding("Empty idents (file lacks invariant import) implies yes false",
				invariant.Bucket_True(yes_axis), invariant.Bucket_True(empty_idents_axis)),
			invariant.Excluding("Max defer_idents contradicts empty_idents true",
				invariant.Bucket_Hi(defer_idents), invariant.Bucket_True(empty_idents_axis)),
			invariant.Excluding("Defer_idents at safety cap is bad",
				invariant.Bucket_Hi(defer_idents), invariant.Bucket_False(empty_idents_axis)),
			invariant.Excluding("Zero defer_idents implies empty_idents true",
				invariant.Bucket_Lo(defer_idents), invariant.Bucket_False(empty_idents_axis)),
		)
	}()
	prologue_ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	empty_idents_axis := invariant.Sometimes(len(invariant_idents) == 0, "Invariant_idents is empty for files without invariant import")
	invariant.Cross_Product(prologue_ii, empty_idents_axis,
		...
		invariant.Excluding("Prologue invariant_idents at safety cap is bad",
			invariant.Bucket_Hi(prologue_ii), invariant.Bucket_False(empty_idents_axis)),
		invariant.Excluding("Zero prologue invariant_idents implies empty_idents true",
			invariant.Bucket_Lo(prologue_ii), invariant.Bucket_False(empty_idents_axis)),
	)
	...
	invariant.Cross_Product(invariant.Sometimes(name == "", "Name is empty for non-invariant calls"))
...
func check_invariant_assertions_walk_assertion_chain(
	...
	defer func() {
		cv := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Covered is bounded by AST budget"})
		cv_empty := invariant.Sometimes(len(input.Covered) == 0, "Input.Covered is empty sometimes")
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded by AST budget"})
		ii_empty := invariant.Sometimes(len(input.Invariant_Idents) == 0, "Input.Invariant_Idents is empty sometimes")
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Constant_Signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Constant_Signs is bounded by AST budget"})
		anp := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Always_Nil_Pointers),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Always_Nil_Pointers is bounded by AST budget"})
		invariant.Cross_Product(cv, cv_empty, ii, ii_empty, cs, anp,
			invariant.Excluding("Hi cv with empty unreachable", invariant.Bucket_Hi(cv), invariant.Bucket_True(cv_empty)),
			invariant.Excluding("Hi cv with non-empty unreachable", invariant.Bucket_Hi(cv), invariant.Bucket_False(cv_empty)),
			invariant.Excluding("Lo cv implies empty true", invariant.Bucket_Lo(cv), invariant.Bucket_False(cv_empty)),
			invariant.Excluding("Hi ii with empty unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_True(ii_empty)),
			invariant.Excluding("Hi ii with non-empty unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_False(ii_empty)),
			invariant.Excluding("Lo ii implies empty true", invariant.Bucket_Lo(ii), invariant.Bucket_False(ii_empty)),
	...
	invariant.Cross_Product(cv, cv_empty, ii, ii_empty, cs,
		...
		invariant.Excluding("Hi ii with non-empty unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_False(ii_empty)),
		invariant.Excluding("Lo ii implies empty true", invariant.Bucket_Lo(ii), invariant.Bucket_False(ii_empty)),
		invariant.Excluding("Hi cs with empty cv unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_True(cv_empty)),
		invariant.Excluding("Hi cs with non-empty cv unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_False(cv_empty)),
	)
	...
	for len(stack) > 0 {
		...
		for _, statement := range top.Statements {
			...
			is_cross := check_invariant_assertions_is_cross_product_statement(statement, input.Invariant_Idents)
			invariant.Cross_Product(invariant.Sometimes(is_cross, "Statement is a top-level Cross_Product sometimes"))
			if is_cross {
...
func check_invariant_assertions_axis_builder_covered_pairs(
	...
	defer func() {
		p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(pairs),
			Lo: 0, Hi: max_coverage_pairs_per_call, Message: "Pairs is bounded by budget"})
		i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call,
			Message: "Invariant_idents is ≥1 (callable path requires invariant import)"})
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		invariant.Cross_Product(p, i, c,
			invariant.Excluding("Both p and i at safety caps is bad", invariant.Bucket_Hi(p), invariant.Bucket_Hi(i)),
			invariant.Excluding("Axis p at safety cap is bad", invariant.Bucket_Hi(p), invariant.Bucket_Lo(i)),
			invariant.Excluding("Both idents and signs at safety caps is bad", invariant.Bucket_Hi(i), invariant.Bucket_Hi(c)),
			invariant.Excluding("Invariant_idents at safety cap is bad", invariant.Bucket_Hi(i), invariant.Bucket_Lo(c)),
			invariant.Excluding("Constant_signs at safety cap is bad", invariant.Bucket_Lo(i), invariant.Bucket_Hi(c)),
		)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is ≥1 (callable path requires invariant import)"})
	c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
	invariant.Cross_Product(invariant.Always(call != nil, "Call is non-nil"),
		i, c,
		invariant.Excluding("Both idents and signs at AST safety caps is bad", invariant.Bucket_Hi(i), invariant.Bucket_Hi(c)),
		invariant.Excluding("Invariant_idents at AST at safety cap is bad", invariant.Bucket_Hi(i), invariant.Bucket_Lo(c)),
		invariant.Excluding("Constant_signs at AST at safety cap is bad", invariant.Bucket_Lo(i), invariant.Bucket_Hi(c)),
	)
	helper_name := check_invariant_assertions_extract_call_name(call, invariant_idents)
	invariant.Cross_Product(invariant.Always(helper_name != "", "Helper_name is non-empty for invariant calls"))
	...
	invariant.Cross_Product(invariant.Always(bare_table != nil, "Bare_table is non-nil at this point"))
	bare_kind, is_bare := bare_table[helper_name]
	if !is_bare {
		return nil
	}
	paths := check_invariant_assertions_extract_identifier_paths(call.Args)
	invariant.Cross_Product(invariant.Sometimes(paths == nil, "Paths is nil for axis builders without identifier args"))
	pairs = []check_invariant_assertions_coverage_pair{}
	...
	if check_invariant_assertions_boundary_input_has_zero_endpoint(call.Args, constant_signs) {
		for _, path := range paths {
			for _, zero_kind := range []string{"zero_int", "zero_float", "zero_string", "zero_slice", "zero_map"} {
				pairs = append(pairs, check_invariant_assertions_coverage_pair{
					Path:               path,
					Kind:               zero_kind,
					From_Cross_Product: true,
	...
	path_axis := invariant.Sometimes(comparison_path == "", "Comparison_path is empty for non-comparison helpers")
	kinds_axis := invariant.Sometimes(comparison_kinds == nil, "Comparison_kinds is nil for non-comparison helpers")
	invariant.Cross_Product(
...
func check_invariant_assertions_extract_prologue_call(
	...
	defer func() {
		...
		invariant.Cross_Product(i, call_axis, binding_axis,
			...
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_True(call_axis), invariant.Bucket_False(binding_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_True(call_axis), invariant.Bucket_True(binding_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i),
				invariant.Bucket_False(call_axis), invariant.Bucket_False(binding_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_False(call_axis), invariant.Bucket_True(binding_axis)),
			invariant.Excluding("Zero invariant_idents with axis-builder binding is impossible",
				invariant.Bucket_Lo(i), invariant.Bucket_False(call_axis), invariant.Bucket_True(binding_axis)),
		)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	empty_axis := invariant.Sometimes(len(invariant_idents) == 0, "Invariant_idents is empty for files without invariant import")
	invariant.Cross_Product(i, empty_axis,
		invariant.Excluding("Max i contradicts empty true",
			invariant.Bucket_Hi(i), invariant.Bucket_True(empty_axis)),
		invariant.Excluding("Invariant_idents at AST at safety cap is bad",
	...
	invariant.Cross_Product(invariant.Sometimes(rhs_name == "", "Rhs_name is empty for non-invariant calls"))
	if rhs_name == "" {
		return nil, false
	}
	bare_table := map[string]string{
...
func check_invariant_assertions_collect_always_nil_paths(
	...
	defer func() {
		p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(paths),
			Lo: 0, Hi: max_string_slice_per_call, Message: "Paths is bounded by budget"})
		empty_axis := invariant.Sometimes(paths == nil, "Paths is nil for non-always-nil calls")
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call,
		...
		invariant.Cross_Product(p, empty_axis, ii,
			...
			invariant.Excluding("Zero paths with empty false at min invariant_idents is bad",
				invariant.Bucket_Lo(p), invariant.Bucket_False(empty_axis), invariant.Bucket_Lo(ii)),
		)
	}()
	ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is ≥1 (call path requires invariant import)"})
	single_axis := invariant.Sometimes(len(invariant_idents) == count_one, "Invariant_idents is exactly 1 sometimes")
	invariant.Cross_Product(
		invariant.Always(call != nil, "Call is non-nil"),
		ii, single_axis,
	...
	if is_cross {
		...
		for i := argument_offset; i < len(call.Args); i++ {
			...
			inner_name := check_invariant_assertions_extract_call_name(inner_call, invariant_idents)
			invariant.Cross_Product(invariant.Sometimes(inner_name == "", "Inner_name is empty for non-invariant inner calls"))
			if inner_name == "" {
				continue
			}
			path := check_invariant_assertions_extract_eq_nil_path(inner_call, inner_name)
			invariant.Cross_Product(invariant.Sometimes(path == "", "Path is empty for non-Always-eq-nil shapes"))
			if path != "" {
...
func check_invariant_assertions_extract_eq_nil_path(call *ast.CallExpr, helper_name string) (path string) {
	defer func() {
		empty_axis := invariant.Sometimes(path == "", "Path is empty for non-Always-eq-nil shapes")
		size_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X:       len(path),
			Lo:      0,
			Hi:      Max_Identifier_Chars,
...
func check_invariant_assertions_is_assertion_defer_shape(defer_statement *ast.DeferStmt) (yes bool) {
	defer func() {
		invariant.Cross_Product(invariant.Sometimes(
...
func check_invariant_assertions_validate_no_compound_predicates(
	...
	ast.Inspect(function.Body, func(node ast.Node) (descend bool) {
		...
		helper_name := check_invariant_assertions_extract_call_name(call, invariant_idents)
		invariant.Cross_Product(invariant.Sometimes(helper_name == "", "Helper_name is empty for non-invariant calls"))
		if helper_name == "" {
			return true
		}
		predicate_index := check_invariant_assertions_nil_predicate_index(helper_name)
		invariant.Cross_Product(invariant.Sometimes(predicate_index < 0, "Predicate_index is negative for non-predicate helpers"))
...
func check_invariant_assertions_validate_defer_position(
	...
	defer func() {
		sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(struct_field_index),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by budget"})
		empty_axis := invariant.Sometimes(len(struct_field_index) == 0, "Struct_field_index is empty sometimes")
		invariant.Cross_Product(
			invariant.Sometimes(diag == nil, "Diag is nil when the defer position is valid"),
			sfi, empty_axis,
			invariant.Excluding("Max sfi contradicts empty true", invariant.Bucket_Hi(sfi), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Sfi at safety cap is bad", invariant.Bucket_Hi(sfi), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Zero sfi implies empty true", invariant.Bucket_Lo(sfi), invariant.Bucket_False(empty_axis)),
		)
		if diag != nil {
			invariant.Cross_Product(invariant.Always(diag.Tier == 0, "Tier is 0 at construction"))
			name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Name),
				Lo: min_defer_position_name_chars, Hi: max_defer_position_name_chars,
				Message: "Diag.Name is one of the defer-position diagnostic name strings"})
			invariant.Cross_Product(name_axis)
			want_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Want),
				Lo: min_defer_position_want_chars, Hi: max_defer_position_want_chars,
				Message: "Diag.Want is `add` (71), `move` (76), or `place` (103) defer-position hint"})
			invariant.Cross_Product(want_axis)
			message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Message),
				Lo: min_defer_position_message_chars, Hi: max_diagnostic_message_chars,
				Message: "Diag.Message is the Sprintf defer-position hint within budget"})
			message_name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Name),
				Lo: min_defer_position_name_chars, Hi: max_defer_position_name_chars,
				Message: "Diag.Name distinguishes the defer-position variant"})
			invariant.Cross_Product(message_axis, message_name_axis,
				invariant.Excluding("Hi message at Lo name unreachable",
					invariant.Bucket_Hi(message_axis), invariant.Bucket_Lo(message_name_axis)),
				invariant.Excluding("Hi message at Hi name unreachable",
					invariant.Bucket_Hi(message_axis), invariant.Bucket_Hi(message_name_axis)),
				invariant.Excluding("Lo message at Lo name unreachable",
					invariant.Bucket_Lo(message_axis), invariant.Bucket_Lo(message_name_axis)),
			)
		}
	}()
	sfi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(struct_field_index),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Struct_field_index is bounded by budget"})
	single_axis := invariant.Sometimes(len(struct_field_index) == count_one, "Struct_field_index is exactly 1 sometimes")
	invariant.Cross_Product(
		invariant.Always(file_set != nil, "File_set is non-nil"),
		invariant.Always(function != nil, "Function is non-nil"),
		sfi, single_axis,
		invariant.Excluding("Max sfi contradicts single true", invariant.Bucket_Hi(sfi), invariant.Bucket_True(single_axis)),
		invariant.Excluding("Sfi at safety cap is bad", invariant.Bucket_Hi(sfi), invariant.Bucket_False(single_axis)),
		invariant.Excluding("Zero sfi contradicts single true", invariant.Bucket_Lo(sfi), invariant.Bucket_True(single_axis)),
	)
	...
	invariant.Cross_Product(invariant.Always(function_label != "", "Function_label is non-empty for named FuncDecls"))
...
func check_invariant_assertions_defer_position_violation(
	...
	defer func() {
		invariant.Cross_Product(invariant.Sometimes(diag == nil, "Diag is nil when the defer position is valid"))
		if diag != nil {
			invariant.Cross_Product(invariant.Always(diag.Tier == 0, "Tier is 0 at construction"))
			name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Name),
				Lo: min_defer_position_name_chars, Hi: max_defer_position_name_chars,
	...
	for index, statement := range function.Body.List {
		...
		if index > 0 {
			return &Diagnostic{
				...
				Want:     "move the assertion defer to be the very first statement of the function body",
				Message: fmt.Sprintf(
		...
		if !check_invariant_assertions_is_assertion_defer_shape(defer_statement) {
			return &Diagnostic{
				...
				Message: fmt.Sprintf(
					"%s: assertion defer must be the very first statement;"+
						" a cleanup defer appears before the assertion defer",
					function_label),
			}
		}
		return nil
...
func check_invariant_assertions_credit_defer(
	...
	defer func() {
		...
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		nrc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(named_return_covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_covered is bounded by budget"})
		invariant.Cross_Product(ii, cs, nrc,
			invariant.Excluding("Both invariant_idents and constant_signs at safety caps is bad",
				invariant.Bucket_Hi(ii), invariant.Bucket_Hi(cs)),
			invariant.Excluding("Invariant_idents at safety cap with zero constant_signs is bad",
			...
			invariant.Excluding("Both constant_signs and named_return_covered at safety caps is bad",
...
func check_invariant_assertions_record_coverage(
	...
	defer func() {
		p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(pairs),
			Lo: 0, Hi: max_coverage_pairs_per_call, Message: "Pairs is bounded by budget"})
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by budget"})
	...
	c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by budget"})
	outside := invariant.Sometimes(outside_if_guard, "At least one credit comes from outside an if-guard")
	invariant.Cross_Product(
	...
	for _, pair := range pairs {
		if covered[pair.Path] == nil {
			covered[pair.Path] = map[string]check_invariant_assertions_coverage_origin{}
		}
...
func check_invariant_assertions_call_covered_pairs(
	...
	defer func() {
		...
		check_invariant_assertions_call_covered_pairs_defer_axes(pairs, invariant_idents, constant_signs)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
	...
	check_invariant_assertions_call_covered_pairs_prologue_axes(call, invariant_idents, constant_signs)
	...
	invariant.Cross_Product(invariant.Always(of_helpers != nil, "Of_helpers is non-nil at this point"))
	if helper_kind, has := of_helpers[helper_name]; has {
		pairs = []check_invariant_assertions_coverage_pair{}
		paths := check_invariant_assertions_extract_identifier_paths(call.Args)
	...
	return check_invariant_assertions_call_covered_pairs_cross_product(call, helper_name, invariant_idents, constant_signs)
}
...
func check_invariant_assertions_numeric_constant_name_set(constant_signs map[string]int) (names map[string]bool) {
	defer func() {
		n := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(names),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Names is bounded by AST budget"})
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by AST budget"})
		empty := invariant.Sometimes(len(names) == 0, "Names is empty sometimes")
		invariant.Cross_Product(n, c, empty,
			invariant.Excluding("Hi n with empty unreachable", invariant.Bucket_Hi(n), invariant.Bucket_True(empty)),
			invariant.Excluding("Hi n non-empty unreachable", invariant.Bucket_Hi(n), invariant.Bucket_False(empty)),
			invariant.Excluding("Lo n implies empty true", invariant.Bucket_Lo(n), invariant.Bucket_False(empty)),
			invariant.Excluding("Hi c with empty unreachable", invariant.Bucket_Hi(c), invariant.Bucket_True(empty)),
			invariant.Excluding("Hi c non-empty unreachable", invariant.Bucket_Hi(c), invariant.Bucket_False(empty)),
		)
	}()
	cc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by AST budget"})
	cc_empty := invariant.Sometimes(len(constant_signs) == 0, "Constant_signs is empty sometimes")
	invariant.Cross_Product(cc, cc_empty,
		invariant.Excluding("Hi cc with empty unreachable", invariant.Bucket_Hi(cc), invariant.Bucket_True(cc_empty)),
		invariant.Excluding("Hi cc non-empty unreachable", invariant.Bucket_Hi(cc), invariant.Bucket_False(cc_empty)),
		invariant.Excluding("Lo cc implies empty true", invariant.Bucket_Lo(cc), invariant.Bucket_False(cc_empty)),
	)
	names = map[string]bool{}
...
func check_invariant_assertions_call_covered_pairs_cross_product(
	...
	defer func() {
		...
		p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(pairs),
			Lo: 0, Hi: max_coverage_pairs_per_call, Message: "Pairs is bounded by budget"})
		h := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(helper_name),
			Lo: cross_product_helper_chars, Hi: recorder_cross_product_helper_chars,
			Message: "Helper_name length matches Cross_Product (13) or Recorder_Cross_Product (22) per caller gate"})
		invariant.Cross_Product(i, c, p, h,
			invariant.Excluding("Both idents and signs at safety caps is bad", invariant.Bucket_Hi(i), invariant.Bucket_Hi(c)),
			invariant.Excluding("Invariant_idents at safety cap is bad", invariant.Bucket_Hi(i), invariant.Bucket_Lo(c)),
			invariant.Excluding("Constant_signs at safety cap is bad", invariant.Bucket_Lo(i), invariant.Bucket_Hi(c)),
			invariant.Excluding("Both constants and pairs at safety caps", invariant.Bucket_Hi(c), invariant.Bucket_Hi(p)),
			invariant.Excluding("Constants at safety cap with zero pairs", invariant.Bucket_Hi(c), invariant.Bucket_Lo(p)),
			invariant.Excluding("Zero constants with max pairs is bad", invariant.Bucket_Lo(c), invariant.Bucket_Hi(p)),
			...
			invariant.Excluding("Recorder_Cross_Product with only pre-bound axes is unused",
				invariant.Bucket_Lo(i), invariant.Bucket_Lo(c), invariant.Bucket_Lo(p), invariant.Bucket_Hi(h)),
		)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call,
	...
	for index := argument_offset; index < len(call.Args); index++ {
		...
		bare_kind, is_bare := bare_table[inner_name]
		if is_bare {
			pairs = append(pairs,
				check_invariant_assertions_bare_axis_pairs(inner_call, bare_kind, constant_signs)...)
		}
		comparison_path, comparison_kinds := check_invariant_assertions_extract_nil_comparison_path(
			inner_call, inner_name, check_invariant_assertions_numeric_constant_name_set(constant_signs))
		path_axis := invariant.Sometimes(comparison_path == "", "Comparison_path is empty for non-comparison helpers")
		kinds_axis := invariant.Sometimes(comparison_kinds == nil, "Comparison_kinds is nil for non-comparison helpers")
		invariant.Cross_Product(
			path_axis, kinds_axis,
...
func check_invariant_assertions_bare_axis_pairs(
	...
	defer func() {
		...
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		pairs_nil_axis := invariant.Sometimes(pairs == nil, "Pairs is nil for inner calls without identifier coverage")
		invariant.Cross_Product(p, c, pairs_nil_axis,
			...
			invariant.Excluding("Zero pairs with max constant_signs is bad",
				invariant.Bucket_Lo(p), invariant.Bucket_Hi(c)),
			invariant.Excluding("Non-nil pairs with zero p, c is impossible by construction",
				invariant.Bucket_Lo(p), invariant.Bucket_Lo(c), invariant.Bucket_False(pairs_nil_axis)),
		)
	}()
	c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
	...
	if !check_invariant_assertions_boundary_input_has_zero_endpoint(inner_call.Args, constant_signs) {
		if !check_invariant_assertions_boundary_input_excludes_zero(inner_call.Args, constant_signs) {
			return pairs
		}
	}
	for _, path := range paths {
...
func check_invariant_assertions_extract_nil_comparison_path(
	...
	defer func() {
		k := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(kinds),
			Lo: 0, Hi: max_string_slice_per_call, Message: "Kinds is bounded by budget"})
		s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(same_file_consts),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by budget"})
		path_axis := invariant.Sometimes(path == "", "Path is empty for non-comparison shapes")
		kinds_axis := invariant.Sometimes(kinds == nil, "Kinds is nil for non-comparison shapes")
		size_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X:       len(path),
			Lo:      0,
			Hi:      Max_Identifier_Chars,
		...
		invariant.Cross_Product(
			...
			invariant.Excluding("Path false with zero kinds is impossible by construction",
				invariant.Bucket_Lo(k), invariant.Bucket_Lo(s), invariant.Bucket_False(path_axis)),
			invariant.Excluding("Path false with zero kinds is impossible by construction",
				invariant.Bucket_Lo(k), invariant.Bucket_Hi(s), invariant.Bucket_False(path_axis)),
			invariant.Excluding("Path true with zero kinds is impossible by construction",
				invariant.Bucket_Lo(k), invariant.Bucket_Hi(s), invariant.Bucket_True(path_axis)),
		)
	}()
	s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(same_file_consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by budget"})
	...
	predicate_index := check_invariant_assertions_nil_predicate_index(helper_name)
	invariant.Cross_Product(invariant.Sometimes(predicate_index == 0, "Predicate_index can be zero on this branch"))
	if predicate_index < 0 {
		return "", nil
	}
	if predicate_index >= len(call.Args) {
...
func check_invariant_assertions_extract_binary_path(
	...
	defer func() {
		...
		invariant.Cross_Product(k, s, path_axis,
			...
			invariant.Excluding("Max-identifier path with empty kinds is impossible by construction",
				invariant.Bucket_Lo(k), invariant.Bucket_Lo(s), invariant.Bucket_Hi(path_axis)),
		)
	}()
	s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(same_file_consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by budget"})
	...
	if is_nil_ident {
		if nil_ident.Name == "nil" {
			return check_invariant_assertions_expression_to_path(binary.X), []string{"pointer"}
		}
	}
	...
	if string_literal, is_string := binary.Y.(*ast.BasicLit); is_string {
		if string_literal.Kind == token.STRING {
			if string_literal.Value == "\"\"" {
				return check_invariant_assertions_expression_to_path(binary.X), []string{"zero_string"}
			}
			if string_literal.Value == "``" {
				return check_invariant_assertions_expression_to_path(binary.X), []string{"zero_string"}
			}
	...
	return check_invariant_assertions_numeric_credit(binary, allow_neq, same_file_consts, lhs_path)
}
...
func check_invariant_assertions_numeric_credit(
	...
	defer func() {
		...
		invariant.Cross_Product(s, k, path_axis,
			...
			invariant.Excluding("Max-identifier path with empty kinds is impossible by construction",
				invariant.Bucket_Lo(s), invariant.Bucket_Lo(k), invariant.Bucket_Hi(path_axis)),
		)
	}()
	s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(same_file_consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by budget"})
	...
	is_zero_literal, is_integer_shape := check_invariant_assertions_numeric_literal_shape(binary.Y)
	invariant.Cross_Product(
		invariant.Sometimes(is_zero_literal, "Is_zero_literal can be true for canonical zero RHS"),
		invariant.Sometimes(is_integer_shape, "Is_integer_shape can be true for int-typed RHS"),
	)
	is_inline_integer := check_invariant_assertions_is_inline_literal(binary.Y, same_file_consts)
...
func check_invariant_assertions_numeric_literal_shape(expression ast.Expr) (is_zero bool, is_integer_shape bool) {
	defer func() {
		invariant.Cross_Product(
			invariant.Sometimes(is_zero, "Is_zero is true for canonical zero literals"),
			invariant.Sometimes(is_integer_shape, "Is_integer_shape is true for int-typed literals"),
		)
	}()
...
func check_invariant_assertions_extract_finite_path(
	...
	case "IsNaN":
		...
		if allow_neq {
			if !negated {
				return argument_path, []string{"nan_float", "boundary_float", "zero_float"}
			}
		}
	...
	case "IsInf":
		...
		if allow_neq {
			if !negated {
				return argument_path, []string{"inf_float", "boundary_float", "zero_float"}
			}
		}
...
func check_invariant_assertions_nil_predicate_index(helper_name string) (index int) {
	defer func() {
		index_boundary := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			...
			Message: "Index is -1 for unknown helpers, 0 for naked, 1 for Recorder_-prefixed",
		})
		index_zero := invariant.Sometimes(index == 0,
...
func check_invariant_assertions_nil_allows_neq(helper_name string) (allow bool) {
	...
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			...
			Message: "Helper_name is in the Always/Sometimes nil-eq family; longest is `Recorder_Always` (15)",
		}),
		invariant.Always(helper_name != "",
			"Helper_name is non-empty per upstream gate"),
	)
...
func check_invariant_assertions_extract_call_name(
	...
	defer func() {
		i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
		empty_axis := invariant.Sometimes(name == "", "Name is empty for non-invariant calls")
		size_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(name), Lo: 0, Hi: max_invariant_helper_name_chars,
			Message: "Name is empty or the invariant helper name; longest is `Recorder_Is_Distinct_Boundary` (29)",
		})
		invariant.Cross_Product(i, empty_axis, size_axis,
			...
			invariant.Excluding("Non-empty input with zero size contradicts size definition",
				invariant.Bucket_False(empty_axis), invariant.Bucket_Lo(size_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_True(empty_axis), invariant.Bucket_Lo(size_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_True(empty_axis), invariant.Bucket_Hi(size_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_False(empty_axis), invariant.Bucket_Lo(size_axis)),
			invariant.Excluding("Invariant_idents at safety cap is bad",
				invariant.Bucket_Hi(i), invariant.Bucket_False(empty_axis), invariant.Bucket_Hi(size_axis)),
			invariant.Excluding("Zero invariant_idents with max helper name is bad",
				invariant.Bucket_Lo(i), invariant.Bucket_False(empty_axis), invariant.Bucket_Hi(size_axis)),
		)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
...
func check_invariant_assertions_extract_identifier_paths(args []ast.Expr) (paths []string) {
	defer func() {
		p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(paths),
			Lo: 0, Hi: max_string_slice_per_call, Message: "Paths is bounded by budget"})
		a := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(args),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Args is ≥1 per call"})
	...
	a := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(args),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Args is ≥1 per call"})
	single_axis := invariant.Sometimes(len(args) == count_one, "Args has exactly one entry sometimes")
	...
	for _, argument := range args {
		...
		invariant.Cross_Product(invariant.Sometimes(path == "", "Path is empty for non-ident-chain args"))
		if path != "" {
			paths = append(paths, path)
			continue
		}
...
func check_invariant_assertions_input_struct_paths(expression ast.Expr) (paths []string) {
	...
	for _, element := range composite.Elts {
		...
		if path == "" {
			...
			path = check_invariant_assertions_expression_to_size_argument_path(key_value.Value)
			invariant.Cross_Product(invariant.Sometimes(path == "",
...
func check_invariant_assertions_boundary_input_has_zero_endpoint(call_args []ast.Expr, constant_signs map[string]int) (yes bool) {
	defer func() {
		ca := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(call_args),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Call_args is ≥1 per call"})
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
	...
	for _, element := range composite.Elts {
		...
		if check_invariant_assertions_is_zero_inline_literal(key_value.Value, constant_signs) {
			return true
		}
	}
	return false
...
// zero entirely).
func check_invariant_assertions_boundary_input_excludes_zero(call_args []ast.Expr, constant_signs map[string]int) (yes bool) {
	defer func() {
		ca := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(call_args),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Call_args is ≥1 per call"})
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
	...
	for _, element := range composite.Elts {
		...
		if key_ident.Name == "Lo" {
			if check_invariant_assertions_is_positive_inline_literal(key_value.Value, constant_signs) {
				return true
			}
		}
		if key_ident.Name == "Hi" {
			if check_invariant_assertions_is_negative_inline_literal(key_value.Value, constant_signs) {
				return true
			}
		}
	}
...
func check_invariant_assertions_expression_to_size_argument_path(expression ast.Expr) (path string) {
	defer func() {
		empty_axis := invariant.Sometimes(path == "", "Path is empty for non-len() expressions")
		size_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X:       len(path),
			Lo:      0,
			Hi:      Max_Identifier_Chars,
	...
	if function_ident.Name == "cap" {
		...
		invariant.Cross_Product(invariant.Always(inner != "", "Inner path resolves for cap(X) where X is an ident chain"))
		return "cap(" + inner + ")"
	}
...
func check_invariant_assertions_expression_to_path(expression ast.Expr) (path string) {
	defer func() {
		empty_axis := invariant.Sometimes(path == "", "Path is empty for non-ident-chain expressions")
		size_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X:       len(path),
			Lo:      0,
			Hi:      Max_Identifier_Chars,
...
// constant reference.
func check_invariant_assertions_collect_same_file_constants(file *ast.File) (consts map[string]bool) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(consts),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Consts is bounded by budget"})
...
func check_invariant_assertions_collect_same_file_constant_signs(file *ast.File) (constant_signs map[string]int) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		empty_axis := invariant.Sometimes(len(constant_signs) == 0, "Constant_signs is empty sometimes")
		invariant.Cross_Product(c, empty_axis,
			invariant.Excluding("Max c contradicts empty true",
				invariant.Bucket_Hi(c), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Constant_signs at AST at safety cap is bad",
	...
	for _, declaration := range file.Decls {
		...
		for _, specification := range generic_declaration.Specs {
			...
			for i_index, name := range value_specification.Names {
				...
				ok_axis := invariant.Sometimes(ok, "Sign resolution succeeded for this const value")
				...
				sign_zero_axis := invariant.Sometimes(sign == 0, "Sign is zero for resolved zero const values")
				invariant.Cross_Product(
...
func check_invariant_assertions_is_inline_literal(
	...
	s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(same_file_consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Same_file_consts is bounded by budget"})
	empty_axis := invariant.Sometimes(len(same_file_consts) == 0, "Same_file_consts is empty sometimes")
...
func check_invariant_assertions_is_zero_inline_literal(expression ast.Expr, constant_signs map[string]int) (yes bool) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		yes_axis := invariant.Sometimes(yes, "Affirmative branch is exercised")
...
func check_invariant_assertions_is_positive_inline_literal(expression ast.Expr, constant_signs map[string]int) (yes bool) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		yes_axis := invariant.Sometimes(yes, "Affirmative branch is exercised")
...
func check_invariant_assertions_is_negative_inline_literal(expression ast.Expr, constant_signs map[string]int) (yes bool) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		yes_axis := invariant.Sometimes(yes, "Affirmative branch is exercised")
...
func check_invariant_assertions_expression_sign(expression ast.Expr, constant_signs map[string]int) (sign int, ok bool) {
	defer func() {
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
	...
	if unary, is_unary := expression.(*ast.UnaryExpr); is_unary {
		...
		inner_ok_axis := invariant.Sometimes(inner_ok, "Inner_ok reflects the inner literal's resolvability")
		...
		invariant.Cross_Product(
			...
			invariant.Excluding("Inner sign extracted despite inner_ok equals false",
				invariant.Bucket_False(inner_ok_axis), invariant.Bucket_Lo(inner_sign_axis)),
			invariant.Excluding("Inner sign extracted despite inner_ok equals false",
				invariant.Bucket_False(inner_ok_axis), invariant.Bucket_Hi(inner_sign_axis)),
		)
		if !inner_ok {
			return 0, false
		}
...
func check_invariant_assertions_basic_lit_sign(literal *ast.BasicLit) (sign int, ok bool) {
	defer func() {
		ok_axis := invariant.Sometimes(ok, "Sign was resolved")
		sign_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: sign, Lo: 0, Hi: sign_positive,
			Message: "Sign is 0 (zero literal) or +1 (positive literal); bare BasicLit stays non-negative",
		})
		sign_zero_axis := invariant.Sometimes(sign == 0, "Sign is zero for zero literals or non-numeric kinds")
		...
		invariant.Cross_Product(
			...
			invariant.Excluding("Negative sign contradicts sign_zero true",
				invariant.Bucket_Lo(sign_axis), invariant.Bucket_False(sign_zero_axis)),
		)
	}()
	invariant.Cross_Product(invariant.Always(
...
func check_invariant_assertions_validate_boundary_input(
	...
	defer func() {
		...
		label_axis_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Input.Function_Label is non-empty bounded by identifier budget"})
		consts_axis_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Consts),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Same_File_Consts is bounded by AST budget"})
		invariant.Cross_Product(d, empty_axis, label_axis_d, consts_axis_d,
			...
			invariant.Excluding("Hi label with empty diags unreachable",
				invariant.Bucket_Hi(label_axis_d), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi label with non-empty diags unreachable",
				invariant.Bucket_Hi(label_axis_d), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi consts with empty diags unreachable",
				invariant.Bucket_Hi(consts_axis_d), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi consts with non-empty diags unreachable",
				invariant.Bucket_Hi(consts_axis_d), invariant.Bucket_False(empty_axis)),
		)
	}()
	label_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_non_empty, Hi: Max_Identifier_Chars,
		Message: "Input.Function_Label is non-empty bounded by identifier budget"})
	consts_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Same_File_Consts is bounded by AST budget"})
	invariant.Cross_Product(
		invariant.Always(input != nil, "Input is non-nil"),
	...
	for _, element := range composite.Elts {
		...
		if check_invariant_assertions_is_inline_literal(key_value.Value, input.Same_File_Consts) {
			continue
		}
		diags = append(diags, Diagnostic{
			Position: input.File_Set.Position(input.Call.Pos()),
			Name:     "invariant_boundary_non_literal",
			Want: "use an inline literal (numeric literal, signed literal, same-file const, " +
				"or package-qualified selector) for Lo and Hi — they anchor the AFL boundary " +
				"buckets and must not vary at runtime",
			Message: fmt.Sprintf("%s: Boundary %s must be an inline literal — got `%s`",
				input.Function_Label, key_ident.Name, types.ExprString(key_value.Value)),
		})
	}
	return diags
}
...
func check_invariant_assertions_boundary_literal_call(
	...
	defer func() {
		...
		empty_axis := invariant.Sometimes(len(diags) == 0, "Diags is empty sometimes")
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded"})
		invariant.Cross_Product(d, empty_axis, ii,
			invariant.Excluding("Hi diags with empty unreachable", invariant.Bucket_Hi(d), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi diags with diags is bad", invariant.Bucket_Hi(d), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Lo d implies empty true", invariant.Bucket_Lo(d), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi ii with empty unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi ii with diags unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_False(empty_axis)),
		)
	}()
	invariant.Cross_Product(
		invariant.Always(input != nil, "Input is non-nil"),
	...
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Function_label is non-empty bounded"})
	invariant.Cross_Product(fl)

	helper_name := check_invariant_assertions_extract_call_name(input.Call, input.Invariant_Idents)
	invariant.Cross_Product(invariant.Sometimes(helper_name == "", "Helper_name is empty for non-invariant calls"))
	if helper_name == "" {
		return nil
	}
	...
	for i := argument_offset; i < len(input.Call.Args); i++ {
		...
		inner_name := check_invariant_assertions_extract_call_name(inner_call, input.Invariant_Idents)
		invariant.Cross_Product(invariant.Sometimes(inner_name == "", "Inner_name is empty for non-invariant axis calls"))
		if inner_name == "" {
			continue
		}
		is_boundary_axis := false
		switch inner_name {
		case "Distinct_Boundary", "Recorder_Distinct_Boundary", "Is_Distinct_Boundary", "Recorder_Is_Distinct_Boundary":
...
func check_invariant_assertions_boundary_literal_block(
	...
	defer func() {
		...
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded"})
		sfc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Consts),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Same_File_Consts is bounded"})
		invariant.Cross_Product(d, nil_axis, ii, sfc,
			invariant.Excluding("Hi d with nil unreachable", invariant.Bucket_Hi(d), invariant.Bucket_True(nil_axis)),
			invariant.Excluding("Hi d with diags unreachable", invariant.Bucket_Hi(d), invariant.Bucket_False(nil_axis)),
			invariant.Excluding("Lo d implies nil true", invariant.Bucket_Lo(d), invariant.Bucket_False(nil_axis)),
			invariant.Excluding("Hi ii unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_True(nil_axis)),
			invariant.Excluding("Hi sfc unreachable", invariant.Bucket_Hi(sfc), invariant.Bucket_True(nil_axis)),
		)
	}()
	body_nil_axis := invariant.Sometimes(input.Body == nil, "Body is nil for FuncDecls with external linkage")
	top_axis := invariant.Sometimes(input.Is_Top_Of_Function, "Recursion entered from a nested context sometimes")
	ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded"})
	sfc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Same_File_Consts),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Same_File_Consts is bounded"})
	invariant.Cross_Product(
		...
		invariant.Always(input.File_Set != nil, "File_set is non-nil"),
		top_axis, ii, sfc,
		invariant.Excluding("FuncDecls with external linkage are filtered before reaching here",
		...
		invariant.Excluding("Hi ii unreachable", invariant.Bucket_Hi(ii), invariant.Bucket_False(body_nil_axis)),
		invariant.Excluding("Hi sfc unreachable", invariant.Bucket_Hi(sfc), invariant.Bucket_False(body_nil_axis)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
	...
	stack := []Frame{{Body: input.Body, Is_Top_Of_Function: input.Is_Top_Of_Function}}
	for range invariant.Game_Loop() {
		if len(stack) == 0 {
			return diags
		}
		...
		for _, statement := range top.Body.List {
			...
			if if_statement, is_if := statement.(*ast.IfStmt); is_if {
				if check_invariant_assertions_is_nil_guard(if_statement.Cond) {
					if if_statement.Body != nil {
						stack = append(stack, Frame{Body: if_statement.Body, Is_Top_Of_Function: false})
					}
					continue
				}
				break
			}
			call, _ := check_invariant_assertions_extract_prologue_call(statement, input.Invariant_Idents)
			invariant.Cross_Product(invariant.Sometimes(call == nil, "Call is nil for non-prologue statements"))
			if call == nil {
				break
			}
			diags = append(diags, check_invariant_assertions_boundary_literal_call(
...
func check_invariant_assertions_validate(
	...
	defer func() {
		...
		invariant.Cross_Product(d, ea, pr, pc, pdr,
			invariant.Excluding("Max d empty true", invariant.Bucket_Hi(d), invariant.Bucket_True(ea)),
			invariant.Excluding("Hi d cap is bad", invariant.Bucket_Hi(d), invariant.Bucket_False(ea)),
			invariant.Excluding("Lo d implies empty true", invariant.Bucket_Lo(d), invariant.Bucket_False(ea)),
			invariant.Excluding("Hi pr with empty d unreachable", invariant.Bucket_Hi(pr), invariant.Bucket_True(ea)),
			invariant.Excluding("Hi pr with non-empty d unreachable", invariant.Bucket_Hi(pr), invariant.Bucket_False(ea)),
			invariant.Excluding("Hi pc with empty d unreachable", invariant.Bucket_Hi(pc), invariant.Bucket_True(ea)),
			invariant.Excluding("Hi pc with non-empty d unreachable", invariant.Bucket_Hi(pc), invariant.Bucket_False(ea)),
			invariant.Excluding("Hi pdr with empty d unreachable", invariant.Bucket_Hi(pdr), invariant.Bucket_True(ea)),
			invariant.Excluding("Hi pdr with non-empty d unreachable", invariant.Bucket_Hi(pdr), invariant.Bucket_False(ea)),
		)
		nrr_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Named_Return_Requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_requirements is bounded by AST budget"})
		nrr_d_empty := invariant.Sometimes(len(input.Named_Return_Requirements) == 0, "Named returns empty sometimes")
		nrc_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Named_Return_Covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_covered is bounded by AST budget"})
		nrc_d_empty := invariant.Sometimes(len(input.Named_Return_Covered) == 0, "Named_return_covered is empty sometimes")
		invariant.Cross_Product(nrr_d, nrr_d_empty,
			invariant.Excluding("Hi nrr emp T", invariant.Bucket_Hi(nrr_d), invariant.Bucket_True(nrr_d_empty)),
			invariant.Excluding("Hi nrr emp F", invariant.Bucket_Hi(nrr_d), invariant.Bucket_False(nrr_d_empty)),
			invariant.Excluding("Lo nrr emp F", invariant.Bucket_Lo(nrr_d), invariant.Bucket_False(nrr_d_empty)),
		)
		invariant.Cross_Product(nrc_d, nrc_d_empty,
			invariant.Excluding("Hi nrc emp T", invariant.Bucket_Hi(nrc_d), invariant.Bucket_True(nrc_d_empty)),
			invariant.Excluding("Hi nrc emp F", invariant.Bucket_Hi(nrc_d), invariant.Bucket_False(nrc_d_empty)),
			invariant.Excluding("Lo nrc emp F", invariant.Bucket_Lo(nrc_d), invariant.Bucket_False(nrc_d_empty)),
		)
	}()
	pr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Parameter_Requirements),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Parameter_requirements is bounded by AST budget"})
	pr_empty := invariant.Sometimes(len(input.Parameter_Requirements) == 0, "Parameter_requirements is empty sometimes")
	pc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Parameter_Covered),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Parameter_covered is bounded by AST budget"})
	pc_empty := invariant.Sometimes(len(input.Parameter_Covered) == 0, "Parameter_covered is empty sometimes")
	nrc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Named_Return_Covered),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_covered is bounded by AST budget"})
	pdr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Parameter_Defer_Requirements),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Parameter_defer_requirements is bounded by AST budget"})
	invariant.Cross_Product(
		...
		invariant.Excluding("Hi pr with empty unreachable", invariant.Bucket_Hi(pr), invariant.Bucket_True(pr_empty)),
		invariant.Excluding("Hi pr with non-empty unreachable", invariant.Bucket_Hi(pr), invariant.Bucket_False(pr_empty)),
		invariant.Excluding("Lo pr implies empty true", invariant.Bucket_Lo(pr), invariant.Bucket_False(pr_empty)),
		invariant.Excluding("Hi pc with empty unreachable", invariant.Bucket_Hi(pc), invariant.Bucket_True(pc_empty)),
		invariant.Excluding("Hi pc with non-empty unreachable", invariant.Bucket_Hi(pc), invariant.Bucket_False(pc_empty)),
		invariant.Excluding("Lo pc implies empty true", invariant.Bucket_Lo(pc), invariant.Bucket_False(pc_empty)),
		invariant.Excluding("Hi nrc unreachable in test corpus", invariant.Bucket_Hi(nrc), invariant.Bucket_True(pr_empty)),
		invariant.Excluding("Hi pdr unreachable in test corpus", invariant.Bucket_Hi(pdr), invariant.Bucket_True(pr_empty)),
	)
	nrr := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Named_Return_Requirements),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Named_return_requirements is bounded by AST budget"})
	nrr_empty := invariant.Sometimes(len(input.Named_Return_Requirements) == 0, "Named returns empty sometimes")
	invariant.Cross_Product(nrr, nrr_empty,
		invariant.Excluding("Hi nrr emp T", invariant.Bucket_Hi(nrr), invariant.Bucket_True(nrr_empty)),
		invariant.Excluding("Hi nrr emp F", invariant.Bucket_Hi(nrr), invariant.Bucket_False(nrr_empty)),
		invariant.Excluding("Lo nrr emp F", invariant.Bucket_Lo(nrr), invariant.Bucket_False(nrr_empty)),
	)
	...
	diags = append(diags, check_invariant_assertions_validate_requirements(
		input.File_Set, input.Parameter_Defer_Requirements, input.Named_Return_Covered, "param_defer")...)
	diags = append(diags, check_invariant_assertions_validate_requirements(
		input.File_Set, input.Named_Return_Requirements, input.Named_Return_Covered, "named_return")...)
	return diags
}
...
func check_invariant_assertions_validate_requirements(file_set *token.FileSet,
	...
	defer func() {
		...
		requirement_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by AST budget"})
		requirement_d_empty := invariant.Sometimes(len(requirements) == 0, "Requirements is empty sometimes")
		covered_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by AST budget"})
		covered_d_empty := invariant.Sometimes(len(covered) == 0, "Covered map is empty sometimes")
		position_d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(position),
			Lo: min_validate_position_chars, Hi: max_validate_position_chars, Message: "Position spans param to named_return"})
		invariant.Cross_Product(position_d)
		invariant.Cross_Product(d, d_empty,
			invariant.Excluding("Hi d empty T", invariant.Bucket_Hi(d), invariant.Bucket_True(d_empty)),
			invariant.Excluding("Hi d empty F", invariant.Bucket_Hi(d), invariant.Bucket_False(d_empty)),
			invariant.Excluding("Lo d empty F", invariant.Bucket_Lo(d), invariant.Bucket_False(d_empty)),
		)
		invariant.Cross_Product(requirement_d, requirement_d_empty,
			invariant.Excluding("Hi rq emp T", invariant.Bucket_Hi(requirement_d), invariant.Bucket_True(requirement_d_empty)),
			invariant.Excluding("Hi rq emp F", invariant.Bucket_Hi(requirement_d), invariant.Bucket_False(requirement_d_empty)),
			invariant.Excluding("Lo rq emp F", invariant.Bucket_Lo(requirement_d), invariant.Bucket_False(requirement_d_empty)),
		)
		invariant.Cross_Product(covered_d, covered_d_empty,
			invariant.Excluding("Hi cv empty T", invariant.Bucket_Hi(covered_d), invariant.Bucket_True(covered_d_empty)),
			invariant.Excluding("Hi cv empty F", invariant.Bucket_Hi(covered_d), invariant.Bucket_False(covered_d_empty)),
			invariant.Excluding("Lo cv empty F", invariant.Bucket_Lo(covered_d), invariant.Bucket_False(covered_d_empty)),
		)
	}()
	requirement_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirements),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Requirements is bounded by AST budget"})
	requirement_axis_empty := invariant.Sometimes(len(requirements) == 0, "Requirements is empty sometimes")
	covered_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by AST budget"})
	covered_axis_empty := invariant.Sometimes(len(covered) == 0, "Covered map is empty sometimes")
	position_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(position),
		Lo: min_validate_position_chars, Hi: max_validate_position_chars, Message: "Position spans param to named_return"})
	invariant.Cross_Product(invariant.Always(file_set != nil, "File_set is non-nil"))
	invariant.Cross_Product(position_axis)
	invariant.Cross_Product(requirement_axis, requirement_axis_empty,
		invariant.Excluding("Hi rq emp T", invariant.Bucket_Hi(requirement_axis), invariant.Bucket_True(requirement_axis_empty)),
		invariant.Excluding("Hi rq emp F", invariant.Bucket_Hi(requirement_axis), invariant.Bucket_False(requirement_axis_empty)),
		invariant.Excluding("Lo rq emp F", invariant.Bucket_Lo(requirement_axis), invariant.Bucket_False(requirement_axis_empty)),
	)
	invariant.Cross_Product(covered_axis, covered_axis_empty,
		invariant.Excluding("Hi cv empty T", invariant.Bucket_Hi(covered_axis), invariant.Bucket_True(covered_axis_empty)),
		invariant.Excluding("Hi cv empty F", invariant.Bucket_Hi(covered_axis), invariant.Bucket_False(covered_axis_empty)),
		invariant.Excluding("Lo cv empty F", invariant.Bucket_Lo(covered_axis), invariant.Bucket_False(covered_axis_empty)),
	)
	for _, requirement := range requirements {
		status := check_invariant_assertions_coverage_status(covered, requirement)
		invariant.Cross_Product(invariant.Sometimes(status == 0, "Status is uncovered sometimes"))
		if status == coverage_status_uncovered {
			diags = append(diags, check_invariant_assertions_build_diagnostic(file_set, requirement, position))
			continue
		}
		if status == coverage_status_inside_if_only {
			diags = append(diags, check_invariant_assertions_build_inside_if_diagnostic(file_set, requirement, position))
			continue
		}
...
func check_invariant_assertions_build_inside_if_diagnostic(
	...
	defer func() {
		...
		invariant.Cross_Product(
			invariant.Always(diag.Tier == 0, "Tier is 0 at construction"),
			invariant.Always(diag.Name != "", "Diag.Name is non-empty for inside-if-only diagnostics"),
			invariant.Always(len(diag.Name) == inside_if_diagnostic_name_chars, "Diag.Name is the fixed inside-if label"),
			invariant.Always(diag.Want != "", "Diag.Want is non-empty for inside-if-only diagnostics"),
			invariant.Always(len(diag.Want) == inside_if_diagnostic_want_chars, "Diag.Want is the fixed inside-if hint"),
			invariant.Always(diag.Message != "", "Diag.Message is non-empty for inside-if-only diagnostics"),
		)
		message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Message),
			Lo: min_inside_if_message_chars, Hi: max_diagnostic_message_chars,
			Message: "Diag.Message is the inside-if-only message"})
		message_mask := invariant.Sometimes(len(requirement.Kind) == min_inside_if_kind_chars,
		...
		invariant.Cross_Product(message_axis, message_mask,
			invariant.Excluding("Hi message unreachable (zero kind)",
				invariant.Bucket_Hi(message_axis), invariant.Bucket_True(message_mask)),
			invariant.Excluding("Hi message unreachable (boundary kind)",
				invariant.Bucket_Hi(message_axis), invariant.Bucket_False(message_mask)))
	}()
	...
	kind_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
		X: len(requirement.Kind), Lo: min_inside_if_kind_chars, Hi: max_inside_if_kind_chars,
		Message: "Kind is an integer-leaf assertion label: zero_int (8) to boundary_int (12)",
	})
	...
	invariant.Cross_Product(kind_axis, kind_is_zero,
		invariant.Excluding("Lo kind zero F", invariant.Bucket_Lo(kind_axis), invariant.Bucket_False(kind_is_zero)),
		invariant.Excluding("Hi kind zero T", invariant.Bucket_Hi(kind_axis), invariant.Bucket_True(kind_is_zero)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Function_label is non-empty bounded"})
	fl_mask := invariant.Sometimes(len(requirement.Kind) == min_inside_if_kind_chars, "Kind is zero_int sometimes")
	invariant.Cross_Product(fl, fl_mask,
		invariant.Excluding("Hi fl mask T", invariant.Bucket_Hi(fl), invariant.Bucket_True(fl_mask)),
		invariant.Excluding("Hi fl mask F", invariant.Bucket_Hi(fl), invariant.Bucket_False(fl_mask)),
	)
	path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Identifier_Path),
		Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Identifier_path is non-empty bounded"})
	path_mask := invariant.Sometimes(len(requirement.Kind) == min_inside_if_kind_chars, "Kind is zero_int sometimes")
	invariant.Cross_Product(path_axis, path_mask,
		invariant.Excluding("Hi path mask T", invariant.Bucket_Hi(path_axis), invariant.Bucket_True(path_mask)),
		invariant.Excluding("Hi path mask F", invariant.Bucket_Hi(path_axis), invariant.Bucket_False(path_mask)),
	)
	field_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Field_Description),
		Lo: min_inside_if_field_chars, Hi: max_field_description_chars,
		Message: "Field_description is `<name> <type>` for a non-nillable inside-if leaf"})
	field_mask := invariant.Sometimes(len(requirement.Kind) == min_inside_if_kind_chars, "Kind is zero_int sometimes")
	invariant.Cross_Product(field_axis, field_mask,
		invariant.Excluding("Hi field mask T", invariant.Bucket_Hi(field_axis), invariant.Bucket_True(field_mask)),
		invariant.Excluding("Hi field mask F", invariant.Bucket_Hi(field_axis), invariant.Bucket_False(field_mask)),
	)
...
func check_invariant_assertions_coverage_status(
	...
	defer func() {
		...
		cv := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by budget"})
	...
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
	k := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Kind),
		Lo: min_credit_kind_chars, Hi: max_credit_kind_chars, Message: "Kind is in credit_kind range"})
...
func check_invariant_assertions_build_diagnostic(
	...
	defer func() {
		...
		want_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Want),
			Lo: min_invariant_suggestion_chars, Hi: max_invariant_suggestion_chars,
			Message: "Diag.Want is the suggestion text from requirement_suggestion"})
		invariant.Cross_Product(invariant.Always(diag.Tier == 0, "Tier is 0 at construction"),
			want_axis,
			invariant.Always(diag.Name != "", "Diag.Name is non-empty for missing-axis diagnostics"),
			invariant.Always(len(diag.Name) == missing_diagnostic_name_chars, "Diag.Name is the fixed missing-axis label"),
			invariant.Always(diag.Message != "", "Diag.Message is non-empty for missing-axis diagnostics"))
	}()
	nillable_axis := invariant.Sometimes(requirement.Has_Nillable_Ancestor,
	...
	return Diagnostic{
		...
		Message: fmt.Sprintf("%s: %s `%s` missing invariant %s assertion: %s",
			requirement.Function_Label, prefix, requirement.Field_Description, requirement.Kind, suggestion),
	}
}
...
func check_invariant_assertions_requirement_suggestion(
	...
	defer func() {
		invariant.Cross_Product(invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(suggestion), Lo: min_invariant_suggestion_chars, Hi: max_invariant_suggestion_chars,
	...
	field_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(requirement.Field_Description),
		...
		Message: "Field_description spans short pointer to long slice typename"})
	invariant.Cross_Product(field_axis)
	axis := check_invariant_assertions_suggested_call(&check_invariant_assertions_suggested_call_input{
		Helper_Name:       check_invariant_assertions_bare_composable_for_kind(requirement.Kind),
...
func check_invariant_assertions_bare_composable_for_kind(kind string) (composable string) {
	defer func() {
		invariant.Cross_Product(
			invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
				X: len(composable), Lo: min_composable_helper_chars, Hi: max_composable_helper_chars,
				Message: "Composable is `Always` (6), `Sometimes` (9), or `Distinct_Boundary` (17)",
			}),
		)
	}()
	invariant.Cross_Product(
...
func check_invariant_assertions_suggested_call(input *check_invariant_assertions_suggested_call_input) (call string) {
	defer func() {
		invariant.Cross_Product(invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(call), Lo: min_suggested_axis_call_chars, Hi: max_suggested_axis_call_chars,
			Message: "Call is the suggested axis-builder template string for one requirement"}))
		hn := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Helper_Name),
			Lo: min_composable_helper_chars, Hi: max_composable_helper_chars, Message: "Helper_name is bounded"})
		invariant.Cross_Product(hn)
		k := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Kind),
			Lo: min_credit_kind_chars, Hi: max_credit_kind_chars, Message: "Kind is in credit_kind range"})
		invariant.Cross_Product(k)
		path_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Identifier_Path),
			Lo: min_non_empty, Hi: Max_Identifier_Chars, Message: "Identifier_path is non-empty bounded"})
		invariant.Cross_Product(path_axis)
		field_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Field_Description),
			Lo: min_requirement_field_description_chars, Hi: max_field_description_chars,
			Message: "Field_description spans short pointer to long slice typename"})
		invariant.Cross_Product(field_axis)
	}()
	...
	case "zero_int":
		...
		return "invariant." + input.Helper_Name + "(" + input.Identifier_Path + " == 0, \"...\")"
	case "zero_float":
		return "invariant." + input.Helper_Name + "(" + input.Identifier_Path + " == 0.0, \"...\")"
	case "zero_string":
		return "invariant." + input.Helper_Name + "(" + input.Identifier_Path + " == \"\", \"...\")"
	case "zero_slice":
		return "invariant." + input.Helper_Name + "(len(" + input.Identifier_Path + ") == 0, \"...\")"
	case "zero_map":
		return "invariant." + input.Helper_Name + "(len(" + input.Identifier_Path + ") == 0, \"...\")"
	case "nan_float":
		return "invariant." + input.Helper_Name + "(math.IsNaN(" + input.Identifier_Path + "), \"...\")"
	case "inf_float":
		return "invariant." + input.Helper_Name + "(math.IsInf(" + input.Identifier_Path + ", 0), \"...\")"
	case "bool":
		return "invariant." + input.Helper_Name + "(" + input.Identifier_Path + ")"
	case "pointer":
		...
		return "invariant." + input.Helper_Name + "(" + input.Identifier_Path + " != nil, \"...\")"
	}
	return "invariant." + input.Helper_Name + "(" + input.Identifier_Path + ")"
}
...
func check_invariant_assertions_validate_declarations(
	...
	defer func() {
		...
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Invariant_Idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Invariant_Idents is bounded"})
		sl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Stdlib_Local_Names),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Stdlib_Local_Names is bounded"})
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Constant_Signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Constant_Signs is bounded"})
	...
	invariant.Cross_Product(
		...
		invariant.Always(input.Function != nil, "Function is non-nil"),
		ii, sl, cs, fl,
		invariant.Excluding("Hi stdlib unreachable (Lo idents)", invariant.Bucket_Hi(sl), invariant.Bucket_Lo(ii)),
		invariant.Excluding("Hi stdlib unreachable (Hi idents)", invariant.Bucket_Hi(sl), invariant.Bucket_Hi(ii)),
		invariant.Excluding("Hi signs unreachable (Lo idents)", invariant.Bucket_Hi(cs), invariant.Bucket_Lo(ii)),
		invariant.Excluding("Hi signs unreachable (Hi idents)", invariant.Bucket_Hi(cs), invariant.Bucket_Hi(ii)),
		invariant.Excluding("Hi idents unreachable (Lo label)", invariant.Bucket_Hi(ii), invariant.Bucket_Lo(fl)),
		invariant.Excluding("Hi idents unreachable (Hi label)", invariant.Bucket_Hi(ii), invariant.Bucket_Hi(fl)),
	)
	...
	invariant.Cross_Product(invariant.Sometimes(obligations == nil, "Obligations can be empty or zero on this branch"))
	for _, obligation := range obligations {
		...
		invariant.Cross_Product(invariant.Sometimes(diag == nil, "Diag is nil when the obligation is satisfied"))
		if diag != nil {
			diags = append(diags, *diag)
		}
	}
...
func check_invariant_assertions_collect_declaration_obligations(
	...
	defer func() {
		o := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by budget"})
		sn := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
	...
	sn := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
	stdlib_empty_axis := invariant.Sometimes(len(stdlib_imports) == 0, "Stdlib_imports is empty sometimes")
	label_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
		X:       len(function_label),
		Lo:      min_function_label_chars,
		Hi:      Max_Identifier_Chars,
	...
	invariant.Cross_Product(
		invariant.Always(function != nil, "Function is non-nil"),
		invariant.Always(function_label != "", "Function_label is non-empty per upstream gate"),
	...
	for len(stack) > 0 {
		...
		for i_index, statement := range top {
			...
			stack = append(stack, check_invariant_assertions_descend(&check_invariant_assertions_descend_input{
				Statement:          statement,
				Function_Label:     function_label,
...
func check_invariant_assertions_descend(
	...
	defer func() {
		u := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(nested_lists),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Nested_lists is bounded by budget"})
		empty_axis := invariant.Sometimes(len(nested_lists) == 0, "Nested_lists is empty sometimes")
		ob := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Obligations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by AST budget"})
		stdlib := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Stdlib_Local_Names),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Stdlib_Local_Names is bounded by AST budget"})
		invariant.Cross_Product(u, empty_axis, ob, stdlib,
			invariant.Excluding("Max u contradicts empty true", invariant.Bucket_Hi(u), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Axis u at safety cap is bad", invariant.Bucket_Hi(u), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Zero u implies empty true", invariant.Bucket_Lo(u), invariant.Bucket_False(empty_axis)),
			invariant.Excluding("Hi u with Hi ob is bad", invariant.Bucket_Hi(u), invariant.Bucket_Hi(ob)),
			invariant.Excluding("Lo u with Hi ob unreachable", invariant.Bucket_Lo(u), invariant.Bucket_Hi(ob)),
			invariant.Excluding("Hi stdlib with empty u", invariant.Bucket_Hi(stdlib), invariant.Bucket_True(empty_axis)),
			invariant.Excluding("Hi stdlib with u", invariant.Bucket_Hi(stdlib), invariant.Bucket_False(empty_axis)),
		)
	}()
	ob := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Obligations),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by AST budget"})
	ob_empty := invariant.Sometimes(len(*input.Obligations) == 0, "Obligations is empty sometimes")
	stdlib := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Stdlib_Local_Names),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Stdlib_Local_Names is bounded by AST budget"})
	invariant.Cross_Product(
		invariant.Always(input != nil, "Input is non-nil"), invariant.Always(input.Obligations != nil, "Obligations is non-nil"),
		ob, ob_empty, stdlib,
		invariant.Excluding("Hi ob with empty unreachable", invariant.Bucket_Hi(ob), invariant.Bucket_True(ob_empty)),
		invariant.Excluding("Hi ob with non-empty unreachable", invariant.Bucket_Hi(ob), invariant.Bucket_False(ob_empty)),
		invariant.Excluding("Lo ob implies empty true", invariant.Bucket_Lo(ob), invariant.Bucket_False(ob_empty)),
		invariant.Excluding("Hi stdlib with empty obligations", invariant.Bucket_Hi(stdlib), invariant.Bucket_True(ob_empty)),
		invariant.Excluding("Hi stdlib with non-empty obligations", invariant.Bucket_Hi(stdlib), invariant.Bucket_False(ob_empty)),
	)
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
		Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Function_label is non-empty bounded"})
	invariant.Cross_Product(fl)
...
func check_invariant_assertions_descend_if(s *ast.IfStmt, input *check_invariant_assertions_descend_input) {
	defer func() {
		ob := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Obligations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by AST budget"})
		...
		label_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Input.Function_Label is non-empty bounded by identifier budget"})
		stdlib_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Stdlib_Local_Names),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Input.Stdlib_Local_Names is bounded by AST budget"})
	...
	identifiers := check_invariant_assertions_declaration_identifiers(s.Init, input.Stdlib_Local_Names)
	invariant.Cross_Product(invariant.Sometimes(identifiers == nil, "Identifiers is nil for non-decl-from-call init"))
	if identifiers == nil {
		return
	}
	*input.Obligations = append(*input.Obligations,
		check_invariant_assertions_obligation_from_body(s.Init, identifiers, input.Function_Label, s.Body))
...
func check_invariant_assertions_descend_for(s *ast.ForStmt, input *check_invariant_assertions_descend_input) {
	defer func() {
		ob := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Obligations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by AST budget"})
		...
		invariant.Cross_Product(ob, has_init, label_axis, stdlib_axis,
			...
			invariant.Excluding("Hi stdlib unreachable (init absent)",
				invariant.Bucket_Hi(stdlib_axis), invariant.Bucket_False(has_init)),
			invariant.Excluding("For without init at Lo obligations unreachable in test corpus (Lo label)",
				invariant.Bucket_False(has_init), invariant.Bucket_Lo(label_axis), invariant.Bucket_Lo(ob)),
			invariant.Excluding("For without init at Lo obligations unreachable in test corpus (Hi label)",
				invariant.Bucket_False(has_init), invariant.Bucket_Hi(label_axis), invariant.Bucket_Lo(ob)),
			invariant.Excluding("For with init at Hi label and Lo obligations unreachable in test corpus",
				invariant.Bucket_True(has_init), invariant.Bucket_Hi(label_axis), invariant.Bucket_Lo(ob)),
		)
	}()
	ob := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Obligations),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by AST budget"})
...
func check_invariant_assertions_descend_switch(s *ast.SwitchStmt, input *check_invariant_assertions_descend_input) {
	defer func() {
		ob := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(*input.Obligations),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Obligations is bounded by AST budget"})
	...
	invariant.Cross_Product(
		...
		invariant.Excluding("Maximal label observed only with accumulated obligations",
			invariant.Bucket_Hi(fl), invariant.Bucket_Lo(ob), invariant.Bucket_Lo(stdlib)),
	)
	if s.Init == nil {
		return
	}
	...
	for _, clause := range s.Body.List {
		...
		*input.Obligations = append(*input.Obligations,
			check_invariant_assertions_obligation_from_case(s.Init, identifiers, input.Function_Label, case_clause))
	}
}
...
func check_invariant_assertions_obligation_from_body(
	...
	defer func() {
		d := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(identifiers),
			Lo: min_non_empty, Hi: max_string_slice_per_call, Message: "Identifiers is ≥1 per caller gate"})
		single_axis := invariant.Sometimes(len(identifiers) == count_one, "Identifiers has exactly one entry sometimes")
		...
		fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Function_Label),
			Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Obligation.function_label is non-empty bounded"})
		invariant.Cross_Product(fl)
		idents := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Identifiers),
			Lo: count_one, Hi: min_pair, Message: "Obligation.identifiers is 1 (single LHS) or 2 (multi-LHS)"})
		invariant.Cross_Product(idents)
		succ := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Successor_Statements),
			Lo: 0, Hi: min_pair, Message: "Obligation.successor_statements is 0 (empty body) or 2 (multi-statement body)"})
		invariant.Cross_Product(succ)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(identifiers),
		Lo: min_non_empty, Hi: max_string_slice_per_call, Message: "Identifiers is ≥1 per caller gate"})
	label_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
		X:       len(function_label),
		Lo:      min_function_label_chars,
...
func check_invariant_assertions_obligation_from_case(
	...
	defer func() {
		...
		invariant.Cross_Product(idents)
		succ := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Successor_Statements),
			Lo: 0, Hi: min_pair, Message: "Obligation.successor_statements is 0 (empty case) or 2 (multi-statement case)"})
		invariant.Cross_Product(succ)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(identifiers),
		Lo: min_non_empty, Hi: max_string_slice_per_call, Message: "Identifiers is ≥1 per caller gate"})
...
func check_invariant_assertions_append_else_obligations(
	...
	defer func() {
		...
		ids := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Identifiers),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call,
			Message: "Identifiers is non-empty (short-decl appends ≥1)"})
		label := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(input.Function_Label),
			Lo: min_non_empty, Hi: Max_Identifier_Chars,
		...
		invariant.Cross_Product(ob, has_else, ids, label,
			...
			invariant.Excluding("Obligations Lo contradicts else (which appended an obligation)",
	...
	invariant.Cross_Product(
		...
		invariant.Excluding("Obligations at safety cap without else branch unreachable",
			invariant.Bucket_Hi(ob), invariant.Bucket_False(has_else)),
		invariant.Excluding("Hi identifiers unreachable", invariant.Bucket_Hi(ids), invariant.Bucket_True(has_else)),
		invariant.Excluding("Hi identifiers unreachable", invariant.Bucket_Hi(ids), invariant.Bucket_False(has_else)),
		invariant.Excluding("Lo ids with Hi label and else unreachable in test corpus",
			invariant.Bucket_Lo(ids), invariant.Bucket_Hi(label), invariant.Bucket_True(has_else)),
	)
	...
	for range invariant.Game_Loop() {
		...
		if block, is_block := current.(*ast.BlockStmt); is_block {
			*input.Obligations = append(*input.Obligations,
				check_invariant_assertions_obligation_from_body(
					input.Init_Statement, input.Identifiers, input.Function_Label, block))
			return
		}
		...
		if else_if.Body != nil {
			*input.Obligations = append(*input.Obligations,
				check_invariant_assertions_obligation_from_body(
					input.Init_Statement, input.Identifiers, input.Function_Label, else_if.Body))
		}
...
func check_invariant_assertions_declaration_identifiers(
	...
	defer func() {
		i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(identifiers),
			Lo: 0, Hi: max_string_slice_per_call, Message: "Identifiers is bounded by budget"})
		nil_axis := invariant.Sometimes(identifiers == nil, "Identifiers is nil for non-decl-from-call statements")
		sn := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
		invariant.Cross_Product(i, nil_axis, sn,
			invariant.Excluding("Max i contradicts nil true", invariant.Bucket_Hi(i), invariant.Bucket_True(nil_axis)),
			invariant.Excluding("Axis i at safety cap is bad", invariant.Bucket_Hi(i), invariant.Bucket_False(nil_axis)),
			invariant.Excluding("Max sn contradicts nil true", invariant.Bucket_Hi(sn), invariant.Bucket_True(nil_axis)),
			invariant.Excluding("Axis sn at safety cap is bad", invariant.Bucket_Hi(sn), invariant.Bucket_False(nil_axis)),
			invariant.Excluding("Zero i with non-nil and zero sn is impossible",
				invariant.Bucket_Lo(i), invariant.Bucket_False(nil_axis), invariant.Bucket_Lo(sn)),
		)
	}()
	sn := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
	empty_axis := invariant.Sometimes(len(stdlib_imports) == 0, "Stdlib_imports is empty sometimes")
	invariant.Cross_Product(sn, empty_axis,
		invariant.Excluding("Max sn contradicts empty true", invariant.Bucket_Hi(sn), invariant.Bucket_True(empty_axis)),
		invariant.Excluding("Axis sn at safety cap is bad", invariant.Bucket_Hi(sn), invariant.Bucket_False(empty_axis)),
		invariant.Excluding("Zero sn implies empty true", invariant.Bucket_Lo(sn), invariant.Bucket_False(empty_axis)),
	)
...
func check_invariant_assertions_call_is_exempt(
	...
	defer func() {
		s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(stdlib_imports),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Stdlib_imports is bounded by budget"})
...
func check_invariant_assertions_is_axis_builder(selector_name string) (yes bool) {
	...
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(selector_name), Lo: min_composable_helper_chars, Hi: max_invariant_selector_chars,
			Message: "Selector_name covers `Always` (6) through `Recorder_Distinct_Boundary` (26)",
		}),
	)
...
func check_invariant_assertions_collect_stdlib_imports(
	...
	defer func() {
		n := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(names),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Names is bounded by budget"})
		empty_axis := invariant.Sometimes(len(names) == 0, "Names is empty sometimes")
		invariant.Cross_Product(n, empty_axis,
			invariant.Excluding("Max n contradicts empty true",
				invariant.Bucket_Hi(n), invariant.Bucket_True(empty_axis)),
...
func check_invariant_assertions_import_path_is_stdlib(import_path string) (yes bool) {
	...
	invariant.Cross_Product(
		invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
			X: len(import_path), Lo: min_non_empty, Hi: Max_Identifier_Chars,
			Message: "Import_path is non-empty per Go import grammar and bounded by identifier budget",
		}),
	)
...
func check_invariant_assertions_paths_covered_by_call(
	...
	defer func() {
		...
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		p := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(paths),
			Lo: 0, Hi: max_coverage_pairs_per_call, Message: "Paths is bounded by budget"})
		from_cross_axis := invariant.Sometimes(from_cross, "From_cross is true when a Cross_Product covers the path")
		invariant.Cross_Product(
			...
			invariant.Excluding("Zero constants with max pairs is bad",
				invariant.Bucket_Lo(c), invariant.Bucket_Hi(p)),
			invariant.Excluding("From_cross_product true with all zero counts is impossible",
				invariant.Bucket_True(from_cross_axis),
				invariant.Bucket_Lo(i), invariant.Bucket_Lo(c), invariant.Bucket_Lo(p)),
		)
	}()
	i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	...
	pairs := check_invariant_assertions_call_covered_pairs(call, invariant_idents, constant_signs)
	invariant.Cross_Product(invariant.Sometimes(pairs == nil, "Pairs is nil for unrecognised calls"))
	if pairs == nil {
		return nil, false
	}
...
func check_invariant_assertions_validate_declaration_obligation(
	...
	defer func() {
		...
		if diag != nil {
			want_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Want),
				Lo: min_declaration_diagnostic_want_chars, Hi: max_declaration_diagnostic_want_chars,
				Message: "Diag.Want is single-LHS or multi-LHS suggestion"})
			invariant.Cross_Product(invariant.Always(diag.Tier == 0, "Tier is 0 at construction"),
				want_axis,
				invariant.Always(diag.Name != "", "Diag.Name is non-empty for declaration diagnostics"),
				invariant.Always(len(diag.Name) == declaration_diagnostic_name_chars, "Diag.Name is the fixed decl label"),
				invariant.Always(diag.Message != "", "Diag.Message is non-empty for declaration diagnostics"))
		}
		fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Function_Label),
			Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Obligation.function_label is non-empty bounded"})
		invariant.Cross_Product(fl)
		idents := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Identifiers),
			Lo: count_one, Hi: max_obligation_identifiers, Message: "Obligation.identifiers is 1 (single LHS) to 3"})
		invariant.Cross_Product(idents)
		succ := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Successor_Statements),
			Lo: 0, Hi: max_successor_statements, Message: "Successor_statements spans 0 to 30 (many-stmt fixture)"})
		invariant.Cross_Product(succ)
		ii := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by AST budget"})
		ii_empty := invariant.Sometimes(len(invariant_idents) == 0, "Invariant_idents is empty sometimes")
		...
		cs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by AST budget"})
		cs_empty := invariant.Sometimes(len(constant_signs) == 0, "Constant_signs is empty sometimes")
		invariant.Cross_Product(cs, cs_empty,
			invariant.Excluding("Hi cs with empty unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_True(cs_empty)),
			invariant.Excluding("Hi cs with non-empty unreachable", invariant.Bucket_Hi(cs), invariant.Bucket_False(cs_empty)),
			invariant.Excluding("Lo cs implies empty true", invariant.Bucket_Lo(cs), invariant.Bucket_False(cs_empty)),
		)
	}()
	invariant.Cross_Product(invariant.Always(file_set != nil, "File_set is non-nil"))
	fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Function_Label),
	...
	if len(obligation.Successor_Statements) == 0 {
		return check_invariant_assertions_build_declaration_diagnostic_pointer(file_set, obligation)
	}
	...
	covered_axis := invariant.Sometimes(covered == nil, "Covered is nil for non-invariant successor chains")
	chain_axis := invariant.Sometimes(chain_includes_cross, "Chain includes a top-level Cross_Product")
...
func check_invariant_assertions_chain_covered_paths(
	...
	defer func() {
		s := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(successors),
			Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Successors is ≥1 per caller gate"})
		di := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
		die := invariant.Sometimes(len(invariant_idents) == 0, "Invariant_idents is empty sometimes")
		ds := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		dse := invariant.Sometimes(len(constant_signs) == 0, "Constant_signs is empty sometimes")
		dc := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by budget"})
		dce := invariant.Sometimes(len(covered) == 0, "Covered is empty sometimes")
		ca := invariant.Sometimes(covered == nil, "Covered is nil for non-invariant successor statements")
		cha := invariant.Sometimes(chain_includes_cross, "Chain includes a top-level Cross_Product")
		invariant.Cross_Product(s, ca, cha,
			...
			invariant.Excluding("Max s contradicts covered true", invariant.Bucket_Hi(s), invariant.Bucket_True(ca)),
			invariant.Excluding("Axis s at safety cap is bad", invariant.Bucket_Hi(s), invariant.Bucket_False(ca)),
			invariant.Excluding("Zero s with chain false implies covered nil",
				invariant.Bucket_Lo(s), invariant.Bucket_False(ca), invariant.Bucket_False(cha)),
		)
		invariant.Cross_Product(di, die,
			invariant.Excluding("Max di contradicts die true", invariant.Bucket_Hi(di), invariant.Bucket_True(die)),
			invariant.Excluding("Axis di at safety cap is bad", invariant.Bucket_Hi(di), invariant.Bucket_False(die)),
			invariant.Excluding("Zero di implies die true", invariant.Bucket_Lo(di), invariant.Bucket_False(die)),
		)
		invariant.Cross_Product(ds, dse,
			invariant.Excluding("Max ds contradicts dse true", invariant.Bucket_Hi(ds), invariant.Bucket_True(dse)),
			invariant.Excluding("Axis ds at safety cap is bad", invariant.Bucket_Hi(ds), invariant.Bucket_False(dse)),
			invariant.Excluding("Zero ds implies dse true", invariant.Bucket_Lo(ds), invariant.Bucket_False(dse)),
		)
		invariant.Cross_Product(dc, dce,
			invariant.Excluding("Max dc contradicts dce true", invariant.Bucket_Hi(dc), invariant.Bucket_True(dce)),
			invariant.Excluding("Axis dc at safety cap is bad", invariant.Bucket_Hi(dc), invariant.Bucket_False(dce)),
			invariant.Excluding("Zero dc implies dce true", invariant.Bucket_Lo(dc), invariant.Bucket_False(dce)),
		)
	}()
	prologue_successors := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(successors),
		Lo: min_non_empty, Hi: max_ast_nodes_per_call, Message: "Successors is ≥1 per caller gate"})
	prologue_idents := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	prologue_signs := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
	invariant.Cross_Product(prologue_successors, prologue_idents, prologue_signs,
		invariant.Excluding("Both successors and idents at safety caps is bad",
			invariant.Bucket_Hi(prologue_successors), invariant.Bucket_Hi(prologue_idents)),
		invariant.Excluding("Max successors with zero idents is bad",
			invariant.Bucket_Hi(prologue_successors), invariant.Bucket_Lo(prologue_idents)),
		invariant.Excluding("Zero successors with max idents is bad",
			invariant.Bucket_Lo(prologue_successors), invariant.Bucket_Hi(prologue_idents)),
	...
	for _, statement := range successors {
		paths, statement_is_cross := check_invariant_assertions_statement_covered_paths(statement, invariant_idents, constant_signs)
		paths_axis := invariant.Sometimes(paths == nil, "Paths is nil for non-coverage statements")
		cross_axis := invariant.Sometimes(statement_is_cross, "Statement is a top-level Cross_Product")
		...
		invariant.Cross_Product(
			...
			invariant.Excluding("Paths nil implies cross_axis false by construction",
				invariant.Bucket_True(paths_axis), invariant.Bucket_True(cross_axis)),
		)
		if paths == nil {
			if covered == nil {
...
func check_invariant_assertions_statement_covered_paths(
	...
	defer func() {
		paths_axis := invariant.Sometimes(paths == nil, "Paths is nil for non-coverage statements")
		cross_axis := invariant.Sometimes(is_cross_product, "Statement is a top-level Cross_Product")
		i := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
		c := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
		...
		invariant.Cross_Product(paths_axis, cross_axis, i, c, p,
			invariant.Excluding("Paths nil implies cross_axis false",
				invariant.Bucket_True(paths_axis), invariant.Bucket_True(cross_axis)),
			invariant.Excluding("Paths false with min i is bad", invariant.Bucket_False(paths_axis), invariant.Bucket_Lo(i)),
			invariant.Excluding("Both i and c at safety caps is bad", invariant.Bucket_Hi(i), invariant.Bucket_Hi(c)),
			invariant.Excluding("Axis i at safety cap is bad", invariant.Bucket_Hi(i), invariant.Bucket_Lo(c)),
			invariant.Excluding("Axis c at safety cap is bad", invariant.Bucket_Lo(i), invariant.Bucket_Hi(c)),
			invariant.Excluding("Both c and pairs at safety caps", invariant.Bucket_Hi(c), invariant.Bucket_Hi(p)),
			invariant.Excluding("Axis c at safety cap with zero pairs", invariant.Bucket_Hi(c), invariant.Bucket_Lo(p)),
			invariant.Excluding("Zero c with max pairs is bad", invariant.Bucket_Lo(c), invariant.Bucket_Hi(p)),
		)
	}()
	pi := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(invariant_idents),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Invariant_idents is bounded by budget"})
	ps := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(constant_signs),
		Lo: 0, Hi: max_ast_nodes_per_call, Message: "Constant_signs is bounded by budget"})
	invariant.Cross_Product(pi, ps,
		invariant.Excluding("Both prologue idents and signs at safety caps", invariant.Bucket_Hi(pi), invariant.Bucket_Hi(ps)),
		invariant.Excluding("Invariant_idents at safety cap is bad", invariant.Bucket_Hi(pi), invariant.Bucket_Lo(ps)),
		invariant.Excluding("Zero idents with max signs is bad", invariant.Bucket_Lo(pi), invariant.Bucket_Hi(ps)),
	)
	if expression_statement, is_expression := statement.(*ast.ExprStmt); is_expression {
		call, is_call := expression_statement.X.(*ast.CallExpr)
		if !is_call {
		...
		covered, _ := check_invariant_assertions_paths_covered_by_call(call, invariant_idents, constant_signs)
		invariant.Cross_Product(invariant.Sometimes(covered == nil, "Covered is nil for non-invariant calls"))
		call_name := check_invariant_assertions_extract_call_name(call, invariant_idents)
		invariant.Cross_Product(invariant.Sometimes(call_name == "", "Call_name is empty for non-invariant calls"))
	...
	invariant.Cross_Product(invariant.Always(bare_table != nil, "Bare_table is non-nil at this point"))
	if _, is_bare := bare_table[call_name]; !is_bare {
		return nil, false
	}
	paths = map[string]bool{}
...
func check_invariant_assertions_build_declaration_diagnostic_pointer(
	...
	defer func() {
		...
		invariant.Cross_Product(
			...
			want_axis,
			invariant.Always(diag.Name != "", "Diag.Name is non-empty for declaration diagnostics"),
			invariant.Always(len(diag.Name) == declaration_diagnostic_name_chars, "Diag.Name is the fixed declaration label"),
			invariant.Always(diag.Message != "", "Diag.Message is non-empty for declaration diagnostics"),
		)
		message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Message),
			Lo: min_declaration_diagnostic_message_chars, Hi: max_diagnostic_message_chars,
		...
		invariant.Cross_Product(message_axis, message_multi,
			invariant.Excluding("Hi message unreachable (single-LHS)",
				invariant.Bucket_Hi(message_axis), invariant.Bucket_False(message_multi)),
			invariant.Excluding("Hi message unreachable (multi-LHS)",
				invariant.Bucket_Hi(message_axis), invariant.Bucket_True(message_multi)),
			invariant.Excluding("Lo message implies single-LHS",
				invariant.Bucket_Lo(message_axis), invariant.Bucket_True(message_multi)))
		fl := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Function_Label),
			Lo: min_function_label_chars, Hi: Max_Identifier_Chars, Message: "Obligation.function_label is non-empty bounded"})
		invariant.Cross_Product(fl)
		idents := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(obligation.Identifiers),
	...
	invariant.Cross_Product(invariant.Always(value.Message != "", "Value.Message is non-empty at this point"))
	return &value
}
...
// field instead.
func check_invariant_assertions_path_covered(covered map[string]bool, identifier string) (yes bool) {
	defer func() {
		cv := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(covered),
			Lo: 0, Hi: max_ast_nodes_per_call, Message: "Covered is bounded by budget"})
	...
	ident_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{
		X: len(identifier), Lo: min_non_empty, Hi: max_if_init_identifier_chars,
		Message: "Identifier is a non-empty Go identifier; capped at 55 chars fitting in if/for/switch init line budget",
	})
	invariant.Cross_Product(
		ident_axis, cv,
...
func check_invariant_assertions_build_declaration_diagnostic(
	...
	suggestion := "add an invariant assertion as the very next statement covering: " + identifier_list
	message := fmt.Sprintf(
		"%s: declaration via function call must be followed by an invariant assertion covering: %s",
...
func check_no_unbounded_apis_lookup(file_set *token.FileSet, selector *ast.SelectorExpr) (diag Diagnostic, found bool) {
	defer func() {
		found_axis := invariant.Sometimes(found, "Lookup matched a banned identifier")
		name_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Name),
			Lo: 0, Hi: Max_Identifier_Chars, Message: "Diag.Name is bounded by identifier budget"})
		want_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Want),
			Lo: 0, Hi: max_filesystem_path_chars, Message: "Diag.Want is bounded by path budget"})
		message_axis := invariant.Distinct_Boundary(&invariant.Boundary_Input[int]{X: len(diag.Message),
			Lo: 0, Hi: max_diagnostic_message_chars, Message: "Diag.Message is bounded by message budget"})
		invariant.Cross_Product(invariant.Always(diag.Tier == 0, "Tier is zero"), found_axis, name_axis, want_axis, message_axis,
			invariant.Excluding("Hi name unreachable (hit)", invariant.Bucket_Hi(name_axis), invariant.Bucket_True(found_axis)),
			invariant.Excluding("Hi name on miss", invariant.Bucket_Hi(name_axis), invariant.Bucket_False(found_axis)),
			invariant.Excluding("Lo name impossible (hit)", invariant.Bucket_Lo(name_axis), invariant.Bucket_True(found_axis)),
			invariant.Excluding("Hi want unreachable (hit)", invariant.Bucket_Hi(want_axis), invariant.Bucket_True(found_axis)),
			invariant.Excluding("Hi want on miss", invariant.Bucket_Hi(want_axis), invariant.Bucket_False(found_axis)),
			invariant.Excluding("Lo want impossible (hit)", invariant.Bucket_Lo(want_axis), invariant.Bucket_True(found_axis)),
			invariant.Excluding("Hi msg hit", invariant.Bucket_Hi(message_axis), invariant.Bucket_True(found_axis)),
			invariant.Excluding("Hi msg miss", invariant.Bucket_Hi(message_axis), invariant.Bucket_False(found_axis)),
			invariant.Excluding("Lo msg hit", invariant.Bucket_Lo(message_axis), invariant.Bucket_True(found_axis)),
		)
	}()
	invariant.Cross_Product(invariant.Always(file_set != nil, "File_set is non-nil"),
	...
	var category, substitution string
	switch qualified {
	case "io.ReadAll":
		category, substitution = "unbounded-read", "io.ReadFull(r, buf) with a bounded buf"
	case "io.Copy", "io.CopyBuffer":
		category, substitution = "unbounded-read", "io.CopyN(dst, src, N)"
	case "os.ReadFile":
		category, substitution = "unbounded-read", "os.Open + io.ReadFull(io.LimitReader(f, N), buf)"
	case "os.ReadDir":
		category, substitution = "unbounded-read", "a bounded-slice helper that caps the result"
	case "bufio.NewScanner", "bufio.NewReader":
		category, substitution = "unbounded-read", "r.Read(buf) with a fixed buf"
	case "json.NewDecoder":
		category, substitution = "unbounded-decode", "json.Unmarshal over a bounded []byte"
	case "xml.NewDecoder":
		category, substitution = "unbounded-decode", "xml.Unmarshal over a bounded []byte"
	case "gob.NewDecoder":
		category, substitution = "unbounded-decode", "bounded read into []byte, then decode manually"
	case "csv.NewReader":
		category, substitution = "unbounded-decode", "bounded read + manual parse"
	case "gzip.NewReader", "flate.NewReader", "zlib.NewReader", "bzip2.NewReader", "lzw.NewReader":
		category, substitution = "unbounded-decompression", "wrap the decompressed reader in io.LimitReader"
	case "zip.NewReader", "zip.OpenReader":
		category, substitution = "unbounded-decompression", "check UncompressedSize64 against a literal cap before reading"
	case "tar.NewReader":
		category, substitution = "unbounded-decompression", "check Header.Size against a literal cap before reading"
	case "bytes.NewBuffer", "bytes.NewBufferString":
		category, substitution = "unbounded-allocation", "a fixed []byte with explicit length tracking"
	case "http.Get":
		category, substitution = "unbounded-http", "(&http.Client{Timeout: N}).Get(...)"
	case "http.Post":
		category, substitution = "unbounded-http", "(&http.Client{Timeout: N}).Post(...)"
	case "http.PostForm":
		category, substitution = "unbounded-http", "(&http.Client{Timeout: N}).PostForm(...)"
	case "http.Head":
		category, substitution = "unbounded-http", "(&http.Client{Timeout: N}).Head(...)"
	case "http.ListenAndServe", "http.ListenAndServeTLS":
		category, substitution = "unbounded-http", "explicit http.Server with timeouts and MaxHeaderBytes set"
	case "http.DefaultClient":
		category, substitution = "unbounded-http", "an explicit http.Client with Timeout set"
	case "http.DefaultServeMux":
		category, substitution = "unbounded-http", "an explicit http.ServeMux"
	case "http.DefaultTransport":
		category, substitution = "unbounded-http", "an explicit http.Transport"
	case "ioutil.ReadAll":
		category, substitution = "deprecated-ioutil", "io.ReadFull(r, buf) with a bounded buf"
	case "ioutil.ReadFile":
		category, substitution = "deprecated-ioutil", "os.Open + io.ReadFull(io.LimitReader(f, N), buf)"
	case "ioutil.ReadDir":
		category, substitution = "deprecated-ioutil", "a bounded-slice helper"
	case "ioutil.WriteFile":
		category, substitution = "deprecated-ioutil", "os.WriteFile"
	case "ioutil.TempFile":
		category, substitution = "deprecated-ioutil", "os.CreateTemp"
	case "ioutil.TempDir":
		category, substitution = "deprecated-ioutil", "os.MkdirTemp"
	case "ioutil.NopCloser":
		category, substitution = "deprecated-ioutil", "io.NopCloser"
	case "ioutil.Discard":
		category, substitution = "deprecated-ioutil", "io.Discard"
	default:
		return Diagnostic{}, false
	}
	return Diagnostic{
		...
		Name:     qualified,
		Want:     substitution,
		Message:  fmt.Sprintf("%s: unbounded API '%s'; use %s instead", category, qualified, substitution),
	}, true
}
