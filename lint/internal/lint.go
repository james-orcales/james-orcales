...
func Main(input *Main_Input) (code int) {
	// Git tier runs first: it's repo-metadata-only, doesn't touch the FS,
	// and surfaces the fastest signal (rebase mistakes don't need a full
	// codebase walk to detect).
...
var (
	snake_case_re = regexp.MustCompile(`^[a-z][a-z0-9]*(_[a-z0-9]+)*$`)
	ada_case_re   = regexp.MustCompile(`^([A-Z][a-z0-9]*|[A-Z][A-Z0-9]*)(_([A-Z][a-z0-9]*|[A-Z][A-Z0-9]*))*$`)
	...
	conventional_commit_re = regexp.MustCompile(`^[a-z]+(\([^)]+\))?!?: \S`)
)
...
func check_file_checks_tier1() (checks []check_function) {
...
// Tier 2: checks that depend on tier-1 contracts. check_no_recursion's
// invariant.Ensure assumes check_shadows has rejected files where a local
// shadows a top-level function name. Running tier-2 on tier-1-dirty input
// would risk an invariant trip on otherwise-legitimate input.
func check_file_checks_tier2() (checks []check_function) {
	return []check_function{
		check_no_recursion,
		check_no_function_init,
		check_no_package_vars,
...
// file-system tier's per-file pass.
func Check_File(file_set *token.FileSet, file *ast.File, source []byte) (diags []Diagnostic) {
	diags = check_file_run_tier(check_file_checks_tier1(), file_set, file, source)
	if len(diags) > 0 {
		return diags
	}
	return check_file_run_tier(check_file_checks_tier2(), file_set, file, source)
}
...
func Check_File_System(input *Check_File_System_Input) (diags []Diagnostic, err error) {
	...
	// Run stream and AST tiers in series and union the diagnostics. The
	// earlier "suppress AST on stream fire" shortcut hid the package-split
	// rule whenever any stream check fired (e.g. a single path-casing
	// nit), which made the AST tier silently invisible on dirty trees.
	// Now: parse failures degrade to per-file diagnostics so a conflict
	// marker inside a Go file still gets named by the stream tier without
	// blowing up the whole AST pass.
	...
	file_diags := check_file_system_run_checks(parsed_files, cpu_count)
	package_diags := check_file_system_package_split(parsed_files)
	prefix_diags := check_file_system_method_prefix(parsed_files)
	binary_layout_diags := check_binary_module_layout(parsed_files, modules)
	shared_library_diags := check_shared_library_no_internal(parsed_files, modules)
	library_tier_diags := check_library_tier_depth(parsed_files, modules)
	ambient_diags := check_no_ambient_stdlib(parsed_files, modules)
	package_documentation_diags := check_package_documentation_comment(parsed_files)
	output := append([]Diagnostic{}, stream_diags...)
	output = append(output, parse_diags...)
	output = append(output, file_diags...)
	output = append(output, package_diags...)
	output = append(output, prefix_diags...)
	output = append(output, binary_layout_diags...)
	output = append(output, shared_library_diags...)
	output = append(output, library_tier_diags...)
	output = append(output, ambient_diags...)
	output = append(output, package_documentation_diags...)
	return output, nil
}
...
func git_input_check(input Git_Input) (diags []Diagnostic) {
	...
	for _, c := range input.Merge_Commits {
		if c.Subject == "" {
			continue
		}
	...
	for _, c := range input.Non_Merge_Commits {
		if c.Subject == "" {
			continue
		}
		if len(c.Subject) > max_commit_subject_chars {
...
// Truncates a git hash to 10 chars (git's default short-hash width). Pass-
// through for already-short or malformed inputs so test fixtures don't have
// to supply full 40-char hashes.
...
func check_file_system_method_prefix_group(files []parsed_file) (diags []Diagnostic) {
	...
	for _, pf := range files {
		for _, declaration := range pf.File.Decls {
			...
			type_name, named := check_file_system_method_prefix_group_first_parameter_type(first_parameter.Type)
			if !named {
				continue
			}
			...
			// Constructor-input exception: when the type is named
			// `<FuncName>_Input` (Ada_Case) or `<func_name>_input`
			// (snake_case), the type is named after the function, not the
			// other way around. Applying the prefix rule here would force
			// absurd names like `New_Input_New`. The check_input_struct
			// rule already governs this pattern.
...
func check_file_system_method_prefix_group_first_parameter_type(expression ast.Expr) (name string, ok bool) {
...
// it, whether that module is the shared lib, and which directories
...
func build_module_index(fsys fs.FS, parsed_files []parsed_file) (index *module_index, err error) {
	...
	// Directory_Package is populated only from non-test, non-main files because
	// the library-tier-depth rule asks "is there a non-main Go package in
	// this ancestor directory?". Test files and main packages are not
	// "library tier" ancestors and must not count toward the depth budget.
	for _, pf := range parsed_files {
		...
		// Canonicalize so v1/v2 sibling versions of the same library both
		// register at the same Directory_Package key — the depth rule
		// must see them as one tier, not two.
...
func module_index_resolve(file_path string, modules []module_information) (index int) {
	for i, m := range modules {
		...
		if strings.HasPrefix(file_path, m.Root+"/") {
			return i
		}
	}
	return -1
...
func check_comments(file_set *token.FileSet, file *ast.File, source []byte) (diags []Diagnostic) {
	for _, group := range file.Comments {
		diags = append(diags, check_comments_group(file_set, source, group)...)
	}
	return diags
}

func check_comments_group(
	file_set *token.FileSet,
	source []byte,
	...
	diags = append(diags,
		check_comments_group_capital(file_set, filtered[0])...)
	diags = append(diags,
		check_comments_group_terminator(
			file_set,
...
func check_no_recursion(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	caller_order, _, edges := build_file_call_graph(file_set, file)
	adj := map[string][]call_edge{}
	for _, e := range edges {
		adj[e.Caller] = append(adj[e.Caller], e)
	}
	return check_no_recursion_find_cycles(file_set, caller_order, adj)
}
...
func build_file_call_graph(
	...
	for _, declaration := range file.Decls {
		...
		caller_order = append(caller_order, function_declaration.Name.Name)
		function_names[function_declaration.Name.Name] = true
		decls[function_declaration.Name.Name] = function_declaration
	}
	for _, declaration := range file.Decls {
		function_declaration, is_function_declaration := declaration.(*ast.FuncDecl)
		if !is_function_declaration {
		...
		v := &recursion_visitor{File_Set: file_set, Caller: function_declaration.Name.Name, Targets: function_names, Edges: &edges}
		ast.Walk(v, function_declaration.Body)
	}
	return caller_order, decls, edges
}
...
func check_no_recursion_find_cycles_dfs_diag(
	...
	for i_index := 0; i_index < len(cycle_nodes)-1; i_index++ {
		...
		for _, ce := range adj[from] {
			if ce.Callee == to {
				invariant.Ensure(!ce.Shadowed, "check_no_recursion: cycle edge shadowed")
				break
			}
		}
	}
	invariant.Ensure(!back_edge.Shadowed, "check_no_recursion: back-edge shadowed")
	return Diagnostic{
...
func banned_segments_universal() (segments []string) {
	return []string{"util", "utils", "utility", "utilities", "length", "len"}
}
...
func check_banned_identifiers(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	package_hit := check_banned_identifiers_find_hit(file.Name.Name, banned_segments_universal())
	if package_hit != "" {
		diags = append(diags, Diagnostic{
			Position: file_set.Position(file.Name.Pos()),
...
func check_banned_identifiers_file_name(file_set *token.FileSet, file *ast.File) (diags []Diagnostic) {
	...
	hit := check_banned_identifiers_find_hit(stem, banned_segments_universal())
	if hit == "" {
		return nil
	}
	return []Diagnostic{{
...
func check_banned_identifiers_walk(file_set *token.FileSet, file *ast.File) (diags []Diagnostic) {
	validate_banned_name := func(pos token.Pos, scope, name string, extra []string) {
		hit := check_banned_identifiers_find_hit(name, extra, banned_segments_universal())
		if hit == "" {
			return
		}
		diags = append(diags, Diagnostic{
...
func check_no_package_vars(file_set *token.FileSet, file *ast.File, _ []byte) (diags []Diagnostic) {
	...
	for _, declaration := range file.Decls {
		...
		for _, specification := range generic_declaration.Specs {
			...
			if check_no_package_vars_is_interface_assertion(vs) {
				continue
			}
...
// `var _ Iface = (*Impl)(nil)`: exactly one blank name, an explicit type,
// and exactly one value. The type makes it a compile-time assertion, not
// a value holder.
func check_no_package_vars_is_interface_assertion(vs *ast.ValueSpec) (yes bool) {
	if vs.Type == nil {
		return false
	}
	if len(vs.Names) != 1 {
...
func check_exported_documentation_comment_generic(file_set *token.FileSet, generic_declaration *ast.GenDecl) (diags []Diagnostic) {
	switch generic_declaration.Tok {
	case token.TYPE, token.VAR, token.CONST:
	default:
		return nil
	}
	group_has_documentation := check_exported_documentation_comment_has_documentation(generic_declaration.Doc)
	kind := "var"
	switch generic_declaration.Tok {
	case token.TYPE:
		kind = "type"
...
type check_file_system_stream_checks_input struct {
	Root_Directory string
	Readlink       func(name string) (target string, err error)
	Stat           func(name string) (info fs.FileInfo, err error)
}

func check_file_system_stream_checks(input *check_file_system_stream_checks_input) (checks []check_function_stream) {
...
func check_file_system_stream(input *check_file_system_stream_input) (diags []Diagnostic, go_paths []string, err error) {
	checks := check_file_system_stream_checks(&check_file_system_stream_checks_input{
		Root_Directory: input.Root_Directory,
		Readlink:       input.Readlink,
		Stat:           input.Stat,
	})
	...
	err = fs.WalkDir(input.Fsys, input.Root, func(p string, d fs.DirEntry, walk_err error) (out error) {
		...
		// The stream walker visits third_party/ so copyleft can see
		// vendored licenses, but Go AST checks intentionally skip it:
		// vendored sources are upstream-owned and not subject to this
		// repo's style rules.
		if path.Ext(p) == ".go" {
	...
	for i, c := range checks {
		for _, d := range per_check[i] {
			// Prefix the check name so the existing %s: %s render
			// produces "file:line:col: <check>: <note>" — readable
			// without restructuring the Main render loop.
...
func check_stream_conflict_markers(p string, information fs.FileInfo, load func() (data []byte, err error), output *[]Diagnostic) {
	...
	// and yield mostly false positives.
