...
func Test_Import_Local_Name_Default_Tier(t *testing.T) {
	...
	got := source.Import_Local_Name(
		implementation, "example.com/shared/sim/aver/default")
	if got != "aver" {
...
func Test_Path_Matches_Glob_Later_Positive_Refines_Negation(t *testing.T) {
	t.Parallel()
	patterns := []string{"**", "!shared/**", "shared/sim/aver/**", "shared/vsr/**"}
	if !source.Path_Matches_Glob("shared/sim/aver/aver.go", patterns) {
		t.Error("shared/sim/aver/** must re-exempt its subtree")
	}
