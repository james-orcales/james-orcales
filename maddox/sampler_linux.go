...
func Proc_Path_Invariants(path Proc_Path, namespace invariant.Namespace) {
	invariant.Assertions(namespace).Range_Int(len(path), BOUND_MIN, BOUND_MAX).Ensure()
}
...
func Utsname_Field_Invariants[T int8 | uint8](
	...
	invariant.Assertions(namespace).Range_Int(len(field), BOUND_MIN, BOUND_MAX).Ensure()
}
